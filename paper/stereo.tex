...
the label tree or QPBO in the other cases.
%when either of the
...
solution pool for fusion. SF-SS implies $\beta=0$, where $\alpha$ is the
free parameter and set to 4. In this case one thread will fuse 4 labels,
together with current solution by TRW-S per iteration and never
exchanges solutions with other threads. We perform a multi-way fusion
of solutions from all the threads at the end to obtain a final solution
% at
...
%
%
To make the comparison simple, we restrict our solution proposals
to be constant-label proposals.
%
