...
that is, one iteration with ($\alpha=0, \beta=1$) in every 5 iterations and other iterations with ($\alpha=1, \beta=0$) for SF-MF, ($\alpha=3$, $\beta=0$) for SF-MF, and one iteration with ($\alpha=0, \beta=3$) in every 5 iterations and other iterations with ($\alpha=3, \beta=0$) for SF-MF.\hang{seems to have some repeating and incomplete sentences here}
