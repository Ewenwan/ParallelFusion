 ...
 FusionThread(const std::shared_ptr<ProposalGenerator<LabelType> > &proposal_generator, const std::shared_ptr<FusionSolver<LabelType> > &fusion_solver, const double MASTER_LIKELIHOOD) : proposal_generator_(proposal_generator), fusion_solver_(fusion_solver), MASTER_LIKELIHOOD_(MASTER_LIKELIHOOD) { updateStatus(); };
  ...
  void runFusion(const int thread_index = 0, const int iteration = 0) //two parameters are just for debugging purpose
  {
    if (is_master_) {
      ...
      LabelSpace<LabelType> label_space = proposal_generator_->getProposal();
      fused_solution_ = fusion_solver_->solve(label_space, fused_solution_energy_);
    }

    proposal_generator_->writeSolution(fused_solution_, thread_index, iteration); //write solution to files for debugging
    updateStatus();
  }
  ...
  void setSolutionPool(const std::vector<std::vector<LabelType> > &solution_pool) //Solution_pool is a set of solutions.
  {
    if (is_master_ && solution_pool.size() > 0) {
      ...
      for (typename std::vector<std::vector<LabelType> >::const_iterator solution_it = solution_pool.begin(); solution_it != solution_pool.end(); solution_it++)
