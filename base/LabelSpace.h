...
public:
  ...
  LabelSpace(const int NUM_NODES)
      : num_nodes_(NUM_NODES), label_space_((size_t)NUM_NODES){};
...
LabelSpace<LabelType>::LabelSpace(const std::vector<LabelType> &single_labels)
...
template <typename LabelType>
LabelSpace<LabelType>::LabelSpace(
    const std::vector<std::vector<LabelType>> &label_space)
    : num_nodes_((int)label_space.size()), label_space_(label_space) {}

template <typename LabelType>
void LabelSpace<LabelType>::setSingleLabels(
    const std::vector<LabelType> &single_labels) {
  // CHECK(single_labels.size() == num_nodes_) << "The number of nodes is
  // inconsistent.";
  num_nodes_ = (int)single_labels.size();
  label_space_.resize((size_t)num_nodes_);
  for (int node_index = 0; node_index < num_nodes_; node_index++)
    label_space_[node_index] =
        std::vector<LabelType>(1, single_labels[node_index]);
}
...
void LabelSpace<LabelType>::unionSpace(const LabelSpace<LabelType> &rhs) {
  ...
  if (num_nodes_ == 0) {
    ...
    for (int node_index = 0; node_index < num_nodes_; node_index++) {
      ...
      std::vector<LabelType> union_node_labels(node_labels.size() +
                                               rhs_node_labels.size());
      typename std::vector<LabelType>::const_iterator union_node_labels_it =
          std::set_union(node_labels.begin(), node_labels.end(),
                         rhs_node_labels.begin(), rhs_node_labels.end(),
                         union_node_labels.begin());
      union_node_labels.resize(union_node_labels_it -
                               union_node_labels.begin());
