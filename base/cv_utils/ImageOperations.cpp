...
Mat drawValuesOnImage(const vector<double> &values, const int IMAGE_WIDTH,
                      const int IMAGE_HEIGHT, const Func &func) {
...
vector<double> calcBoxIntegrationMask(const vector<double> &values,
...
double calcBoxIntegration(const vector<double> &mask, const int IMAGE_WIDTH,
                          const int IMAGE_HEIGHT, const int x_1, const int y_1,
  ...
  double value_1 =
      (min_x >= 0 && min_y >= 0) ? mask[min_y * IMAGE_WIDTH + min_x] : 0;
...
void calcWindowMeansAndVars(const std::vector<double> &values,
                            ...
                            const int WINDOW_SIZE, vector<double> &means,
                            vector<double> &vars) {
  vector<double> sum_mask =
      calcBoxIntegrationMask(values, IMAGE_WIDTH, IMAGE_HEIGHT);
  vector<double> values2(IMAGE_WIDTH * IMAGE_HEIGHT);
  transform(values.begin(), values.end(), values2.begin(),
            [](const double &x) { return pow(x, 2); });
  vector<double> sum2_mask =
      calcBoxIntegrationMask(values2, IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    ...
    int area =
        (min(pixel % IMAGE_WIDTH + (WINDOW_SIZE - 1) / 2, IMAGE_WIDTH - 1) -
         max(pixel % IMAGE_WIDTH - (WINDOW_SIZE - 1) / 2, 0) + 1) *
        (min(pixel / IMAGE_WIDTH + (WINDOW_SIZE - 1) / 2, IMAGE_HEIGHT - 1) -
         max(pixel / IMAGE_WIDTH - (WINDOW_SIZE - 1) / 2, 0) + 1);
    double mean = calcBoxIntegration(sum_mask, IMAGE_WIDTH, IMAGE_HEIGHT, x_1,
                                     y_1, x_2, y_2) /
                  area;
    double var = calcBoxIntegration(sum2_mask, IMAGE_WIDTH, IMAGE_HEIGHT, x_1,
                                    y_1, x_2, y_2) /
                     area -
                 pow(mean, 2);
    means[pixel] = mean;
...
void calcWindowMeansAndVars(const std::vector<std::vector<double>> &values,
  ...
  vector<vector<double>> values2(NUM_CHANNELS * NUM_CHANNELS,
                                 vector<double>(IMAGE_WIDTH * IMAGE_HEIGHT));
  for (int c_1 = 0; c_1 < NUM_CHANNELS; c_1++)
    for (int c_2 = 0; c_2 < NUM_CHANNELS; c_2++)
      transform(values[c_1].begin(), values[c_1].end(), values[c_2].begin(),
                values2[c_1 * NUM_CHANNELS + c_2].begin(),
                [](const double &x, const double &y) { return x * y; });
  vector<vector<double>> sum2_masks(NUM_CHANNELS * NUM_CHANNELS);
  for (int c_1 = 0; c_1 < NUM_CHANNELS; c_1++)
    for (int c_2 = 0; c_2 < NUM_CHANNELS; c_2++)
      sum2_masks[c_1 * NUM_CHANNELS + c_2] = calcBoxIntegrationMask(
          values2[c_1 * NUM_CHANNELS + c_2], IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  if (guidance_image.channels() == 1 && input_image.channels() == 1) {
    ...
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
      for (int x = 0; x < IMAGE_WIDTH; x++) {
        ...
        guidance_image_values[pixel] =
            1.0 * guidance_image.at<uchar>(y, x) / 256;
        input_image_values[pixel] = 1.0 * input_image.at<uchar>(y, x) / 256;
        guidance_image_input_image_values[pixel] =
            (1.0 * guidance_image.at<uchar>(y, x) / 256) *
            (1.0 * input_image.at<uchar>(y, x) / 256);
      }
    }
    vector<double> guidance_image_means;
    vector<double> guidance_image_vars;
    calcWindowMeansAndVars(guidance_image_values, IMAGE_WIDTH, IMAGE_HEIGHT,
                           radius * 2 + 1, guidance_image_means,
                           guidance_image_vars);
    vector<double> input_image_means;
    vector<double> dummy_vars;
    calcWindowMeansAndVars(input_image_values, IMAGE_WIDTH, IMAGE_HEIGHT,
                           radius * 2 + 1, input_image_means, dummy_vars);
    vector<double> guidance_image_input_image_means;
    calcWindowMeansAndVars(guidance_image_input_image_values, IMAGE_WIDTH,
                           IMAGE_HEIGHT, radius * 2 + 1,
                           guidance_image_input_image_means, dummy_vars);

    vector<double> a_values(NUM_PIXELS);
    vector<double> b_values(NUM_PIXELS);
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      a_values[pixel] =
          (guidance_image_input_image_means[pixel] -
           guidance_image_means[pixel] * input_image_means[pixel]) /
          (guidance_image_vars[pixel] + epsilon);
      b_values[pixel] = input_image_means[pixel] -
                        a_values[pixel] * guidance_image_means[pixel];
    }

    vector<double> a_means;
    vector<double> b_means;
    calcWindowMeansAndVars(a_values, IMAGE_WIDTH, IMAGE_HEIGHT, radius * 2 + 1,
                           a_means, dummy_vars);
    calcWindowMeansAndVars(b_values, IMAGE_WIDTH, IMAGE_HEIGHT, radius * 2 + 1,
                           b_means, dummy_vars);
    ...
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
      for (int x = 0; x < IMAGE_WIDTH; x++) {
        int pixel = y * IMAGE_WIDTH + x;
        output_image.at<uchar>(y, x) = max(
            min((a_means[pixel] * input_image_values[pixel] + b_means[pixel]) *
                    256,
                255.0),
            0.0);
      }
    ...
    vector<vector<double>> guidance_image_input_image_values(
        3, vector<double>(NUM_PIXELS));
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
      for (int x = 0; x < IMAGE_WIDTH; x++) {
        int pixel = y * IMAGE_WIDTH + x;
        ...
        for (int c = 0; c < 3; c++) {
          ...
          guidance_image_input_image_values[c][pixel] =
              (1.0 * guidance_image_color[c] / 256) *
              (1.0 * input_image_color[c] / 256);
        }
      }
    }
    ...
    for (int c = 0; c < 3; c++) {
      vector<double> dummy_vars;
      calcWindowMeansAndVars(guidance_image_values[c], IMAGE_WIDTH,
                             IMAGE_HEIGHT, radius * 2 + 1,
                             guidance_image_means[c], guidance_image_vars[c]);
      calcWindowMeansAndVars(input_image_values[c], IMAGE_WIDTH, IMAGE_HEIGHT,
                             radius * 2 + 1, input_image_means[c], dummy_vars);
      calcWindowMeansAndVars(guidance_image_input_image_values[c], IMAGE_WIDTH,
                             IMAGE_HEIGHT, radius * 2 + 1,
                             guidance_image_input_image_means[c], dummy_vars);
    }
    ...
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      ...
      for (int c = 0; c < 3; c++) {
        guidance_image_input_image_covariance +=
            guidance_image_input_image_means[c][pixel] -
            guidance_image_means[c][pixel] * input_image_means[c][pixel];
        guidance_image_var += guidance_image_vars[c][pixel];
      }
      a_values[pixel] = guidance_image_input_image_covariance /
                        (guidance_image_var + epsilon);
      for (int c = 0; c < 3; c++)
        b_values[c][pixel] = input_image_means[c][pixel] -
                             a_values[pixel] * guidance_image_means[c][pixel];
    }
    ...
    for (int c = 0; c < 3; c++) {
      calcWindowMeansAndVars(b_values[c], IMAGE_WIDTH, IMAGE_HEIGHT,
                             radius * 2 + 1, b_means[c], dummy_vars);
    }
    ...
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
      for (int x = 0; x < IMAGE_WIDTH; x++) {
        ...
        Vec3b color;
        for (int c = 0; c < 3; c++)
          color[c] = max(min((a_means[pixel] * input_image_values[c][pixel] +
                              b_means[c][pixel]) *
                                 256,
                             255.0),
                         0.0);
    ...
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      ...
      for (int c = 0; c < 3; c++)
        guidance_image_input_image_covariance[c] =
            guidance_image_input_image_means[c][pixel] -
            guidance_image_means[c][pixel] * input_image_means[pixel];
      ...
      for (int c_1 = 0; c_1 < 3; c_1++)
        for (int c_2 = 0; c_2 < 3; c_2++)
          a_value[c_1] += guidance_image_var_inverse[c_1][c_2] *
                          guidance_image_input_image_covariance[c_2];
      for (int c = 0; c < 3; c++)
    ...
    for (int c = 0; c < 3; c++)
      calcWindowMeansAndVars(a_values[c], IMAGE_WIDTH, IMAGE_HEIGHT,
