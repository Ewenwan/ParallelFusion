...
ImageMask::ImageMask(const vector<bool> &mask, const int width,
                     const int height)
    : mask_(mask), width_(width), height_(height) {}

ImageMask::ImageMask(const bool value, const int width, const int height)
    : mask_(vector<bool>(width * height, value)), width_(width),
      height_(height) {}

ImageMask::ImageMask(const vector<int> &pixels, const int width,
                     const int height)
    : width_(width), height_(height) {
  mask_.assign(width * height, false);
  for (vector<int>::const_iterator pixel_it = pixels.begin();
       pixel_it != pixels.end(); pixel_it++)
    mask_[*pixel_it] = true;
}

ImageMask::ImageMask(const Mat &image) { readMaskImage(image); }
...
void ImageMask::setMask(const vector<bool> &mask, const int width,
                        const int height) {
  mask_ = mask;
  width_ = width;
  height_ = height;
...
void ImageMask::resize(const int new_width, const int new_height) {
  ...
  for (int new_x = 0; new_x < new_width; new_x++) {
    for (int new_y = 0; new_y < new_height; new_y++) {
      const int ori_x = min(
          static_cast<int>(1.0 * new_x * width_ / new_width + 0.5), width_ - 1);
      const int ori_y =
          min(static_cast<int>(1.0 * new_y * height_ / new_height + 0.5),
              height_ - 1);
...
void ImageMask::resizeWithBias(const int new_width, const int new_height,
  ...
  for (int new_x = 0; new_x < new_width; new_x++) {
    for (int new_y = 0; new_y < new_height; new_y++) {
      ...
      if (mask_[ori_y_1 * width_ + ori_x_1] == desired_value ||
          mask_[ori_y_1 * width_ + ori_x_2] == desired_value ||
          mask_[ori_y_2 * width_ + ori_x_1] == desired_value ||
          mask_[ori_y_2 * width_ + ori_x_2] == desired_value)
...
void ImageMask::dilate(const int num_iterations, const bool USE_PANORAMA,
                       const int NEIGHBOR_SYSTEM) {
  for (int iteration = 0; iteration < num_iterations; iteration++) {
    ...
    for (int pixel = 0; pixel < width_ * height_; pixel++) {
      ...
      vector<int> neighbor_pixels =
          findNeighbors(pixel, width_, height_, USE_PANORAMA, NEIGHBOR_SYSTEM);
...
void ImageMask::erode(const int num_iterations, const bool USE_PANORAMA,
                      const int NEIGHBOR_SYSTEM) {
  for (int iteration = 0; iteration < num_iterations; iteration++) {
    vector<bool> new_mask = mask_;
    for (int pixel = 0; pixel < width_ * height_; pixel++) {
...
Mat ImageMask::drawMaskImage(const int num_channels) const {
  Mat mask_image = num_channels == 1 ? Mat::zeros(height_, width_, CV_8UC1)
                                     : Mat::zeros(height_, width_, CV_8UC3);
  for (int pixel = 0; pixel < width_ * height_; pixel++)
    if (mask_[pixel] == true)
      if (num_channels == 1)
        mask_image.at<uchar>(pixel / width_, pixel % width_) = 255;
      else
        mask_image.at<Vec3b>(pixel / width_, pixel % width_) =
            Vec3b(255, 255, 255);
  return mask_image;
}

cv::Mat ImageMask::drawImageWithMask(const cv::Mat &image,
                                     const bool use_mask_color,
                                     const Vec3b mask_color,
                                     const bool use_outside_color,
                                     const Vec3b outside_color) const {
...
bool ImageMask::at(const int &pixel) const { return mask_[pixel]; }
...
vector<double>
ImageMask::calcDistanceMapOutside(const bool USE_PANORAMA,
                                  const int NEIGHBOR_SYSTEM) const {
  ...
  while (border_pixels.size() > 0) {
    ...
    for (vector<int>::const_iterator border_pixel_it = border_pixels.begin();
      ...
      for (vector<int>::const_iterator neighbor_pixel_it =
        ...
        double distance_delta =
            sqrt(pow(*neighbor_pixel_it % width_ - pixel % width_, 2) +
                 pow(*neighbor_pixel_it / width_ - pixel / width_, 2));
...
vector<double>
ImageMask::calcDistanceMapInside(const bool USE_PANORAMA,
                                 const int NEIGHBOR_SYSTEM) const {
  vector<double> distance_map(width_ * height_, 1000000);
  vector<int> border_pixels;
  for (int pixel = 0; pixel < width_ * height_; pixel++) {
...
void ImageMask::calcBoundaryDistanceMap(vector<int> &boundary_map,
...
ImageMask::findConnectedComponents(const bool USE_PANORAMA,
                                   const int NEIGHBOR_SYSTEM) {
  ...
  for (int pixel = 0; pixel < width_ * height_; pixel++) {
    ...
    while (true) {
      ...
      for (vector<int>::const_iterator border_pixel_it = border_pixels.begin();
        ...
        vector<int> neighbor_pixels = findNeighbors(
            *border_pixel_it, width_, height_, USE_PANORAMA, NEIGHBOR_SYSTEM);
        for (vector<int>::const_iterator neighbor_pixel_it =
                 neighbor_pixels.begin();
             neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
          if (mask_[*neighbor_pixel_it] == true &&
  ...
  sort(connected_components.begin(), connected_components.end(),
       [](const vector<int> &vec_1, const vector<int> &vec_2) {
...
ImageMask operator+(const ImageMask &image_mask_1,
                    const ImageMask &image_mask_2) {
...
ostream &operator<<(ostream &out_str, const ImageMask &image_mask) {
  out_str << image_mask.width_ << '\t' << image_mask.height_ << '\t'
          << image_mask.getNumPixels() << endl;
...
void ImageMask::smooth(const string type, const int window_size,
                       const double sigma) {
...
void ImageMask::readMaskImage(const Mat &mask_image) {
  ...
  for (int pixel = 0; pixel < width_ * height_; pixel++)
    mask_[pixel] =
        mask_image_gray.at<uchar>(pixel / width_, pixel % width_) >= 128;
}

std::vector<int> ImageMask::findMaskWindowPixels(const int pixel,
                                                 const int WINDOW_SIZE,
                                                 const int USE_PANORAMA) const {
  vector<int> window_pixels =
      findWindowPixels(pixel, width_, height_, WINDOW_SIZE, USE_PANORAMA);
