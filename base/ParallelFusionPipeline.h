...
{
  std::vector<std::vector<LabelType> > solution_pool(pipeline_params.NUM_THREADS, initial_solution);
  ...
  for (int iteration = 0; iteration < pipeline_params.NUM_FUSION_ITERATIONS; iteration++) {
    ...
    for (typename std::vector<std::unique_ptr<FusionThread<LabelType> > >::iterator fusion_thread_it = fusion_threads.begin(); fusion_thread_it != fusion_threads.end(); fusion_thread_it++) {
      int thread_index = fusion_thread_it - fusion_threads.begin();
      std::cout << "thread: " << thread_index << '\t' << (*fusion_thread_it)->getStatus() << std::endl;
      std::vector<LabelType> current_solution = solution_pool[cv_utils::drawFromArray(solution_confidence_vec)];
