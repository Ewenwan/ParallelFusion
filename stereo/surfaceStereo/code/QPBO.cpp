	  ...
	  zero_energy(0)
{
	...
	int64 border = 2 * arc_shift;

	//memset(arcs[0], 0, border);

	char *dummy = (char*) arcs[0];
	for (int64 i = 0; i < border; i++)
	{
		*dummy = (char) 0;
		dummy++;
	}
...
/*	int *dummy = (int*) arcs[0];
	for (int i = 0; i < arc_shift / 2; i++)
	{
		dummy[i] = 0;
	}*/
