...
SimpleStereo::SimpleStereo(const FileIO &file_io_, const int anchor_,
                           const int dispResolution_, const int downsample_,
                           const double weight_smooth_, const int num_threads_,
                           const int num_proposals_,
                           const int exchange_interval_,
                           const int exchange_amount_)
    : file_io(file_io_), anchor(anchor_), downsample(downsample_),
      num_threads(num_threads_), model(new MRFModel<int>()),
      num_proposals{num_proposals_}, exchange_interval{exchange_interval_},
      exchange_amount{exchange_amount_} {
  CHECK_GE(file_io.getTotalNum(), 2) << "Too few images at "
                                     << file_io.getDirectory();
  images.resize((size_t)file_io.getTotalNum());
  CHECK(downsample == 1 || downsample == 2 || downsample == 4 ||
        downsample == 8)
      << "Invalid downsample ratio!";
...
void SimpleStereo::computeMatchingCost() {
  ...
  if (fin.is_open()) {
    ...
    printf("Cached data: anchor:%d, resolution:%d, Energytype:%d\n", frame,
           resolution, type);
    if (frame == anchor && resolution == model->nLabel && ds == downsample &&
        type == sizeof(int)) {
      printf("Reading unary term from cache...\n");
      fin.read((char *)model->MRF_data,
               model->width * model->height * model->nLabel * sizeof(int));
      recompute = false;
    }
    fin.close();
  }
...
#pragma omp parallel for
        for (int d = 0; d < model->nLabel; ++d) {
          ...
          model->MRF_data[model->nLabel * (y * width + x) + d] =
              (int)((mCost + 1) * model->MRFRatio);
        }
      }
    }
    ...
    cacheOut.write((char *)model->MRF_data, sizeof(EnergyType) * model->width *
                                                model->height * model->nLabel);
    cacheOut.close();
  }
  cout << "Done" << endl;
    ...
    for (auto x = 0; x < width; ++x) {
      ...
      if (y < height - 1) {
        ...
        if (diff > t)
          ...
          model->vCue[y * width + x] =
              (EnergyType)((diff - t) * (diff - t) * ratio);
      }
      if (x < width - 1) {
...
void SimpleStereo::runStereo() {
  ...
  //         ParallelOptimize optimize_multiway(file_io, model, num_threads,
  //         "Swarn_multiway", labelList, true);
  //         Depth result_multiway;
  //         optimize_multiway.optimize(result_multiway, 2);
  //         sprintf(buffer, "%s/temp/result_multiway.jpg",
  //         file_io.getDirectory().c_str());
  //         result_multiway.saveImage(buffer);

  //        cout << "========================================" << endl;
  //        cout << "Runing Victor's method" << endl;
  //        VictorOptimize optimize_victor(file_io, model, num_threads,
  //        "Victor", labelList);
  //        Depth result_victor;
  //        optimize_victor.optimize(result_victor, 1);
  //        sprintf(buffer, "%s/temp/result_victor.jpg",
  //        file_io.getDirectory().c_str());
  //        result_victor.saveImage(buffer);

  // cout << "========================================" << endl;
  // cout << "Runing Victor's method multiway" << endl;
  // VictorOptimize optimize_victor_multiway(file_io, model, num_threads,
  // "Victor_multiway", labelList, true);
  // Depth result_victor_multiway;
  // optimize_victor_multiway.optimize(result_victor_multiway, 2);
  // sprintf(buffer, "%s/temp/result_victor_multiway.jpg",
  // file_io.getDirectory().c_str());
  // result_victor_multiway.saveImage(buffer);

  // cout << "========================================" << endl;
  // cout << "Runing Hierarchy method" << endl;
  // HierarchyOptimize optimize_hierarchy(file_io, model, num_threads,
  // labelList);
  // Depth result_hierarchy;
  // optimize_hierarchy.optimize(result_hierarchy, 1);
  // sprintf(buffer, "%s/temp/result_hierarchy.jpg",
  // file_io.getDirectory().c_str());
  ...
  //        sprintf(buffer, "%s/temp/result_firstOrder.jpg",
  //        file_io.getDirectory().c_str());
