...
public:
  Depth()
      : depthwidth(0), depthheight(0), average_depth(0), median_depth(0),
        depth_var(0), max_depth(-1), min_depth(10000000),
        statics_computed(false) {}
  ...
  inline double getDepthAtInd(int ind) const { return this->operator[](ind); }
  ...
  inline bool insideDepth(const Eigen::Vector2d &loc) const {
    return (loc[0] >= 0 && loc[1] >= 0 && loc[0] < getWidth() - 1 &&
            loc[1] < getHeight() - 1);
  }
...
void fillHoleLinear(std::vector<double> &data, const int width,
                    const int height, const std::vector<bool> &mask);
void possionSmooth(std::vector<double> &data, const int width, const int height,
                   const std::vector<bool> &mask, const double kDataCost = 1.0);
}
