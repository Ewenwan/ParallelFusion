...
SceStereo::SceStereo(const FileIO &file_io_, const int anchor_,
                     const int tWindow_, const int resolution_)
    : file_io(file_io_), anchor(anchor_), dispResolution(resolution_),
      MRFRatio(1000), pR(3) {
...
#pragma omp parallel for
        for (int d = 0; d < dispResolution; ++d) {
          ...
          double mCost =
              local_matcher::sumMatchingCost(patches, anchor - offset);
          MRF_data[dispResolution * (y * width + x) + d] =
              (EnergyType)((mCost + 1) * MRFRatio);
        }
      }
    }
    ...
    cacheOut.write((char *)MRF_data.data(),
                   sizeof(EnergyType) * MRF_data.size());
    cacheOut.close();
  }
  cout << "Done" << endl;
...
void SceStereo::runStereo() {
  ...
  sprintf(buffer, "%s/temp/unaryDisp%05d.jpg", file_io.getDirectory().c_str(),
          anchor);
  unaryDisp.saveImage(string(buffer), 256.0 / (dispResolution)*4);

  //        FirstOrderOptimize firstOrderOptimize(file_io, (int)images.size(),
  //        images[anchor-offset], MRF_data, (float)MRFRatio, dispResolution,
  //        (EnergyType)(0.01 * MRFRatio));
  //        Depth result_firstorder;
  //        firstOrderOptimize.optimize(result_firstorder, 10);
  //        sprintf(buffer, "%s/temp/result_firstorder%05d.jpg",
  //        file_io.getDirectory().c_str(), anchor);
  //        result_firstorder.saveImage(string(buffer), 256.0 / (dispResolution)
  //        * 4);

  SecondOrderOptimizeFusionMove fusionmove(
      file_io, (int)images.size(), images[anchor - offset], MRF_data,
      (float)MRFRatio, dispResolution, anchor - offset, unaryDisp);
  Depth result_fusionmove;
  fusionmove.optimize(result_fusionmove, 500);

  sprintf(buffer, "%s/temp/result_fusionmove%05d.jpg",
          file_io.getDirectory().c_str(), anchor);
