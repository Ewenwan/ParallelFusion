...
// 1. Energy construction. Only AddNode(), AddNodeData() and AddEdge() may be
// called.
//
// Any call ZeroMessages(), SetAutomaticOrdering(), Minimize_TRW_S() or
// Minimize_BP()
// completes graph construction; MRFEnergy goes to the second phase:
// 2. Only functions AddNodeData(), ZeroMessages(), Minimize_TRW_S(),
// Minimize_BP()
// or GetSolution() may be called. (The last function can be called only after
// Minimize_TRW_S() or Minimize_BP()).

inline void DefaultErrorFn(const std::string &msg) { CHECK(true) << msg; }
...
public:
  ...
  // Constructor. Function errorFn is called with an error message, if an error
  // occurs.
  ...
  // Adds to all message entries a value drawn uniformly from [min_value,
  // max_value].
  // Normally, min_value can be set to 0 (except for TypeBinaryFast, in which
  // case min_value = -max_value)
  void AddRandomMessages(unsigned int random_seed, REAL min_value,
                         REAL max_value);
  ...
  // It may speed up the algorithm if, for example, the original order is
  // random.
  //
  ...
  struct Options {
    ...
    // stopping criterion
    REAL m_eps; // stop if the increase in the lower bound during one iteration
                // is less or equal than m_eps.
    ...
    int m_printIter;    // print lower bound and energy every m_printIter
                        // iterations
    int m_printMinIter; // do not print lower bound and energy before
                        // m_printMinIter iterations
  ...
  // used. Normally, it's (# nodes)*(# labels). Exception: for TypeBinaryFast
  // it's (# nodes).
...
private:
  ...
  char *m_buf; // buffer of size m_vectorMaxSizeInBytes
               //              + max(m_vectorMaxSizeInBytes,
               //              Edge::GetBufSizeInBytes(m_vectorMaxSizeInBytes))

  void CompleteGraphConstruction(); // nodes and edges cannot be added after
                                    // calling this function
  void SetMonotonicTrees();

  REAL ComputeSolutionAndEnergy(); // sets Node::m_solution, returns value of
                                   // the energy
  ...
  struct Node {
    ...
    MRFEdge
        *m_firstForward; // first edge going to nodes with greater m_ordering
    MRFEdge
        *m_firstBackward; // first edge going to nodes with smaller m_ordering
    ...
    Vector m_D; // must be the last member in the struct since its size is not
                // fixed
  };
  ...
  struct MRFEdge {
    ...
    Edge m_message; // must be the last member in the struct since its size is
                    // not fixed.
    ...
    // by the forward message only temporarily inside Minimize_TRW_S() and
    // Minimize_BP().
  };

  // Use our own Malloc since
  // (a) new in C++ is slow and allocates minimum memory of 64 bytes (in Visual
  // C++)
  // (b) we want simple (one function) deallocation instead of going through
  // every allocated element
  struct MallocBlock {
...
template <class T> inline char *MRFEnergy<T>::Malloc(int bytesNum) {
  if (!m_mallocBlockFirst ||
      m_mallocBlockFirst->m_current + bytesNum > m_mallocBlockFirst->m_last) {
    int size = (bytesNum > MallocBlock::minBlockSizeInBytes)
                   ? bytesNum
                   : MallocBlock::minBlockSizeInBytes;
    MallocBlock *b = (MallocBlock *)new char[sizeof(MallocBlock) + size];
    if (!b)
      m_errorFn("Not enough memory");
...
template <class T>
inline typename T::Label MRFEnergy<T>::GetSolution(NodeId i) {
  return i->m_solution;
}
