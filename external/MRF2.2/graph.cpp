...
Graph::node_id Graph::add_node() {
  ...
  if (!node_block_first ||
      node_block_first->current + 1 >
          &node_block_first->nodes[NODE_BLOCK_SIZE - 1]) {
...
void Graph::add_edge(node_id from, node_id to, captype cap, captype rev_cap) {
  ...
  if (!arc_for_block_first ||
    ...
    if ((PTR_CAST)ptr & 1)
      arc_for_block_first = (arc_for_block *)(ptr + 1);
    else
      arc_for_block_first = (arc_for_block *)ptr;
  ...
  if (!arc_rev_block_first ||
    ...
    if ((PTR_CAST)ptr & 1)
      arc_rev_block_first = (arc_rev_block *)(ptr + 1);
    else
      arc_rev_block_first = (arc_rev_block *)ptr;
...
void Graph::prepare_graph() {
  ...
  for (nb = node_block_first; nb; nb = nb->next) {
    for (i = &nb->nodes[0]; i < nb->current; i++) {
      ...
      if (a_for + k > &ab_for->arcs_for[ARC_BLOCK_SIZE]) {
        if (k > ARC_BLOCK_SIZE) {
          if (error_function)
            (*error_function)("# of arcs per node exceeds block size!");
          exit(1);
        }
        if (for_flag)
          ab_for = NULL;
        ...
        if (ab_for == NULL) {
          ...
          a_rev_scan = &ab_rev_scan->arcs_rev[0];
        a_for = &ab_for->arcs_for[0];
      }
      if (ab_rev_scan) {
        a_rev_scan += k;
        ...
        i->parent = (arc_forward *)&a_rev_tmp;
      a_for += k;
      i->first_out = a_for;
      ab_for->last_node = i;
  ...
  for (ab_for = ab_for_first, ab_rev = ab_rev_first; ab_for;
       ab_for = ab_for->next, ab_rev = ab_rev->next)
    for (a_for = &ab_for->arcs_for[0], a_rev = &ab_rev->arcs_rev[0];
         a_for < &ab_for->arcs_for[ARC_BLOCK_SIZE]; a_for++, a_rev++) {
      arc_forward *af;
      arc_reverse *ar;
      node *from;
