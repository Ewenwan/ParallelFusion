...
void GCoptimization::commonGridInitialization(PixelType width, PixelType height,
                                              int nLabels) {
  terminateOnError((width < 0) || (height < 0) || (nLabels < 0),
                   "Illegal negative parameters");
...
void GCoptimization::setParameters(int numParam, void *param) {
  if (numParam != 1)
    printf("\nInvalid number of parameters, can only set one parameter\\that "
           "is boolean label order\n");
  else {
...
void GCoptimization::commonNonGridInitialization(PixelType nupixels,
                                                 int num_labels) {
  terminateOnError((nupixels < 0) || (num_labels < 0),
                   "Illegal negative parameters");
...
GCoptimization::GCoptimization(PixelType width, PixelType height,
                               LabelType nLabels, EnergyFunction *eng)
...
GCoptimization::GCoptimization(PixelType nPixels, int nLabels,
                               EnergyFunction *eng)
...
void GCoptimization::setData(DataCostFn dataFn) { m_dataFnPix = dataFn; }

/**************************************************************************************/

void GCoptimization::setSmoothness(EnergyTermType *V) { m_smoothcost = V; }

/**************************************************************************************/

void GCoptimization::setSmoothness(int smoothExp, CostVal smoothMax,
                                   CostVal lambda) {
  int i, j;
  CostVal cost;
  ...
  for (i = 0; i < m_nLabels; i++)
    for (j = i; j < m_nLabels; j++) {
      ...
      if (cost > smoothMax)
        cost = smoothMax;
      m_smoothcost[i * m_nLabels + j] = m_smoothcost[j * m_nLabels + i] =
          cost * lambda;
    }
}
...
GCoptimization::EnergyType GCoptimization::smoothnessEnergy() {
  ...
  if (m_grid_graph) {
    if (m_smoothType != FUNCTION) {
      if (m_varWeights)
        return (giveSmoothEnergy_G_ARRAY_VW());
      else
        return (giveSmoothEnergy_G_ARRAY());
    } else
      return (giveSmoothEnergy_G_FnPix());
  } else {
    if (m_smoothType != FUNCTION)
      return (giveSmoothEnergy_NG_ARRAY());
    else
      return (giveSmoothEnergy_NG_FnPix());
  }
  return (0);
}
  ...
  for (i = 0; i < m_nPixels; i++)
    if (!m_neighbors[i].isEmpty()) {
      ...
      while (m_neighbors[i].hasNext()) {
        ...
        if (i < temp->to_node)
          eng = eng + m_smoothFnPix(i, temp->to_node, m_labeling[i],
                                    m_labeling[temp->to_node]);
      }
    }
  ...
  for (i = 0; i < m_nPixels; i++)
    if (!m_neighbors[i].isEmpty()) {
      ...
      while (m_neighbors[i].hasNext()) {
        ...
        if (i < temp->to_node)
          eng = eng +
                m_smoothcost(m_labeling[i], m_labeling[temp->to_node]) *
                    (temp->weight);
      }
    }
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergy_G_ARRAY_VW() {
  ...
  for (y = 0; y < m_height; y++)
    for (x = 1; x < m_width; x++) {
      pix = x + y * m_width;
      eng = eng +
            m_smoothcost(m_labeling[pix], m_labeling[pix - 1]) *
                m_horizWeights[pix - 1];
    }
  ...
  for (y = 1; y < m_height; y++)
    for (x = 0; x < m_width; x++) {
      pix = x + y * m_width;
      eng = eng +
            m_smoothcost(m_labeling[pix], m_labeling[pix - m_width]) *
                m_vertWeights[pix - m_width];
    }
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergy_G_FnPix() {
  ...
  for (y = 0; y < m_height; y++)
    for (x = 1; x < m_width; x++) {
      pix = x + y * m_width;
      eng = eng +
            m_smoothFnPix(pix, pix - 1, m_labeling[pix], m_labeling[pix - 1]);
    }

  for (y = 1; y < m_height; y++)
    for (x = 0; x < m_width; x++) {
      pix = x + y * m_width;
      eng = eng + m_smoothFnPix(pix, pix - m_width, m_labeling[pix],
                                m_labeling[pix - m_width]);
    }
...
void GCoptimization::add_t_links_ARRAY(Energy *e, Energy::Var *variables,
                                       int size, LabelType alpha_label) {
...
void GCoptimization::add_t_links_FnPix(Energy *e, Energy::Var *variables,
                                       int size, LabelType alpha_label) {
  for (int i = 0; i < size; i++)
...
void GCoptimization::setNeighbors(PixelType pixel1, int pixel2,
  ...
  assert(pixel1 < m_nPixels && pixel1 >= 0 && pixel2 < m_nPixels &&
         pixel2 >= 0);
...
Swap::Swap(PixelType width, PixelType height, int num_labels,
           EnergyFunction *eng)
    : GCoptimization(width, height, num_labels, eng) {
...
Swap::Swap(PixelType nPixels, int num_labels, EnergyFunction *eng)
    : GCoptimization(nPixels, num_labels, eng) {
  m_pixels = new PixelType[m_nPixels];
  terminateOnError(!m_pixels, "Not enough memory");
}
...
GCoptimization::EnergyType Swap::swap() { return (start_swap(MAX_INTT)); }
...
GCoptimization::EnergyType Swap::start_swap(int max_num_iterations) {
  ...
  while (old_energy < 0 ||
         (old_energy > new_energy && curr_cycle <= max_num_iterations)) {
...
GCoptimization::EnergyType Swap::alpha_beta_swap(LabelType alpha_label,
                                                 LabelType beta_label) {
  terminateOnError(alpha_label < 0 || alpha_label >= m_nLabels ||
                       beta_label < 0 || beta_label >= m_nLabels,
...
void Swap::perform_alpha_beta_swap(LabelType alpha_label,
                                   LabelType beta_label) {
  PixelType i, size = 0;
  Energy *e = new Energy();
  ...
  if (m_dataType == ARRAY)
    add_t_links_ARRAY_swap(e, variables, size, alpha_label, beta_label,
                           m_pixels);
  else
    add_t_links_FnPix_swap(e, variables, size, alpha_label, beta_label,
                           m_pixels);

  if (m_grid_graph) {
    if (m_smoothType != FUNCTION) {
      if (m_varWeights)
        set_up_swap_energy_G_ARRAY_VW(size, alpha_label, beta_label, m_pixels,
                                      e, variables);
      else
        set_up_swap_energy_G_ARRAY(size, alpha_label, beta_label, m_pixels, e,
                                   variables);
    } else
      set_up_swap_energy_G_FnPix(size, alpha_label, beta_label, m_pixels, e,
                                 variables);

  } else {
    if (m_smoothType != FUNCTION)
      set_up_swap_energy_NG_ARRAY(size, alpha_label, beta_label, m_pixels, e,
                                  variables);
    else
      set_up_swap_energy_NG_FnPix(size, alpha_label, beta_label, m_pixels, e,
                                  variables);
  }
  ...
  for (i = 0; i < size; i++)
    if (e->get_var(variables[i]) == 0)
      ...
      m_labeling[m_pixels[i]] = beta_label;
...
void Swap::set_up_swap_energy_NG_ARRAY(int size, LabelType alpha_label,
                                       LabelType beta_label, PixelType *pixels,
...
void Swap::set_up_swap_energy_NG_FnPix(int size, LabelType alpha_label,
                                       LabelType beta_label, PixelType *pixels,
                                       Energy *e, Energy::Var *variables) {
  PixelType nPix, pix, i;
  Neighbor *tmp;
...
void Swap::set_up_swap_energy_G_FnPix(int size, LabelType alpha_label,
                                      LabelType beta_label, PixelType *pixels,
                                      Energy *e, Energy::Var *variables) {
...
void Swap::set_up_swap_energy_G_ARRAY_VW(int size, LabelType alpha_label,
                                         LabelType beta_label,
                                         PixelType *pixels, Energy *e,
  ...
  for (i = 0; i < size; i++) {
    ...
    if (x < m_width - 1) {
      ...
      if (!(m_labeling[nPix] == alpha_label || m_labeling[nPix] == beta_label))
        e->add_term1(
            variables[i],
            m_smoothcost(alpha_label, m_labeling[nPix]) * m_horizWeights[pix],
    ...
    if (y < m_height - 1) {
      ...
      if (!(m_labeling[nPix] == alpha_label || m_labeling[nPix] == beta_label))
        e->add_term1(
            variables[i],
            m_smoothcost(alpha_label, m_labeling[nPix]) * m_vertWeights[pix],
...
void Swap::set_up_swap_energy_G_ARRAY(int size, LabelType alpha_label,
                                      LabelType beta_label, PixelType *pixels,
                                      Energy *e, Energy::Var *variables)
...
void Swap::optimizeAlg(int nIterations) { swap(nIterations); }
...
GCoptimization::EnergyType Expansion::alpha_expansion(LabelType label) {
  using namespace std;
  terminateOnError(label < 0 || label >= m_nLabels,
                   "Illegal Label to Expand On");
...
void Expansion::perform_alpha_expansion(LabelType alpha_label) {
  ...
  if (size > 0) {
    ...
    if (m_dataType == ARRAY)
      add_t_links_ARRAY(e, variables, size, alpha_label);
    else
      add_t_links_FnPix(e, variables, size, alpha_label);

    if (m_grid_graph) {
      if (m_smoothType != FUNCTION) {
        if (m_varWeights)
          set_up_expansion_energy_G_ARRAY_VW(size, alpha_label, e, variables);
        else
          set_up_expansion_energy_G_ARRAY(size, alpha_label, e, variables);
      } else
        set_up_expansion_energy_G_FnPix(size, alpha_label, e, variables);

    } else {
      if (m_smoothType != FUNCTION)
        set_up_expansion_energy_NG_ARRAY(size, alpha_label, e, variables);
      else if (m_smoothType == FUNCTION)
        set_up_expansion_energy_NG_FnPix(size, alpha_label, e, variables);
    }
...
/**********************************************************************************************/
/* Performs alpha-expansion for non regular grid graph for case when energy
 * terms are NOT     */
/* specified by a function */

void Expansion::set_up_expansion_energy_NG_ARRAY(int size,
                                                 LabelType alpha_label,
                                                 Energy *e,
                                                 Energy::Var *variables) {
  EnergyTermType weight;
  Neighbor *tmp;
...
void Expansion::set_up_expansion_energy_NG_FnPix(int size,
                                                 LabelType alpha_label,
                                                 Energy *e,
                                                 Energy::Var *variables) {
  Neighbor *tmp;
    ...
    if (!m_neighbors[pix].isEmpty()) {
      ...
      while (m_neighbors[pix].hasNext()) {
        ...
        if (m_labeling[nPix] != alpha_label) {
          if (pix < nPix)
            e->add_term2(
                variables[i], variables[m_lookupPixVar[nPix]],
                m_smoothFnPix(pix, nPix, alpha_label, alpha_label),
                m_smoothFnPix(pix, nPix, alpha_label, m_labeling[nPix]),
...
/**********************************************************************************************/
/* Performs alpha-expansion for  regular grid graph for case when energy terms
 * are NOT        */
/* specified by a function */
void Expansion::set_up_expansion_energy_G_ARRAY_VW(int size,
                                                   LabelType alpha_label,
                                                   Energy *e,
                                                   Energy::Var *variables) {
  int i, nPix, pix, x, y;
  ...
  for (i = size - 1; i >= 0; i--) {
    ...
    if (x > 0) {
      ...
      if (m_labeling[nPix] == alpha_label)
        e->add_term1(
            variables[i],
            m_smoothcost(alpha_label, m_labeling[nPix]) * m_horizWeights[nPix],
    ...
    if (y > 0) {
      ...
      if (m_labeling[nPix] == alpha_label)
        e->add_term1(
            variables[i],
            m_smoothcost(alpha_label, alpha_label) * m_vertWeights[nPix],
 ...
 * are NOT        */
/* specified by a function */
void Expansion::set_up_expansion_energy_G_ARRAY(int size, LabelType alpha_label,
                                                Energy *e,
                                                Energy::Var *variables) {
  int i, nPix, pix, x, y;
 ...
 * are NOT        */
/* specified by a function */
void Expansion::set_up_expansion_energy_G_FnPix(int size, LabelType alpha_label,
                                                Energy *e,
                                                Energy::Var *variables) {
  int i, nPix, pix, x, y;
...
GCoptimization::EnergyType Expansion::oneExpansionIteration() {
  terminateOnError(
      m_dataType == NONE,
      "You have to set up the data cost before running optimization");
  terminateOnError(
      m_smoothType == NONE,
      "You have to set up the smoothness cost before running optimization");
...
void Expansion::optimizeAlg(int nIterations) { expansion(nIterations); }
