...
public:
  ...
  LabelSpace(const int NUM_NODES)
      : num_nodes_(NUM_NODES), label_space_(NUM_NODES){};
  ...
  void clear();
  void assign(const int NUM_NODES,
              const std::vector<int> &node_labels = std::vector<int>()) {
    num_nodes_ = NUM_NODES;
    label_space_.assign(NUM_NODES, node_labels);
  };
  void setSingleLabels(const std::vector<int> &single_labels);
  void setLabelSpace(const std::vector<std::vector<int>> label_space) {
    num_nodes_ = label_space.size();
