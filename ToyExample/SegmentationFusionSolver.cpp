...
vector<int> SegmentationFusionSolver::solve(const LabelSpace<int> &label_space,
                                            double &energy) const {
    ...
    for (vector<int>::const_iterator neighbor_pixel_it =
      ...
      for (int proposal_index = 0; proposal_index < pixel_num_labels[pixel];
        ...
        for (int neighbor_proposal_index = 0;
             neighbor_proposal_index < pixel_num_labels[*neighbor_pixel_it];
             neighbor_proposal_index++) {
          int label = label_space_vec[pixel][proposal_index];
          int neighbor_label =
              label_space_vec[*neighbor_pixel_it][neighbor_proposal_index];
  ...
  solver.arg(labels);
  cout << "energy: " << solver.value() << " lower bound: " << solver.bound()
       << endl;
