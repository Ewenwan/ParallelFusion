...
#include "../BaseTest/BaseDataStructures.h"
...
int main() {
  ...
  if (true) {
    ...
    vector<double> fusion_thread_master_likelihood_vec(
        pipeline_params.NUM_THREADS, 0);
    ...
    for (int fusion_thread_index = 0;
         fusion_thread_index < pipeline_params.NUM_THREADS;
         fusion_thread_index++)
      proposal_generators.push_back(
          dynamic_pointer_cast<ProposalGenerator<int>>(
              shared_ptr<SegmentationProposalGenerator>(
                  new SegmentationProposalGenerator(image))));
    vector<shared_ptr<FusionSolver<int>>> fusion_solvers;
    for (int fusion_thread_index = 0;
         fusion_thread_index < pipeline_params.NUM_THREADS;
         fusion_thread_index++)
      fusion_solvers.push_back(dynamic_pointer_cast<FusionSolver<int>>(
          shared_ptr<SegmentationFusionSolver>(
              new SegmentationFusionSolver(image))));
    for (int fusion_thread_index = 0;
         fusion_thread_index < pipeline_params.NUM_THREADS;
         fusion_thread_index++)
      fusion_threads.push_back(
          move(unique_ptr<FusionThread<int>>(new FusionThread<int>(
    ...
    vector<int> fused_solution =
        parallelFuse(fusion_threads, pipeline_params,
                     vector<int>(image.cols * image.rows, 0));
  }

  // sequential fusion move
  if (false) {
    shared_ptr<SegmentationProposalGenerator> proposal_generator(
        new SegmentationProposalGenerator(image));
    shared_ptr<SegmentationFusionSolver> fusion_solver(
        new SegmentationFusionSolver(image));
...

      {
        ...
        for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
          solution_image.at<Vec3b>(pixel / IMAGE_WIDTH, pixel % IMAGE_WIDTH) =
              color_table[current_solution[pixel]];
        }
        imwrite("Test/sequential_solution_image_" + to_string(iteration) +
                    ".png",
                solution_image);
      }
    }
