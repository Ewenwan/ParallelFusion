...
{  
  ...
  flow_out_str.write(reinterpret_cast<char *>(&flow_values_float[0]), sizeof(float) * IMAGE_WIDTH * IMAGE_HEIGHT * 2);
  flow_out_str.close();
}
...
std::vector<std::pair<double, double> > disturbFlows(const std::vector<std::pair<double, double> > &flows, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const double radius)
{
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++)
    disturbed_flows[pixel] = make_pair(flows[pixel].first + cv_utils::drawFromNormalDistribution(0, radius), flows[pixel].second + cv_utils::drawFromNormalDistribution(0, radius));
...
double calcFlowsDiff(const std::vector<std::pair<double, double> > &flows_1, const std::vector<std::pair<double, double> > &flows_2, const int IMAGE_WIDTH, const int IMAGE_HEIGHT)
{
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    if (onBorder(pixel, IMAGE_WIDTH, IMAGE_HEIGHT))
      continue;
    if (abs(flows_1[pixel].first) > IMAGE_WIDTH || abs(flows_1[pixel].second) > IMAGE_HEIGHT || abs(flows_2[pixel].first) > IMAGE_WIDTH || abs(flows_2[pixel].second) > IMAGE_HEIGHT)
      continue;
    flow_diff2_sum += pow(flows_1[pixel].first - flows_2[pixel].first, 2) + pow(flows_1[pixel].second - flows_2[pixel].second, 2);
    num_valid_pixels++;
  }
...
bool onBorder(const int pixel, const int IMAGE_WIDTH, const int IMAGE_HEIGHT)
{
  ...
  if (pixel % IMAGE_WIDTH < IMAGE_BORDER || pixel % IMAGE_WIDTH > IMAGE_WIDTH - 1 - IMAGE_BORDER || pixel / IMAGE_WIDTH < IMAGE_BORDER || pixel / IMAGE_WIDTH > IMAGE_HEIGHT - 1 - IMAGE_BORDER)
    return true;
  else
    return false;
}
