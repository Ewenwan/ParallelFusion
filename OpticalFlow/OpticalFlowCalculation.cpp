...
calcFlowsPyrLK(const Mat &image_1, const Mat &image_2, const int NUM_LEVELS) {
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++)
    points_1.at<Vec2f>(0, pixel) =
        Vec2f(pixel % IMAGE_WIDTH, pixel / IMAGE_WIDTH);

  Mat points_2;
  Mat status;
  Mat err;
  calcOpticalFlowPyrLK(image_1, image_2, points_1, points_2, status, err,
                       Size(21, 21), NUM_LEVELS);
...
calcFlowsFarneback(const cv::Mat &image_1, const cv::Mat &image_2,
                   const int NUM_LEVELS, const int POLY_N, const int FLAGS) {
  ...
  Mat flow_image;
  calcOpticalFlowFarneback(image_1_gray, image_2_gray, flow_image, PYR_SCALE,
                           NUM_LEVELS, WINDOW_SIZE, NUM_ITERATIONS, POLY_N,
                           POLY_SIGMA, FLAGS);
...
std::vector<std::pair<double, double>>
calcFlowsLayerWise(const cv::Mat &image_1, const cv::Mat &image_2,
                   const double ALPHA, const double RATIO) {
  //   const int IMAGE_WIDTH = image_1.cols;
  //   const int IMAGE_HEIGHT = image_1.rows;
  ...
  //   for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
  //     Vec3b color_1 = image_1.at<Vec3b>(pixel / IMAGE_WIDTH, pixel %
  //     IMAGE_WIDTH);
  //     for (int c = 0; c < 3; c++)
  //       d_image_1[pixel * 3 + c] = 1.0 * color_1[c] / 255;
  //     Vec3b color_2 = image_2.at<Vec3b>(pixel / IMAGE_WIDTH, pixel %
  //     IMAGE_WIDTH);
  //     for (int c = 0; c < 3; c++)
  ...
  //   OpticalFlow::Coarse2FineFlow(vx, vy, warped_image, d_image_1, d_image_2,
  //   weight_image, ALPHA, RATIO, MIN_WIDTH, NUM_OUTER_FP_ITERATIONS,
  //   NUM_INNER_FP_ITERATIONS, NUM_CG_ITERATIONS);
...
std::vector<std::pair<double, double>> calcFlowsBrox(const cv::Mat &image_1,
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    Vec2f flow =
        flow_image_CPU.at<Vec2f>(pixel / IMAGE_WIDTH, pixel % IMAGE_WIDTH);
    flows[pixel].first = flow[0];
    flows[pixel].second = flow[1];
  }
  return flows;
...
std::vector<std::pair<double, double>>
calcFlowsNearestNeighbor(const cv::Mat &image_1, const cv::Mat &image_2,
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    flows[pixel].first =
        nearest_neighbor_field[pixel] % IMAGE_WIDTH - pixel % IMAGE_WIDTH;
    flows[pixel].second =
        nearest_neighbor_field[pixel] / IMAGE_WIDTH - pixel / IMAGE_WIDTH;
  }
  return flows;
