...
void OpticalFlowProposalGenerator::writeSolution(const std::vector<pair<double, double> > &solution, const int iteration, const int thread_index) const
{
...
LabelSpace<pair<double, double> > OpticalFlowProposalGenerator::generateProposalMoveAround()
{
  ...
  for (int proposal_solution_index = 0; proposal_solution_index < NUM_PROPOSAL_SOLUTIONS; proposal_solution_index++) {
    ...
    vector<pair<double, double> > proposal_flows = shiftFlows(current_solution_, IMAGE_WIDTH_, IMAGE_HEIGHT_, shift_x, shift_y);
    proposed_label_space += LabelSpace<pair<double, double> >(proposal_flows);

    imwrite("Test/proposal_flows_MoveAround_" + to_string(proposal_solution_index) + ".png", drawFlows(proposal_flows, IMAGE_WIDTH_, IMAGE_HEIGHT_));
  }
...
LabelSpace<pair<double, double> > OpticalFlowProposalGenerator::generateProposalShift()
{
  ...
  for (int proposal_solution_index = 0; proposal_solution_index < NUM_PROPOSAL_SOLUTIONS; proposal_solution_index++) {
    ...
    imwrite("Test/proposal_flows_Shift_" + to_string(proposal_solution_index) + ".png", drawFlows(proposal_flows, IMAGE_WIDTH_, IMAGE_HEIGHT_));
  }
  return proposed_label_space;
}
...
LabelSpace<pair<double, double> > OpticalFlowProposalGenerator::generateProposalCluster()
{
  ...
  for (int y = IMAGE_BORDER; y < IMAGE_HEIGHT_ - IMAGE_BORDER; y++)
    for (int x = IMAGE_BORDER; x < IMAGE_WIDTH_ - IMAGE_BORDER; x++)
      flow_points.at<Vec2f>((y - IMAGE_BORDER) * (IMAGE_WIDTH_ - IMAGE_BORDER * 2) + (x - IMAGE_BORDER)) = Vec2f(current_solution_[y * IMAGE_WIDTH_ + x].first, current_solution_[y * IMAGE_WIDTH_ + x].second);
  ...
  for (int proposal_solution_index = 0; proposal_solution_index < NUM_PROPOSAL_SOLUTIONS; proposal_solution_index++) {
    ...
    imwrite("Test/proposal_flows_Cluster_" + to_string(proposal_solution_index) + ".png", drawFlows(proposal_flows, IMAGE_WIDTH_, IMAGE_HEIGHT_));
  }
  return proposal_label_space;
}
