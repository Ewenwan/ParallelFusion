...
DEFINE_int32(num_threads, 5, "The number of threads.");
DEFINE_int32(num_iterations, 3, "The number of iterations.");
DEFINE_int32(num_proposed_solutions, 4, "The number of proposed solution for each thread.");
DEFINE_bool(write_log, false, "Write log file or not.");
...
int main(int argc, char *argv[])
{
  ...
  if (FLAGS_evaluation == false) {
    ...
    double ground_truth_energy = fusion_solver.checkSolutionEnergy(ground_truth_flows);
    ...
    imwrite("Test/Comparison/ground_truth.png", drawFlows(ground_truth_flows, IMAGE_WIDTH, IMAGE_HEIGHT));
    {
      ...
      cout << "NearestNeighbor energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;

      if (FLAGS_write_log) {
        LOG(INFO) << "NearestNeighbor error: " << calcFlowsDiff(flows, ground_truth_flows, IMAGE_WIDTH, IMAGE_HEIGHT) << endl;
        LOG(INFO) << "NearestNeighbor energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
      }
    ...
    }
    {
      ...
      cout << "PyrLK energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;

      if (FLAGS_write_log) {
	LOG(INFO) << "PyrLK error: " << calcFlowsDiff(flows, ground_truth_flows, IMAGE_WIDTH, IMAGE_HEIGHT) << endl;
	LOG(INFO) << "PyrLK energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
      }
    ...
    }
    {
      ...
      cout << "Farneback energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;

      if (FLAGS_write_log) {
	LOG(INFO) << "Farneback error: " << calcFlowsDiff(flows, ground_truth_flows, IMAGE_WIDTH, IMAGE_HEIGHT) << endl;
	LOG(INFO) << "Farneback energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
      }
    ...
    }
    {
      ...
      cout << "LayerWise energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;

      if (FLAGS_write_log) {
	LOG(INFO) << "LayerWise error: " << calcFlowsDiff(flows, ground_truth_flows, IMAGE_WIDTH, IMAGE_HEIGHT) << endl;
	LOG(INFO) << "LayerWise energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
      }
  ...
  //   cout << "Brox energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
  // }
  ...
  //   double energy;
  //   vector<pair<double, double> > solution = fusion_solver.solve(label_space, energy);
  //   double checked_energy  = fusion_solver.checkSolutionEnergy(solution);
  //   cout << energy << '\t' << checked_energy << endl;
  //   if (energy < ground_truth_energy)
  //     cout << c << '\t' << energy << '\t' << checked_energy << endl;
  // }
  // exit(1);
  // cout << "energy: " << fusion_solver.checkSolutionEnergy(flows) << endl;
  // cout << "energy: " << fusion_solver.checkSolutionEnergy(vector<pair<double, double> >(image_1.cols * image_1.rows, make_pair(0, 0))) << endl;
  // exit(1);
  ...
  if (FLAGS_write_log) {
    ...
    LOG(INFO) << "final energy: " << fusion_solver.checkSolutionEnergy(fused_solution) << endl;;
