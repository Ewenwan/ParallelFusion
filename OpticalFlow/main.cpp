...
// }
...

  {
    VideoWriter video_writer(
        solution_path + "solution_video_global.mp4",
        VideoWriter::fourcc('X', '2', '6', '4'), FPS,
        Size(IMAGE_WIDTH - BORDER_WIDTH * 2, IMAGE_HEIGHT - BORDER_WIDTH * 2));
    if (video_writer.isOpened()) {
      Mat previous_image =
          drawFlows(vector<pair<double, double>>(IMAGE_WIDTH * IMAGE_HEIGHT),
                    IMAGE_WIDTH, IMAGE_HEIGHT);
      ...
      for (int frame = 0; frame < max_time * FPS; frame++) {
        ...
        if (time_global_solution_map.count(time) > 0) {
          Mat image =
              imread(solution_path + "solution_image_" +
                     to_string(time_global_solution_map[time].thread_id) + "_" +
                     to_string(time) + ".png");
...
ParallelFusion::ParallelFusionPipeline<LABELSPACE>
...
for (auto thread_id = 0;
  ...
  vector<Solution> solutions =
      dynamic_pointer_cast<OpticalFlowProposalGenerator>(generators[thread_id])
          ->getSolutions();
  ...
  for (vector<Solution>::iterator solution_it = solutions.begin();
    ...
    imwrite("Test/solution_image_" + to_string(solution_it->thread_id) + "_" +
