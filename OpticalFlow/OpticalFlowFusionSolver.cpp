...
OpticalFlowFusionSolver::OpticalFlowFusionSolver(const cv::Mat &image_1, const cv::Mat &image_2) : image_1_(image_1.clone()), image_2_(image_2.clone()), IMAGE_WIDTH_(image_1.cols), IMAGE_HEIGHT_(image_1.rows)
{
  calcNeighborInfo();
...
vector<double> OpticalFlowFusionSolver::getImageColor(const bool left_or_right, const double x, const double y) const
{
...
double OpticalFlowFusionSolver::calcDataCost(const int pixel, const pair<double, double> &flow) const
{
  ...
  vector<double> color_2 = getImageColor(false, pixel % IMAGE_WIDTH_ + flow.first, pixel / IMAGE_WIDTH_ + flow.second);
...
{
  if (abs(flow_1.first) > IMAGE_WIDTH_ || abs(flow_1.second) > IMAGE_HEIGHT_ || abs(flow_2.first) > IMAGE_WIDTH_ || abs(flow_2.second) > IMAGE_HEIGHT_)
    return 0;

  const double NU = 0.2;
  double pixel_distance = sqrt(pow(pixel_1 % IMAGE_WIDTH_ - pixel_2 % IMAGE_WIDTH_, 2) + pow(pixel_1 / IMAGE_WIDTH_ - pixel_2 / IMAGE_WIDTH_, 2));
...
vector<pair<double, double> > OpticalFlowFusionSolver::solve(const LabelSpace<pair<double, double> > &label_space, double &energy) const
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    nodes[pixel] = energy_function->AddNode(TypeGeneral::LocalSize(pixel_num_labels[pixel]), TypeGeneral::NodeData(&data_cost[0]));
  }
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    for (map<int, double>::const_iterator neighbor_pixel_it = pixel_neighbor_weights_[pixel].begin(); neighbor_pixel_it != pixel_neighbor_weights_[pixel].end(); neighbor_pixel_it++) {
      ...
      for (int proposal_index = 0; proposal_index < pixel_num_labels[pixel]; proposal_index++) {
        for (int neighbor_proposal_index = 0; neighbor_proposal_index < pixel_num_labels[neighbor_pixel]; neighbor_proposal_index++) {
          pair<double, double> label = label_space_vec[pixel][proposal_index];
          pair<double, double> neighbor_label = label_space_vec[neighbor_pixel][neighbor_proposal_index];
	  f(proposal_index, neighbor_proposal_index) = calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label) * max(neighbor_pixel_it->second, 0.0) * SMOOTHNESS_TERM_WEIGHT_;
        }
      }
      FunctionIdentifier id = gm.addFunction(f);
	...
	for (int neighbor_proposal_index = 0; neighbor_proposal_index < pixel_num_labels[neighbor_pixel]; neighbor_proposal_index++) {
	  pair<double, double> label = label_space_vec[pixel][proposal_index];
          pair<double, double> neighbor_label = label_space_vec[neighbor_pixel][neighbor_proposal_index];
          smoothness_cost[proposal_index + neighbor_proposal_index * pixel_num_labels[pixel]] = calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label) * max(neighbor_pixel_it->second, 0.0) * SMOOTHNESS_TERM_WEIGHT_;
	}
      }
      energy_function->AddEdge(nodes[pixel], nodes[neighbor_pixel], TypeGeneral::EdgeData(TypeGeneral::GENERAL, &smoothness_cost[0]));
    }
  }
...
double OpticalFlowFusionSolver::checkSolutionEnergy(const vector<pair<double, double> > &solution)
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    for (map<int, double>::const_iterator neighbor_pixel_it = pixel_neighbor_weights_[pixel].begin(); neighbor_pixel_it != pixel_neighbor_weights_[pixel].end(); neighbor_pixel_it++) {
      ...
      double smoothness_cost = calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label) * max(neighbor_pixel_it->second, 0.0) * SMOOTHNESS_TERM_WEIGHT_;
...
void OpticalFlowFusionSolver::calcNeighborInfo()
{
  ...
  calcWindowMeansAndVars(guidance_image_values, IMAGE_WIDTH_, IMAGE_HEIGHT_, NEIGHBOR_WINDOW_SIZE, guidance_image_means, guidance_image_vars);
  ...
  for (int pixel = 0; pixel < IMAGE_WIDTH_ * IMAGE_HEIGHT_; pixel++) {
    ...
    for (vector<int>::const_iterator window_pixel_it = window_pixels.begin(); window_pixel_it != window_pixels.end(); window_pixel_it++) {
      for (vector<int>::const_iterator other_window_pixel_it = window_pixels.begin(); other_window_pixel_it != window_pixels.end(); other_window_pixel_it++) {
	...
	for (int c_1 = 0; c_1 < 3; c_1++)
          for (int c_2 = 0; c_2 < 3; c_2++)
	    weight += (color_1[c_1] - guidance_image_mean[c_1]) * guidance_image_var_inverse[c_1][c_2] * (color_2[c_2] - guidance_image_mean[c_2]);
	weight = (weight + 1) / pow(NEIGHBOR_WINDOW_SIZE, 4);

	if (abs(*window_pixel_it % IMAGE_WIDTH_ - *other_window_pixel_it % IMAGE_WIDTH_) <= 1 && abs(*window_pixel_it / IMAGE_WIDTH_ - *other_window_pixel_it / IMAGE_WIDTH_) <= 1)
