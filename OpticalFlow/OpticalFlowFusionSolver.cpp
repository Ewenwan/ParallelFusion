...
vector<double> OpticalFlowFusionSolver::readColorVec(const bool left_or_right,
                                                     const int x,
                                                     const int y) const {
  Vec3b color_high_freq = left_or_right ? image_1_high_freq_.at<Vec3b>(y, x)
                                        : image_2_high_freq_.at<Vec3b>(y, x);
  Vec3b color =
      left_or_right ? image_1_.at<Vec3b>(y, x) : image_2_.at<Vec3b>(y, x);
  uchar color_gray = left_or_right ? image_1_gray_.at<uchar>(y, x)
                                   : image_2_gray_.at<uchar>(y, x);
...
vector<double> OpticalFlowFusionSolver::getImageColor(const bool left_or_right,
                                                      const double x,
  ...
  if (lower_x == upper_x && lower_y == upper_y)
    ...
    for (int c = 0; c < color_1.size(); c++)
      average_color[c] =
          color_1[c] * (upper_y - y) + color_2[c] * (y - lower_y);
    return average_color;
  } else if (lower_y == upper_y) {
    vector<double> average_color(color_1.size(), 0);
    for (int c = 0; c < color_1.size(); c++)
      average_color[c] =
          color_1[c] * (upper_x - x) + color_3[c] * (x - lower_x);
    return average_color;
  } else {
    ...
    vector<double> average_color(color_1.size(), 0);
    for (int c = 0; c < color_1.size(); c++)
      average_color[c] = color_1[c] * area_4 + color_2[c] * area_3 +
                         color_3[c] * area_2 + color_4[c] * area_1;
    return average_color;
  }
}

double
OpticalFlowFusionSolver::calcDataCost(const int pixel,
                                      const pair<double, double> &flow) const {
  if (abs(flow.first) > IMAGE_WIDTH_ || abs(flow.second) > IMAGE_HEIGHT_)
    return 0;

  const int MU = 16;
  vector<double> color_1 =
      getImageColor(true, pixel % IMAGE_WIDTH_, pixel / IMAGE_WIDTH_);
  vector<double> color_2 =
      getImageColor(false, pixel % IMAGE_WIDTH_ + flow.first,
                    pixel / IMAGE_WIDTH_ + flow.second);
  double color_distance = cv_utils::calcDistance(color_1, color_2);
  double data_cost =
      pow(color_distance, 2) / (pow(color_distance, 2) + pow(MU, 2));
...
double OpticalFlowFusionSolver::calcSmoothnessCost(
    const int pixel_1, const int pixel_2, const pair<double, double> &flow_1,
    const pair<double, double> &flow_2) const {
  if (abs(flow_1.first) > IMAGE_WIDTH_ || abs(flow_1.second) > IMAGE_HEIGHT_ ||
      abs(flow_2.first) > IMAGE_WIDTH_ || abs(flow_2.second) > IMAGE_HEIGHT_)
    return 0;
  ...
  double smoothness_cost =
      log(1 + pow(flow_1.first - flow_2.first, 2) / (2 * pow(NU, 2))) +
      log(1 + pow(flow_1.second - flow_2.second, 2) / (2 * pow(NU, 2)));
  // double smoothness_cost = pow(pow(flow_1.first - flow_2.first, 2) +
  // pow(flow_1.second - flow_2.second, 2) + 0.000001, 0.5);

  vector<double> color_1 =
      getImageColor(true, pixel_1 % IMAGE_WIDTH_, pixel_1 / IMAGE_WIDTH_);
  vector<double> color_2 =
      getImageColor(false, pixel_2 % IMAGE_WIDTH_, pixel_2 / IMAGE_WIDTH_);
  double color_distance = cv_utils::calcDistance(color_1, color_2);
  // double weight = exp(-pow(color_distance, 0.8));
  //  double color_difference =
  //  calcDistance(convertVec3bToVector(image_1_.at<Vec3b>(pixel_1 /
  //  IMAGE_WIDTH_, pixel_1 % IMAGE_WIDTH_)),
  //  convertVec3bToVector(image_1_.at<Vec3b>(pixel_2 / IMAGE_WIDTH_, pixel_2 %
  //  IMAGE_WIDTH_)));
  // double weight = 1; //0.01 * exp(-pow(color_difference / 30, 2) / 2);
  double weight = color_distance < 30 ? 0.024 : 0.008;
  smoothness_cost *= weight;
  // double flow_distance = sqrt(pow(flow_1.first - flow_2.first, 2) +
  // pow(flow_1.second - flow_2.second, 2));

  // if (flow_distance > IMAGE_WIDTH_ + IMAGE_HEIGHT_)
  //   return 0;
  // //cout << flow_distance << endl;
  // double smoothness_cost = log(1 + pow(flow_distance, 2) / (2 * pow(NU, 2)))
  // / sqrt(pow(pixel_1 % IMAGE_WIDTH_ - pixel_2 % IMAGE_WIDTH_, 2) +
  // pow(pixel_1 / IMAGE_WIDTH_ - pixel_2 / IMAGE_WIDTH_, 2));
  // if (smoothness_cost > 2)
  //   cout << pixel_1 << '\t' << pixel_2 << '\t' << flow_distance << '\t' <<
  //   smoothness_cost << endl;
  return smoothness_cost;
}

void OpticalFlowFusionSolver::solve(
  ...
  vector<vector<pair<double, double>>> label_space_vec =
      proposals.getLabelSpace();
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    nodes[pixel] = energy_function->AddNode(
        TypeGeneral::LocalSize(pixel_num_labels[pixel]),
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    for (map<int, double>::const_iterator neighbor_pixel_it =
      ...
      //        neighbor_proposal_index < pixel_num_labels[neighbor_pixel];
      //        neighbor_proposal_index++) {
      //     pair<double, double> label =
      //     label_space_vec[pixel][proposal_index];
      //     pair<double, double> neighbor_label =
      //     label_space_vec[neighbor_pixel][neighbor_proposal_index];
      //     f(proposal_index, neighbor_proposal_index) =
      //             calcSmoothnessCost(pixel, neighbor_pixel, label,
      //             neighbor_label) *
      //             max(neighbor_pixel_it->second, 0.0) *
      //             SMOOTHNESS_TERM_WEIGHT_;
      //   }
      // }
      // FunctionIdentifier id = gm.addFunction(f);
      ...
      vector<double> smoothness_cost(
          pixel_num_labels[pixel] * pixel_num_labels[neighbor_pixel], 0);
      for (int proposal_index = 0; proposal_index < pixel_num_labels[pixel];
           proposal_index++) {
        for (int neighbor_proposal_index = 0;
             neighbor_proposal_index < pixel_num_labels[neighbor_pixel];
          ...
          pair<double, double> neighbor_label =
              label_space_vec[neighbor_pixel][neighbor_proposal_index];
          smoothness_cost[proposal_index +
                          neighbor_proposal_index * pixel_num_labels[pixel]] =
              calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label) *
              max(neighbor_pixel_it->second, 0.0) * SMOOTHNESS_TERM_WEIGHT_;
        }
      }
      energy_function->AddEdge(
          nodes[pixel], nodes[neighbor_pixel],
  ...
  if (false) {
    ...
    // opengm::TRWSi<Model, opengm::Minimizer>::VerboseVisitorType
    // verbose_visitor;
    // solver.infer(verbose_visitor);
    // solver.arg(selected_proposal_indices);
    // cout << "energy: " << solver.value() << " lower bound: " <<
    // solver.bound() << endl;

    // double energy = solver.value();

    // for (int pixel = 0; pixel < NUM_PIXELS; pixel++)
    //   fused_solution[pixel] =
    //   label_space_vec[pixel][selected_proposal_indices[pixel]];
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++)
    fused_solution[pixel] =
        label_space_vec[pixel][energy_function->GetSolution(nodes[pixel])];
...
void OpticalFlowFusionSolver::calcNeighborInfo() {
  ...
  calcWindowMeansAndVars(guidance_image_values, IMAGE_WIDTH_, IMAGE_HEIGHT_,
                         NEIGHBOR_WINDOW_SIZE, guidance_image_means,
                         guidance_image_vars);

  double epsilon = 0.00001;
  for (int pixel = 0; pixel < IMAGE_WIDTH_ * IMAGE_HEIGHT_; pixel++) {
    vector<int> window_pixels; // = cv_utils::findWindowPixels(pixel,
    ...
    for (vector<int>::const_iterator window_pixel_it = window_pixels.begin();
      ...
      for (vector<int>::const_iterator other_window_pixel_it =
               window_pixels.begin();
           other_window_pixel_it != window_pixels.end();
           other_window_pixel_it++) {
        ...
        if (abs(*window_pixel_it % IMAGE_WIDTH_ -
                *other_window_pixel_it % IMAGE_WIDTH_) <= 1 &&
            abs(*window_pixel_it / IMAGE_WIDTH_ -
                *other_window_pixel_it / IMAGE_WIDTH_) <= 1)
          pixel_neighbor_weights_[*window_pixel_it][*other_window_pixel_it] =
              1; // += weight;
      }
    }
  }
...
double
OpticalFlowFusionSolver::evaluateEnergy(const LABELSPACE &solution) const {
  const int NUM_PIXELS = IMAGE_WIDTH_ * IMAGE_HEIGHT_;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
    ...
    //   smoothness_energy += calcSmoothnessCost(pixel, *neighbor_pixel_it,
    //   label, neighbor_label) * SMOOTHNESS_TERM_WEIGHT_;
    //   smoothness_energy_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
    //   IMAGE_WIDTH_) = min(calcSmoothnessCost(pixel, *neighbor_pixel_it,
    //   label, neighbor_label) * SMOOTHNESS_TERM_WEIGHT_ / 3.0 * 255, 255.0);
    // }
    ...
    for (map<int, double>::const_iterator neighbor_pixel_it =
      ...
      double cost_temp =
          calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label);
      // if (pixel == 231 * IMAGE_WIDTH_ + 244 && neighbor_pixel == 231 *
      // IMAGE_WIDTH_ + 245) {
      ...
      // cout << calcSmoothnessCost(pixel, neighbor_pixel, label,
      // neighbor_label) * neighbor_pixel_it->second * SMOOTHNESS_TERM_WEIGHT_ /
      // 0.3 * 255 << endl;
      // if (calcSmoothnessCost(pixel, neighbor_pixel, label, neighbor_label) >
      // 0.1)
      // cout << pixel % IMAGE_WIDTH_ << '\t' << pixel / IMAGE_WIDTH_ << '\t' <<
      // neighbor_pixel % IMAGE_WIDTH_ << '\t' << neighbor_pixel / IMAGE_WIDTH_
      // << '\t' << neighbor_pixel_it->second << endl;
      // if (cost_temp > 3) {
      //  smoothness_energy_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
      // IMAGE_WIDTH_) = min(cost_temp / 0.2 * 255, 255.0);
      //  smoothness_energy_image.at<uchar>(neighbor_pixel / IMAGE_WIDTH_,
      // neighbor_pixel % IMAGE_WIDTH_) = min(cost_temp / 0.2 * 255, 255.0);
      // }
    }
    // cout << pixel_smoothness_cost_sum << endl;
  }
  //  imwrite("Test/smoothness_energy_image.png", smoothness_energy_image);
  cout << "data energy: " << data_energy << '\t'
       << "smoothness energy: " << smoothness_energy << endl;
