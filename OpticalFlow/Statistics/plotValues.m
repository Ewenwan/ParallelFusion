...
%figureHandle = gcf;
figure(1);
plot(sequential(:, 1), log(sequential(:, 2)), 'm', Victor(:, 1), log(Victor(:, 2)), 'c', solution_exchange(:, 1), log(solution_exchange(:, 2)), 'b', multiway(:, 1), log(multiway(:, 2)), 'g', full(:, 1), log(full(:, 2)), 'r', hierarchy(:, 1), log(hierarchy(:, 2)), 'y', 'LineWidth', 2);
