...
for i = 2:size(raw_values, 1)
    if (raw_values(i, 3) == 4)
        if (use_final_fusion == 1)
            final_fusion_time_energy_value = raw_values(i, :);    
        end
        continue;
    end
    ...
    if (raw_values(i, 1) > start_time + 428)
        break;
    end
end
if (use_final_fusion == 1)
    final_fusion_time_energy_value(1) = final_fusion_time_energy_value(1) - start_time;
