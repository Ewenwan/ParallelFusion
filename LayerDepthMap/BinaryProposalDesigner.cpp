...
BinaryProposalDesigner::BinaryProposalDesigner(const Mat &image, const vector<double> &point_cloud, const vector<double> &normals, const vector<double> &camera_parameters, const int num_layers, const RepresenterPenalties penalties, const DataStatistics statistics, const int scene_index) : image_(image), point_cloud_(point_cloud), normals_(normals), IMAGE_WIDTH_(image.cols), IMAGE_HEIGHT_(image.rows), CAMERA_PARAMETERS_(camera_parameters), penalties_(penalties), statistics_(statistics), NUM_PIXELS_(image.cols * image.rows), NUM_LAYERS_(num_layers), SCENE_INDEX_(scene_index), NUM_ALL_PROPOSAL_ITERATIONS_(3)
{
...
BinaryProposalDesigner::~BinaryProposalDesigner()
{
}

void BinaryProposalDesigner::setCurrentSolution(const LayerLabelSpace &current_solution_label_space)
{
...
bool BinaryProposalDesigner::getProposal(LayerLabelSpace &label_space, const int proposal_type_index)
{
...
bool BinaryProposalDesigner::getProposal(int &iteration, vector<vector<int> > &proposal_labels, int &proposal_num_surfaces, map<int, Segment> &proposal_segments, string &proposal_type)
{
  // if (iteration != 2) {
  //   cout << current_solution_num_surfaces_ << '\t' << current_solution_segments_.size() << endl;
  //   exit(1);
  // }
  bool use_proposal_all = false;
  if (use_proposal_all) {
    generateSegmentAddingProposalAll();
...
bool BinaryProposalDesigner::getProposalAll(int &iteration, vector<vector<int> > &proposal_labels, int &proposal_num_surfaces, map<int, Segment> &proposal_segments, string &proposal_type)
{
  // if (iteration != 2) {
  //   cout << current_solution_num_surfaces_ << '\t' << current_solution_segments_.size() << endl;
  //   exit(1);
...
bool BinaryProposalDesigner::getLastProposal(vector<vector<int> > &proposal_labels, int &proposal_num_surfaces, map<int, Segment> &proposal_segments, string &proposal_type)
{
  NUM_LAYERS_++;
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
...
// string BinaryProposalDesigner::getProposalType()
// {
...
vector<int> BinaryProposalDesigner::getInitialLabels()
{
  //generateEmptyRepresentationProposal();
  return proposal_labels_[0];
}

void BinaryProposalDesigner::convertProposalLabelsFormat()
{
...
void BinaryProposalDesigner::addIndicatorVariables(const int num_indicator_variables)
{
...
void BinaryProposalDesigner::addSegmentLayerProposals(const bool restrict_segment_in_one_layer)
{
  //int num = num_indicator_variables == -1 ? pow(NUM_SURFACES_ + 1, NUM_LAYERS_) : num_indicator_variables;
...
bool BinaryProposalDesigner::checkLabelValidity(const int pixel, const int label, const int num_surfaces, const map<int, Segment> &segments)
{
...
void BinaryProposalDesigner::writeSegmentationImage(const vector<int> &segmentation, const string filename)
{ 
...
void BinaryProposalDesigner::writeSegmentationImage(const Mat &segmentation_image, const string filename)
{
  map<int, int> segment_center_x;
  map<int, int> segment_center_y;
  map<int, int> segment_pixel_counter;
...
bool BinaryProposalDesigner::generateSegmentRefittingProposal()
{
  ...
  for (map<int, map<int, vector<int> > >::const_iterator segment_it = segment_layer_visible_pixels.begin(); segment_it != segment_layer_visible_pixels.end(); segment_it++) {
    ...
    if (rand() % 2 == 0 || visible_pixels.size() > statistics_.bspline_surface_num_pixels_threshold) {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label / static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - layer_index)) % (current_solution_num_surfaces_ + 1);
      if (pixel_layer_surfaces_map.count(layer_index) == 0) {
	if (surface_id < current_solution_num_surfaces_)
    ...
    //assert(valid_pixel_proposals.size() <= 1);
    for (int i = valid_pixel_proposals.size(); i < 2; i++)
      valid_pixel_proposals.push_back(convertToProposalLabel(current_solution_label));
...
// bool BinaryProposalDesigner::generateBSplineSurfaceProposal()
// {
...
bool BinaryProposalDesigner::generateSingleSurfaceExpansionProposal(const int denoted_expansion_segment_id, const int denoted_expansion_type)
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    if (expansion_type == 0) {
      ...
      if (surface_id < current_solution_num_surfaces_ && surface_id != expansion_segment_id)
        for (int target_layer_index = expansion_segment_layer_index - 1; target_layer_index < expansion_segment_layer_index; target_layer_index++)
    ...
    //assert(valid_pixel_proposals.size() <= 1);
    for (int i = valid_pixel_proposals.size(); i < 2; i++)
      valid_pixel_proposals.push_back(current_solution_label);
...
bool BinaryProposalDesigner::generateLayerSwapProposal()
{
      ...
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it = neighbor_pixels.begin(); neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
          ...
          for (set<int>::const_iterator segment_it = pixel_segment_indices_map[pixel].begin(); segment_it != pixel_segment_indices_map[pixel].end(); segment_it++) {
            if (new_pixel_segment_indices_map[*neighbor_pixel_it].count(*segment_it) > 0 || new_pixel_segment_indices_map[*neighbor_pixel_it].size() > 0)
              continue;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_ - 1; layer_index++) {
      ...
      if (segments.size() > 0) {
	if (layer_index > 0)
	  pixel_layer_surfaces_map[layer_index - 1].insert(*segments.begin());
	else
	  pixel_layer_surfaces_map[layer_index + 1].insert(*segments.begin());
      }
      pixel_layer_surfaces_map[layer_index].insert(proposal_num_surfaces_);
    ...
    valid_pixel_proposals[1] = current_solution_label;
...
bool BinaryProposalDesigner::generateConcaveHullProposal(const bool consider_background)
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label / static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_  - 1 - layer_index)) % (current_solution_num_surfaces_ + 1);
      if (layer_pixel_inpainting_surface_ids[layer_index][pixel] != -1 && layer_pixel_inpainting_surface_ids[layer_index][pixel] != surface_id && layer_index == NUM_LAYERS_ - 1) {
        pixel_layer_surfaces_map[layer_index].insert(layer_pixel_inpainting_surface_ids[layer_index][pixel]);
	pixel_layer_surfaces_map[max(layer_index - 1, 0)].insert(surface_id);
      }
    }
...
// bool BinaryProposalDesigner::generateCleanUpProposal()
// {
...
bool BinaryProposalDesigner::generateSegmentAddingProposal(const int denoted_segment_adding_type)
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    map<int, set<int> > pixel_layer_surfaces_map;
    for (set<int>::const_iterator segment_it = pixel_segment_indices_map[pixel].begin(); segment_it != pixel_segment_indices_map[pixel].end(); segment_it++) {
      pixel_layer_surfaces_map[max(foremost_empty_layer_index, 0)].insert(*segment_it);
      if (pixel_layer_surfaces_map[max(foremost_empty_layer_index, 0)].size() == 2 - segment_adding_type)
	break;
    }
    if (segment_adding_type == 0 && pixel_segment_indices_map[pixel].size() == 0)
      pixel_layer_surfaces_map[max(foremost_empty_layer_index, 0)].insert(current_solution_num_surfaces_ + rand() % (proposal_num_surfaces_ - current_solution_num_surfaces_));
    ...
    if (segment_adding_type == 1) {
      ...
      //assert(valid_pixel_proposals.size() == 2);
    }
    // if (valid_pixel_proposals.size() > 1)
    //   cout << "yes" << endl;
    if (valid_pixel_proposals.size() == 0) {
...
bool BinaryProposalDesigner::generateSegmentAddingProposalAll(const int denoted_segment_adding_type)
{
  cout << "generate segment adding proposal" << endl;
  proposal_type_ = "segment_adding_proposal";
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++)
      for (int segment_id = 0; segment_id <= proposal_num_surfaces_; segment_id++)
	pixel_layer_surfaces_map[layer_index].insert(segment_id);
...
bool BinaryProposalDesigner::generateStructureExpansionProposal(const int denoted_expansion_layer_index, const int denoted_expansion_pixel)
{
...
bool BinaryProposalDesigner::generateBackwardMergingProposal(const int denoted_target_layer_index)
{
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    if (pixel_segment_indices_map[pixel].size() > 0)
      pixel_layer_surfaces_map[target_layer_index].insert(*pixel_segment_indices_map[pixel].begin());
...
bool BinaryProposalDesigner::generateBoundaryRefinementProposal()
{
...
// bool BinaryProposalDesigner::generateContourCompletionProposal()
// {
...
bool BinaryProposalDesigner::generateDesiredProposal()
{
...
bool BinaryProposalDesigner::generateSingleProposal()
{
...
// bool BinaryProposalDesigner::generateRansacProposal()
// {
...
void BinaryProposalDesigner::initializeCurrentSolution()
{
...
vector<int> BinaryProposalDesigner::calcPixelProposals(const int num_surfaces, const map<int, set<int> > &pixel_layer_surfaces_map)
{
...
int BinaryProposalDesigner::convertToProposalLabel(const int current_solution_label)
{
...
vector<int> BinaryProposalDesigner::getCurrentSolutionIndices()
{
