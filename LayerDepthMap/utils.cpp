...
vector<vector<double> > recoverSurfaceModels(Mat &surface_id_image, const vector<double> &point_cloud, Mat &disp_image, const bool use_3D_positions)
{
  int width = surface_id_image.cols;
    ...
    vector<vector<double> > surfacemodels(width * height);

    const int BORDER_SIZE = 0;
    vector<bool> border_mask(width * height, false);
    for (int y = BORDER_SIZE; y < height - BORDER_SIZE; y++) {
      uchar *data = surface_id_image.ptr<uchar>(y);
	uchar *neighbor_data = surface_id_image.ptr<uchar>(y + 1);
        for (int x = BORDER_SIZE; x < width - BORDER_SIZE; x++) {
            int index = (data[x * 3 + 0] * 256 * 256 + data[x * 3 + 1] * 256 + data[x * 3 + 2]);
            int neighbor_index_1 = (data[(x + 1) * 3 + 0] * 256 * 256 + data[(x + 1) * 3 + 1] * 256 + data[(x + 1) * 3 + 2]);
            int neighbor_index_2 = (neighbor_data[x * 3 + 0] * 256 * 256 + neighbor_data[x * 3 + 1] * 256 + neighbor_data[x * 3 + 2]);
            if (index != neighbor_index_1 || index != neighbor_index_2)
                for (int delta_x = -BORDER_SIZE; delta_x <= BORDER_SIZE; delta_x++)
                    for (int delta_y = -BORDER_SIZE; delta_y <= BORDER_SIZE; delta_y++)
                        border_mask[(y + delta_y) * width + (x + delta_x)] = true;
        }
    }
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < BORDER_SIZE; y++)
            border_mask[y * width + x] = true;
        for (int y = height - BORDER_SIZE; y < height; y++)
            border_mask[y * width + x] = true;
    }
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < BORDER_SIZE; x++)
            border_mask[y * width + x] = true;
        for (int x = width - BORDER_SIZE; x < width; x++)
            border_mask[y * width + x] = true;
    }

    map<int, vector<CvPoint> > segment_points;
    int segment_index = 0;
      ...
      uchar *data = surface_id_image.ptr<uchar>(y);
        for (int x = 0; x < width; x++) {
            ...
            if (index_map.count(index) == 0)
                index_map[index] = segment_index++;
            segment_points[index_map[index]].push_back(CvPoint(x, y));
        }
    }

    
    map<int, vector<int> > invalid_points;
    map<int, vector<double> > valid_planes;
    for (map<int, vector<CvPoint> >::const_iterator segment_it = segment_points.begin(); segment_it != segment_points.end(); segment_it++) {
        
        const int num_points = segment_it->second.size();
        if (num_points < 3) {
            for (int i = 0; i < num_points; i++)
                invalid_points[segment_it->first].push_back(segment_it->second[i].y * width + segment_it->second[i].x);
            continue;
        }
        
        if (use_3D_positions == true) {
            vector<double> points_3D;
            for (int i = 0; i < num_points; i++) {
                int x = segment_it->second[i].x;
                int y = segment_it->second[i].y;
                if (x == 201 && y == 344)
                    bool wait = true;
                int index = y * width + x;
                if (border_mask[y * width + x] == true)
                    continue;
                points_3D.push_back(point_cloud[index * 3 + 0]);
                points_3D.push_back(point_cloud[index * 3 + 1]);
                points_3D.push_back(point_cloud[index * 3 + 2]);
            }
            
            if (segment_it->first == 12)
                bool wait = true;

//            vector<double> extreme_values = calcExtremeValues(points_3D);
//            for (int i = 0; i < extreme_values.size(); i++)
//                cout << extreme_values[i] << '\t';
//            cout << endl;


//            if (points_3D.size() / 3 < width * height * 0.01) {
//                double dummy;
//                plane = fitPlane(points_3D, dummy);
//            } else {
//                double plane_error_threshold = 0.01;
//                plane = fitPlaneRobust(points_3D, plane_error_threshold);
//            }
            double plane_error_threshold = 0.01 * points_3D.size() / (width * height);
            vector<double> plane = fitPlaneRobust(points_3D, plane_error_threshold);
            
            if (plane.size() > 0) {
	      vector<double> plane_model = plane;
                
	      valid_planes[segment_it->first] = plane_model;
	      for (int i = 0; i < num_points; i++) {
		int x = segment_it->second[i].x;
		int y = segment_it->second[i].y;
		surfacemodels[y * width + x] = plane_model;
	      }
                
	      double error = 0;
	      for (int i = 0; i < points_3D.size() / 3; i++) {
		error += abs(points_3D[i * 3 + 0] * plane[0] + points_3D[i * 3 + 1] * plane[1] + points_3D[i * 3 + 2] * plane[2] - plane[3]);
		if (abs(points_3D[i * 3 + 0] * plane[0] + points_3D[i * 3 + 1] * plane[1] + points_3D[i * 3 + 2] * plane[2] - plane[3]) > 10) {
		  cout << segment_it->second[i].x << '\t' << segment_it->second[i].y << '\t' << points_3D[i * 3 + 0] << '\t' << points_3D[i * 3 + 1] << '\t' << points_3D[i * 3 + 2] << endl;
		  bool wait = true;
		}
	      }
	      error /= num_points;
	      cout << "fit error for plane " << segment_it->first << ": " << error << endl;
            }
            else {
	      for (int i = 0; i < num_points; i++)
		invalid_points[segment_it->first].push_back(segment_it->second[i].y * width + segment_it->second[i].x);
            ...
            vector<double> points_3D(num_points * 3);
            for (int i = 0; i < num_points; i++) {
                int x = segment_it->second[i].x;
                int y = segment_it->second[i].y;
                ...
                points_3D[i * 3 + 0] = x;
                points_3D[i * 3 + 1] = y;
                double disp = disp_image.at<uchar>(y, x);
                points_3D[i * 3 + 2] = disp;
            }
            double plane_error_threshold = 1;
            
            if (segment_it->first == 3)
                bool wait = true;
            
            vector<double> plane = fitPlaneRobust(points_3D, plane_error_threshold);
            if (plane.size() > 0 && plane[2] != 0) {
	      vector<double> plane_model(4);
	      plane_model[0] = -plane[0] / plane[2];
	      plane_model[1] = -plane[1] / plane[2];
	      plane_model[2] = plane[3] / plane[2];
                valid_planes[segment_it->first] = plane_model;
                for (int i = 0; i < num_points; i++) {
                    int x = segment_it->second[i].x;
                    int y = segment_it->second[i].y;
...
//        CvPoint first_point = segment_it->second[0];
//        uchar *data = (uchar *)(surface_id_image->imageData + first_point.y * surface_id_image->widthStep);
//        if ((data[3 * first_point.x + 0] == data[3 * first_point.x + 1] && data[3 * first_point.x + 0] == data[3 * first_point.x + 2]) || data[3 * first_point.x + 2] == 255) {
//            
//            if (coordinates.size() > 0)
//                FitPlaneToSegment_ROBUST (segments.segments[i], 1, coordinates, width, 1);
//            else
//                FitDispPlaneToSegment_ROBUST(segments.segments[i], 1, disp_image, scale);
//
//            Plane *plane = new Plane;
//			*plane = *(Plane*) (segments.segments[i].surfacemodel);
//            
//            int surface_id = data[3 * first_point.x + 0] + data[3 * first_point.x + 1] + data[3 * first_point.x + 2];
//            if (plane->A == -1)
//            {
//                for (int j = 0; j < segments.segments[i].segpoints.size(); j++)
//                {
//                    CvPoint point = segments.segments[i].segpoints[j];
//                    invalid_points[surface_id].push_back(point.y * width + point.x);
//                }
//            }
//            else
//            {
//                valid_planes[surface_id] = plane;
//                for (int j = 0; j < segments.segments[i].segpoints.size(); j++)
//                {
//                    CvPoint point = segments.segments[i].segpoints[j];
//                    surfacemodels [point.y * width + point.x] = (SurfaceModel*)plane;
//                }
//            }
//		}
//		else
//		{
//            int spline_order = 1;
//			FitSplineToSegment (segments.segments[i], disp_image, coordinates, scale, spline_order);
//            
...
//			{
//				CvPoint point = segments.segments[i].segpoints[j];
//				surfacemodels [point.y * width + point.x] = (SurfaceModel*) spline;
//			}
//		}
	}
    set<int> unknown_points;
    for (map<int, vector<int> >::const_iterator id_points_it = invalid_points.begin(); id_points_it != invalid_points.end(); id_points_it++)
    {
        if (valid_planes.count(id_points_it->first) == 0)
            unknown_points.insert(id_points_it->second.begin(), id_points_it->second.end());
        else
            for (vector<int>::const_iterator point_it = id_points_it->second.begin(); point_it != id_points_it->second.end(); point_it++)
                surfacemodels[*point_it] = valid_planes[id_points_it->first];
    }
    
    while (unknown_points.size() > 0) {
        set<int> removed_points;
        for (set<int>::const_iterator point_it = unknown_points.begin(); point_it != unknown_points.end(); point_it++) {
            vector<int> neighbor_points;
            neighbor_points.push_back(*point_it - 1);
            neighbor_points.push_back(*point_it + 1);
            neighbor_points.push_back(*point_it - width);
            neighbor_points.push_back(*point_it + width);
            vector<int> available_neighbor_points;
            for (vector<int>::const_iterator neighbor_point_it = neighbor_points.begin(); neighbor_point_it != neighbor_points.end(); neighbor_point_it++) {
                if (*neighbor_point_it < 0 || *neighbor_point_it >= width * height)
                    continue;
                if (unknown_points.count(*neighbor_point_it) == 0)
                    available_neighbor_points.push_back(*neighbor_point_it);
            }
            if (available_neighbor_points.size() > 0) {
                surfacemodels[*point_it] = surfacemodels[available_neighbor_points[rand() % available_neighbor_points.size()]];
                removed_points.insert(*point_it);
            }
        }
        for (set<int>::const_iterator point_it = removed_points.begin(); point_it != removed_points.end(); point_it++)
            unknown_points.erase(*point_it);
    }
    
//	Mat &disp = cvCreateImage(cvGetSize(disp_image), IPL_DEPTH_8U, 1);
//	cvZero(disp);
//    
//	for (int y = 0; y < disp->height; y++)
//		for (int x = 0; x < disp->width; x++)
//		{
//			float d = surfacemodels[y * width + x]->PointDisp(cvPoint (x, y), scale);
//            
//			int d_int = (int) (min (d * scale, 255.f) + 0.5f);
//            
//			disp->imageData [y * disp->widthStep + x] = (uchar) d_int;
//		}
//    cvSaveImage("test/disp.bmp", disp);
  ...
  Mat disp(height, width, CV_8UC1);
    for (int y = 0; y < height; y++) {
      uchar *data = disp.ptr<uchar>(y);
        for (int x = 0; x < width; x++) {
	  vector<double> surface_model = surface_models[y * width + x];
            if (use_panoramic_image == true) {
	      //                int index = y * width + x;
	      //                double point_x = coordinates[index * 3 + 0];
	      //                double point_y = coordinates[index * 3 + 1];
	      //                double point_z = coordinates[index * 3 + 2];
	      double R = width / 6.28;
	      double angle_1 = (width / 2 - x) / R;
	      double angle_2 = (height / 2 - y) / R;
	      double point_x = cos(angle_2) * cos(angle_1);
	      double point_y = cos(angle_2) * sin(angle_1);
	      double point_z = sin(angle_2);
                    
	      double distance_1 = point_x * surface_model[0] + point_y * surface_model[1] + point_z * surface_model[2] - surface_model[3];
	      double distance_2 = -surface_model[3];
	      double ratio = distance_2 / (distance_2 - distance_1);
	      point_x *= ratio;
	      point_y *= ratio;
	      point_z *= ratio;
	      double depth = sqrt(pow(point_x, 2) + pow(point_y, 2));
	      //                double depth = sqrt(pow(plane->x, 2) + pow(plane->y, 2));
	      data[x] = min(static_cast<int>(DISP_DEPTH_C / depth + 0.5), 255);
            }
            else if (use_3D_positions == true) {
                vector<double> point_3D = calc3DPointOnPlane(x, y, surface_model, camera_parameters);
                data[x] = max(min(static_cast<int>(100 / point_3D[2]), 255), 0);
            }
        }
    }
    return disp;
...
vector<int> deleteSmallSegments(const vector<int> &segmentation, const int width, const int small_segment_threshold)
{
  const int height = segmentation.size() / width;
    set<int> small_segment_points;
    vector<bool> visited_points(segmentation.size(), false);

    for (int i = 0; i < segmentation.size(); i++) {
      if (visited_points[i] == true)
            continue;
        int segment_index = segmentation[i];
        vector<int> border_points;
        border_points.push_back(i);
        vector<int> region_points;
        while (border_points.size() > 0) {
            vector<int> new_border_points;
            for (int j = 0; j < border_points.size(); j++) {
                int point = border_points[j];
                if (visited_points[point] == true || segmentation[point] != segment_index)
                    continue;
        ...
        if (region_points.size() < small_segment_threshold)
            small_segment_points.insert(region_points.begin(), region_points.end());
    }

    vector<int> new_segmentation = segmentation;
    while (small_segment_points.size() > 0) {
        set<int> delete_indices;
        for (set<int>::const_iterator point_it = small_segment_points.begin(); point_it != small_segment_points.end(); point_it++) {
            int point = *point_it;
            vector<int> neighbor_points;
            int x = point % width;
            int y = point / width;
            ...
            if (y < height - 1)
                neighbor_points.push_back(point + width);
            int neighbor_point = neighbor_points[rand() % neighbor_points.size()];
            if (small_segment_points.count(neighbor_point) == 0) {
                new_segmentation[point] = segmentation[neighbor_point];
                delete_indices.insert(point);
            }
        }
        small_segment_points.erase(delete_indices.begin(), delete_indices.end());
    }
...
Mat drawSegmentationImage(const vector<int> &segmentation, const int width)
{
  ...
  map<int, int> color_table;
  for (int i = 0; i < segmentation.size(); i++) {
    int index = segmentation[i];
    if (color_table.count(index) == 0) {
      int new_color = rand() % 256;
      while (true) {
	bool exists = false;
	for (map<int, int>::const_iterator color_it = color_table.begin(); color_it != color_table.end(); color_it++) {
	  if (color_it->second == new_color) {
	    exists = true;
	    break;
	  }
	}
	...
	new_color = rand() % 256;
      ...
      color_table[index] = new_color;
    }
    int color = color_table[index];
    int x = i % width;
    int y = i / width;
    //segmentation_image.at<uchar>(y, x) = color;
    segmentation_image.at<Vec3b>(y, x) = Vec3b(static_cast<int>(pow(index + 3, 5)) % 256, static_cast<int>(pow(index + 5, 5)) % 256, static_cast<int>(pow(index + 7, 5)) % 256);
  }
...
Mat drawSegmentationImage(const vector<int> &segmentation, const int width, const Mat &image, const char type)
{
  const int height = segmentation.size() / width;
    ...
    map<int, int> color_table;
    for (int y = 0; y < height; y++) {
      const uchar *ori_image_data = image.ptr<uchar>(y);
      uchar *segmentation_image_data = segmentation_image.ptr<uchar>(y);
      for (int x = 0; x < width; x++) {
	int label = segmentation[y * width + x];
	if (label == -1) {
	  for (int c = 0; c < 3; c++)
	    segmentation_image_data[x * 3 + c] = 255;
	  continue;
	}
	...
	if (color_table.count(label) > 0)
	  ...
	  if (type == 'C') {
	    ...
	    int color_index = (color / (256 * 256) + color % (256 * 256) / 256 + color % 256) / 3;
	    while (used_color_indices.count(color_index) > 0) {
	      color = rand() % (256 * 256 * 256);
	      color_index = (color / (256 * 256) + color % (256 * 256) / 256 + color % 256) / 3;
	    }
	    used_color_indices.insert(color_index);
	  }
	  else {
	    color = rand() % 256 * (256 * 256 + 256 + 1);
	    int color_index = (color / (256 * 256) + color % (256 * 256) / 256 + color % 256) / 3;
	    while (used_color_indices.count(color_index) > 0) {
	      color = rand() % 256 * (256 * 256 + 256 + 1);
	      color_index = (color / (256 * 256) + color % (256 * 256) / 256 + color % 256) / 3;
	...
	if (ori_image_data == NULL) {
	  segmentation_image_data[x * 3 + 0] = color / (256 * 256);
	  segmentation_image_data[x * 3 + 1] = color % (256 * 256) / 256;
	  segmentation_image_data[x * 3 + 2] = color % 256;
	  continue;
	}

	segmentation_image_data[x * 3 + 0] = min((ori_image_data[x * 3 + 0] + color / (256 * 256)) / 2, 255);
	segmentation_image_data[x * 3 + 1] = min((ori_image_data[x * 3 + 1] + color % (256 * 256) / 256) / 2, 255);
	segmentation_image_data[x * 3 + 2] = min((ori_image_data[x * 3 + 2] + color % 256) / 2, 255);
...
{
    ofstream out_str(filename);
    out_str << segmentation.size() << endl;
    for (int i = 0; i < segmentation.size(); i++)
        out_str << segmentation[i] << endl;
    out_str.close();
}
...
vector<int> loadSegmentation(const char *filename)
{
    ...
    int num_pixels;
    in_str >> num_pixels;
    vector<int> segmentation(num_pixels);
    for (int i = 0; i < num_pixels; i++)
        in_str >> segmentation[i];
    in_str.close();
    return segmentation;
}
...
Mat drawDispImage(const vector<double> &point_cloud, const int width, const int height)
{
  ...
  Mat disp_uchar(height, width, CV_8UC1);
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      double depth = point_cloud[(y * width + x) * 3 + 2];
      if (depth < 0)
	continue;
      //cout << depth << '\t';
      disp_real.at<double>(y, x) = 1.0 / depth;
      disp_uchar.at<uchar>(y, x) = min(100.0 / depth, 255.0);
      //cout << depth << endl;
    }
  }
  return disp_uchar;
...
{
  map<int, map<int, int> > relations;
  for (map<int, vector<double> >::const_iterator point_cloud_it = point_clouds.begin(); point_cloud_it != point_clouds.end(); point_cloud_it++)
    for (map<int, vector<double> >::const_iterator surface_model_it = surface_models.begin(); surface_model_it != surface_models.end(); surface_model_it++)
      if (point_cloud_it->first == surface_model_it->first)
	relations[point_cloud_it->first][surface_model_it->first] = -1;
      else
	relations[point_cloud_it->first][surface_model_it->first] = findPlaneRelation(point_cloud_it->second, surface_model_it->second, opposite_distance_threshold, num_outliers_threshold_ratio);
}
...
void zoomScene(Mat &image, vector<double> &point_cloud, const double scale_x, const double scale_y)
{
  ...
  Mat cropped_image;
  resize(image, cropped_image, Size(new_width, new_height), 0, 0, INTER_AREA);
  vector<double> new_point_cloud(new_width * new_height * 3);
...
{
  vector<double> new_point_cloud = point_cloud;
  for (int pixel = 0; pixel < segmentation.size(); pixel++) {
    int segment_id = segmentation[pixel];
    ...
    depth_values.push_back(point_cloud[pixel * 3 + 2]);
    for (vector<int>::const_iterator neighbor_pixel_it = neighbor_pixels.begin(); neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
      if (segmentation[*neighbor_pixel_it] != segment_id)
	continue;
      double neighbor_depth = point_cloud[*neighbor_pixel_it * 3 + 2];
      depth_values.push_back(neighbor_depth);
    }
    sort(depth_values.begin(), depth_values.end());
    if (depth_values.size() % 2 == 0)
      new_point_cloud[pixel * 3 + 2] = (depth_values[depth_values.size() / 2 - 1] + depth_values[depth_values.size() / 2]) / 2;
    else
      new_point_cloud[pixel * 3 + 2] = depth_values[depth_values.size() / 2];
  }
  return new_point_cloud;
}
