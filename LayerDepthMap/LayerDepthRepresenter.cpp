...
LayerDepthRepresenter::LayerDepthRepresenter(
    ...
    const RepresenterPenalties &penalties, const DataStatistics &statistics,
    const int scene_index, const Mat &ori_image,
    const vector<double> &ori_point_cloud, const bool first_time,
    const int num_layers, const PipelineParams &pipeline_params)
    : image_(image), point_cloud_(point_cloud), IMAGE_WIDTH_(image.cols),
      IMAGE_HEIGHT_(image.rows), NUM_PIXELS_(IMAGE_WIDTH_ * IMAGE_HEIGHT_),
      PENALTIES_(penalties), STATISTICS_(statistics), SCENE_INDEX_(scene_index),
      ori_image_(ori_image), ori_point_cloud_(ori_point_cloud),
      FIRST_TIME_(first_time), num_layers_(num_layers),
  ...
  if (true) {
    ...
    estimateCameraParameters(point_cloud_, IMAGE_WIDTH_, IMAGE_HEIGHT_,
                             camera_parameters_);
    cout << "camera parameters: " << camera_parameters_[0] << '\t'
         << camera_parameters_[1] << '\t' << camera_parameters_[2] << endl;
    // exit(1);
...
void LayerDepthRepresenter::optimizeLayerRepresentation() {
  // num_layers_ = min(estimateNumLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_,
  // initial_segmentation_, surface_depths_), 3);
  ...
  // normal_image_filename << "Cache/scene_" << SCENE_INDEX_ <<
  // "/normal_image.bmp";
  ...
  // curvature_image_filename << "Cache/scene_" << SCENE_INDEX_ <<
  // "/curvature_image.bmp";
  ...
  vector<shared_ptr<ParallelFusion::ProposalGenerator<LayerLabelSpace>>>
      generators((size_t)option.num_threads);
  vector<shared_ptr<ParallelFusion::FusionSolver<LayerLabelSpace>>> solvers(
      (size_t)option.num_threads);
  vector<LayerLabelSpace> initials((size_t)option.num_threads);
  vector<ParallelFusion::ThreadOption> thread_options(
      (size_t)option.num_threads);

  for (auto i = 0; i < option.num_threads; ++i) {
    generators[i] =
        shared_ptr<ParallelFusion::ProposalGenerator<LayerLabelSpace>>(
            new ProposalDesigner(image_, point_cloud_, normals_,
                                 camera_parameters_, num_layers_, PENALTIES_,
                                 STATISTICS_, SCENE_INDEX_));
    solvers[i] = shared_ptr<ParallelFusion::FusionSolver<LayerLabelSpace>>(
        new TRWSFusion(image_, point_cloud_, normals_, PENALTIES_,
                       STATISTICS_));
    initials[i].setSingleLabels(vector<int>(NUM_PIXELS_, 0));
    ...
    thread_options[i].solution_exchange_interval =
        PIPELINE_PARAMS_.solution_exchange_interval;
  ...
  ParallelFusion::ParallelFusionPipeline<LayerLabelSpace>
  ...
  for (auto thread_id = 0; thread_id < option.num_threads; ++thread_id) {
    vector<Solution> solutions =
        dynamic_pointer_cast<ProposalDesigner>(generators[thread_id])
            ->getSolutions();

    ofstream out_str("Test/solution_images/binary_fusion/solutions_" +
                     to_string(thread_id));
    ...
    for (vector<Solution>::iterator solution_it = solutions.begin();
      ...
      imwrite("Test/solution_images/binary_fusion/solution_image_" +
                  to_string(solution_it->time) + "_" +
                  to_string(solution_it->thread_id) + ".png",
  ...
  // iteration_info_filename << "Cache/scene_" << SCENE_INDEX_ <<
  // "/iteration_info.txt";
  ...
  //     // double lower_bound;
  //     // double time;
  //     iteration_info_in_str >> iteration >> proposal_type >> statistics[0] >>
  ...
  //     cout << iteration << '\t' << proposal_type << '\t' << statistics[0] <<
  //     '\t' << statistics[1] << '\t' << statistics[2] << endl;
  ...
  // for (map<int, vector<double> >::const_iterator iteration_it =
  // iteration_statistics_map.begin(); iteration_it !=
  // iteration_statistics_map.end(); iteration_it++) {
  ...
  // unique_ptr<ProposalDesigner> proposal_designer(new ProposalDesigner(image_,
  // point_cloud_, normals_, camera_parameters_, num_layers_, PENALTIES_,
  // STATISTICS_, SCENE_INDEX_));

  // //unique_ptr<BinaryProposalDesigner> proposal_designer(new
  // BinaryProposalDesigner(image_, point_cloud_, normals_, camera_parameters_,
  // num_layers_, PENALTIES_, STATISTICS_, SCENE_INDEX_));
  // unique_ptr<TRWSFusion> TRW_solver(new TRWSFusion(image_, point_cloud_,
  // normals_, PENALTIES_, STATISTICS_));
  ...
  //   for (int iteration = 0; iteration <= previous_energy_iteration;
  //   iteration++) {
  //     vector<int> solution_labels;
  //     int solution_num_surfaces;
  //     map<int, Segment> solution_segments;
  //     //readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_,
  //     PENALTIES_, STATISTICS_, num_layers_, solution_labels,
  //     solution_num_surfaces, solution_segments, SCENE_INDEX_, iteration) ==
  //     true;
  //     //writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_,
  //     camera_parameters_, num_layers_, solution_labels,
  //     solution_num_surfaces, solution_segments, SCENE_INDEX_, iteration,
  //     ori_image_, ori_point_cloud_);

  //     //       readLayers(solution_labels, solution_num_surfaces,
  //     solution_segments, iteration);
  //     // stringstream disp_image_filename;
  //     // disp_image_filename << "Test/disp_image_" << iteration << ".bmp";
  //     // writeDispImageFromSegments(solution_labels, solution_num_surfaces,
  //     solution_segments, num_layers_, IMAGE_WIDTH_, IMAGE_HEIGHT_,
  //     disp_image_filename.str());
  //     //writeLayers(solution_labels, solution_num_surfaces,
  //     solution_segments, iteration);
  //     //exit(1);
  //   }
  //   //exit(1);
  ...
  //   bool read_success = readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_,
  //   camera_parameters_, PENALTIES_, STATISTICS_, num_layers_,
  //   previous_solution_labels, previous_solution_num_surfaces,
  //   previous_solution_segments, SCENE_INDEX_, previous_energy_iteration) ==
  //   true;
  //   //bool read_success = readLayers(previous_solution_labels,
  //   previous_solution_num_surfaces, previous_solution_segments,
  //   previous_energy_iteration) == true;

  //   assert(read_success);
  //   stringstream disp_image_filename;
  //   disp_image_filename << "Test/previous_solution_disp_image.bmp";
  //   //writeDispImageFromSegments(previous_solution_labels,
  //   previous_solution_num_surfaces, previous_solution_segments, num_layers_,
  //   IMAGE_WIDTH_, IMAGE_HEIGHT_, disp_image_filename.str());
  //   // writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_,
  //   camera_parameters_, num_layers_, previous_solution_labels,
  //   previous_solution_num_surfaces, previous_solution_segments, SCENE_INDEX_,
  //   10000, ori_image_);
  //   // exit(1);
  //   proposal_designer->setCurrentSolution(previous_solution_labels,
  //   previous_solution_num_surfaces, previous_solution_segments);
  // }

  // int best_solution_iteration = previous_energy_iteration;
  // for (int iteration = iteration_start_index; iteration < NUM_ITERATIONS +
  // num_single_surface_expansion_proposals; iteration++) {
  ...
  //   //   int solution_num_surfaces;
  //   //   map<int, Segment> solution_segments;
  //   //   if (readLayers(solution_labels, solution_num_surfaces,
  //   solution_segments, iteration) == true) {
  //   //   proposal_designer->setCurrentSolution(solution_labels,
  //   solution_num_surfaces, solution_segments);
  //   //   continue;
  //   //   }
  //   // }
  ...
  //   //proposal_designer.setCurrentSolution(previous_solution,
  //   previous_solution_num_surfaces, previous_solution_surface_depths);
  //   string proposal_type;
  //   if (proposal_designer->getProposal(iteration, proposal_labels,
  //   proposal_num_surfaces, proposal_segments, proposal_type) == false)
  //     break;

  //   if (proposal_type == "single_surface_expansion_proposal")
  //     num_single_surface_expansion_proposals++;

  //   vector<int> previous_solution_indices =
  //   proposal_designer->getCurrentSolutionIndices();
  //   vector<int> current_solution_labels = TRW_solver->fuse(proposal_labels,
  //   proposal_num_surfaces, num_layers_, proposal_segments,
  //   previous_solution_indices);
  ...
  //   statistics.push_back(static_cast<double>(clock() - begin_time) /
  //   CLOCKS_PER_SEC);
  //   iteration_statistics_map[iteration] = statistics;
  //   iteration_proposal_type_map[iteration] = proposal_type;

  //   writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_,
  //   camera_parameters_, num_layers_, current_solution_labels,
  //   proposal_num_surfaces, proposal_segments, SCENE_INDEX_, iteration,
  //   ori_image_, ori_point_cloud_);
  ...
  //   //   //exit(1);
  //   // }
  //   proposal_designer->setCurrentSolution(current_solution_labels,
  //   current_solution_num_surfaces, current_solution_segments);

  //   ofstream iteration_info_out_str(iteration_info_filename.str());
  //   for (map<int, vector<double> >::const_iterator iteration_it =
  //   iteration_statistics_map.begin(); iteration_it !=
  //   iteration_statistics_map.end(); iteration_it++) {
  //     iteration_info_out_str << iteration_it->first << '\t' <<
  //     iteration_proposal_type_map[iteration_it->first] << '\t' <<
  //     iteration_it->second[0] << '\t' << iteration_it->second[1] << '\t' <<
  //     iteration_it->second[2] << endl;
  //   }
  ...
  // //     vector<int> proposal_distance_to_boundaries;
  // //     string proposal_type;
  // //     if (proposal_designer->getLastProposal(proposal_labels,
  // proposal_num_surfaces, proposal_segments, proposal_type) == true) {
  // //   vector<int> previous_solution_indices =
  // proposal_designer->getCurrentSolutionIndices();
  // //   vector<int> current_solution_labels =
  // TRW_solver->fuse(proposal_labels, proposal_num_surfaces, num_layers_,
  // proposal_segments, previous_solution_indices);
  // //   double current_solution_energy = TRW_solver->getEnergy();
  ...
  // //   writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_,
  // camera_parameters_, num_layers_, current_solution_labels,
  // proposal_num_surfaces, proposal_segments, SCENE_INDEX_, NUM_ITERATIONS);
  // //   writeDispImageFromSegments(current_solution_labels,
  // proposal_num_surfaces, proposal_segments, num_layers_, IMAGE_WIDTH_,
  // IMAGE_HEIGHT_, "Test/final_disp_image.bmp");
  // //       if (current_solution_energy >= previous_energy && previous_energy
  // >= 0) {
  // //     cout << "energy increases" << endl;
  // //     num_layers_--;
  // //   } else
  ...
  // generateLayerImageHTML(SCENE_INDEX_, iteration_statistics_map,
  // iteration_proposal_type_map);
  ...
  // bool read_success = readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_,
  // camera_parameters_, PENALTIES_, STATISTICS_, num_layers_, solution_labels,
  // solution_num_surfaces, solution_segments, SCENE_INDEX_,
  ...
  int solution_num_surfaces = solution_label_space.getNumSegments();
  map<int, Segment> solution_segments = solution_label_space.getSegments();

  writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_,
              camera_parameters_, num_layers_, solution_labels,
              solution_num_surfaces, solution_segments, SCENE_INDEX_, 10000,
              ori_image_, ori_point_cloud_);
  // exit(1);
  ...
  if (false) {
    ...
    if (readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_, PENALTIES_,
                   STATISTICS_, num_layers_, refined_solution_labels,
                   refined_solution_num_surfaces, refined_solution_segments,
                   SCENE_INDEX_, 10000) == false ||
        true) {
      // refineSolution(solution_labels, solution_num_surfaces,
      // solution_segments, refined_solution_labels);
  ...
  // writeDispImageFromSegments(solution_labels, solution_num_surfaces,
  // solution_segments, num_layers_, IMAGE_WIDTH_, IMAGE_HEIGHT_,
  // "Test/final_disp_image.bmp");
  // writeLayers(solution_labels, solution_num_surfaces, solution_segments,
  // 100);
...
void LayerDepthRepresenter::writeSegmentationImage(
    const vector<int> &segmentation, const string filename) {
  ...
  for (int i = 0; i < NUM_PIXELS_; i++) {
    ...
    segmentation_image.at<Vec3b>(y, x) =
        Vec3b(surface_color % 256, surface_color % 256, surface_color % 256);

    segment_center_x[surface_id] += x;
    segment_center_y[surface_id] += y;
    segment_pixel_counter[surface_id]++;
  }
  for (map<int, int>::const_iterator segment_it = segment_pixel_counter.begin();
       segment_it != segment_pixel_counter.end(); segment_it++) {
    Point origin(segment_center_x[segment_it->first] / segment_it->second,
                 segment_center_y[segment_it->first] / segment_it->second);
    char *text = new char[10];
    sprintf(text, "%d", segment_it->first);
    putText(segmentation_image, text, origin, FONT_HERSHEY_PLAIN, 0.6,
            Scalar(0, 0, 255, 1));
  }
...
void LayerDepthRepresenter::writeDispImage(const vector<int> &segmentation,
                                           const string filename) {
  Mat disp_image = Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_, CV_8UC1);
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    int x = pixel % IMAGE_WIDTH_;
    int y = pixel / IMAGE_WIDTH_;
    // if (surface_depths_[1][pixel] > surface_depths_[5][pixel] &&
    // surface_depths_[5][pixel] > 0)
    //      test_image.at<uchar>(y, x) = 255;

    // for (int surface_id = 0; surface_id < num_surfaces_; surface_id++) {
    //      if (surface_id == segmentation[pixel]) {
    double depth = segmentation[pixel] < num_surfaces_
                       ? surface_depths_[segmentation[pixel]][pixel]
                       : -1;
    if (depth > 0)
      disp_image.at<uchar>(y, x) =
          min(max(disp_image_numerator_ / depth, 0.0), 255.0);
    else
...
void writeLayers(const Mat &image, const int image_width,
                 const int image_height, const vector<double> &point_cloud,
                 const vector<double> &camera_parameters, const int num_layers,
                 const vector<int> &solution, const int solution_num_surfaces,
                 const map<int, Segment> &solution_segments,
                 const int scene_index, const int result_index,
                 const Mat &ori_image, const vector<double> &ori_point_cloud) {
  ...
  for (int layer_index = 0; layer_index < num_layers; layer_index++)
    for (map<int, int>::const_iterator counter_it =
             layer_surface_counter[layer_index].begin();
         counter_it != layer_surface_counter[layer_index].end(); counter_it++)
      layer_surface_centers[layer_index][counter_it->first] =
          (layer_surface_y_sum[layer_index][counter_it->first] /
           counter_it->second) *
              image_width +
          (layer_surface_x_sum[layer_index][counter_it->first] /
           counter_it->second);

  // map<int, int> surface_colors = calcSurfaceColors(image_,
  ...
  if (true) {
    ...
    for (int layer_index = 0; layer_index < num_layers; layer_index++) {
      ...
      vector<int> surface_ids = layer_surface_ids[layer_index];
      for (int ori_pixel = 0; ori_pixel < ori_image.cols * ori_image.rows;
           ori_pixel++) {
        int ori_x = ori_pixel % ori_image.cols;
        int ori_y = ori_pixel / ori_image.cols;
        int x = min(
            static_cast<int>(round(1.0 * ori_x / ori_image.cols * image_width)),
            image_width - 1);
        int y = min(static_cast<int>(
                        round(1.0 * ori_y / ori_image.rows * image_height)),
                    image_height - 1);
        int pixel = y * image_width + x;
        int surface_id = surface_ids[pixel];
        if (surface_id != solution_num_surfaces) {
          if (visible_mask[ori_pixel] == true) {
            layer_image.at<Vec3b>(ori_y, ori_x) =
                ori_image.at<Vec3b>(ori_y, ori_x);
      ...
      // Mat line_image = Mat::zeros(image_height * LINE_UPSAMPLING_RATIO,
      // image_width * LINE_UPSAMPLING_RATIO, CV_8UC3);
      ...
      for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
        ...
        if (color_table.count(surface_id) == 0)
          color_table[surface_id] =
              Vec3b(rand() % 256, rand() % 256, rand() % 256);

        int ori_x =
            min(static_cast<int>(round(1.0 * x / image_width * ori_image.cols)),
                ori_image.cols - 1);
        int ori_y = min(
            static_cast<int>(round(1.0 * y / image_height * ori_image.rows)),
            ori_image.rows - 1);
        line_mask[ori_y * ori_image.cols + ori_x] = surface_id;

        // line_mask[min(static_cast<int>(round(1.0 * (*pixel_it / (image_width
        // * LINE_UPSAMPLING_RATIO)) / LINE_UPSAMPLING_RATIO)), image_height -
        // 1) * image_width + min(static_cast<int>(round(1.0 * (*pixel_it %
        // (image_width * LINE_UPSAMPLING_RATIO)) / LINE_UPSAMPLING_RATIO)),
        // image_width - 1)] = true;
        // vector<int> upsampling_pixels;
        // for (int delta_x = - (LINE_WINDOW_SIZE - 1) / 2; delta_x <=
        // (LINE_WINDOW_SIZE - 1) / 2; delta_x++)
        //   for (int delta_y = - (LINE_WINDOW_SIZE - 1) / 2; delta_y <=
        //   (LINE_WINDOW_SIZE - 1) / 2; delta_y++)
        //     if (upsampling_x + delta_x >= 0 && upsampling_x + delta_x <
        //     image_width * LINE_UPSAMPLING_RATIO && upsampling_y + delta_y >=
        //     0 && upsampling_y + delta_y < image_height *
        //     LINE_UPSAMPLING_RATIO)
        //       upsampling_pixels.push_back((upsampling_y + delta_y) *
        //       image_width * LINE_UPSAMPLING_RATIO + upsampling_x + delta_x);
        // for (vector<int>::const_iterator pixel_it =
        // upsampling_pixels.begin(); pixel_it != upsampling_pixels.end();
        // pixel_it++) {
        //   line_image.at<Vec3b>(*pixel_it / (image_width *
        //   LINE_UPSAMPLING_RATIO), *pixel_it % (image_width *
        //   LINE_UPSAMPLING_RATIO)) = color_table[surface_id];
        //   line_mask[min(static_cast<int>(round(1.0 * (*pixel_it /
        //   (image_width * LINE_UPSAMPLING_RATIO)) / LINE_UPSAMPLING_RATIO)),
        //   image_height - 1) * image_width + min(static_cast<int>(round(1.0 *
        //   (*pixel_it % (image_width * LINE_UPSAMPLING_RATIO)) /
        //   LINE_UPSAMPLING_RATIO)), image_width - 1)] = true;
        // }
        // if (on_boundary == true) {
        //   if (color_table.count(surface_id) == 0)
        //     color_table[surface_id] = Vec3b(rand() % 256, rand() % 256,
        //     rand() % 256);
        //   layer_image.at<Vec3b>(y, x) = layer_image.at<Vec3b>(y, x) * 0.3 +
        //   color_table[surface_id] * 0.7;
        // }
      }
      ...
      for (int ori_pixel = 0; ori_pixel < ori_image.cols * ori_image.rows;
        ...
        if (surface_id != solution_num_surfaces) {
          ...
          Vec3b blended_color;
          for (int c = 0; c < 3; c++)
            blended_color[c] = min(image_color[c] * BLENDING_ALPHA +
                                       layer_color[c] * (1 - BLENDING_ALPHA),
                                   255.0);
      ...
      layer_mask_image_filename << "Results/scene_" << scene_index << "/"
                                << "layer_mask_image" << scene_index << "_"
                                << layer_index << ".bmp";
      ...
      // for (map<int, int>::const_iterator surface_it =
      // surface_centers.begin(); surface_it != surface_centers.end();
      // surface_it++) {
      //   char *text = new char[10];
      //   sprintf(text, "%d", surface_it->first);
      //  if (solution_segments.at(surface_it->first).getType() == 0)
      //    putText(layer_image, text, Point(surface_it->second %
      // image_width, surface_it->second / image_width), FONT_HERSHEY_PLAIN,
      // 0.6, color_table[surface_it->first]);
      //   else
      //    putText(layer_image, text, Point(surface_it->second %
      // image_width, surface_it->second / image_width), FONT_HERSHEY_PLAIN &
      // FONT_ITALIC, 0.6, color_table[surface_it->first]);
      // }
    ...
    for (int layer_index = 0; layer_index < num_layers; layer_index++) {
      ...
      for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
        ...
        if (surface_id != solution_num_surfaces) {
          ...
          if (visible_mask[pixel] == true) {
            // color += image_.at<Vec3b>(pixel / IMAGE_WIDTH_, pixel %
            // IMAGE_WIDTH_);
            visible_mask[pixel] = false;
          }
          layer_image.at<Vec3b>(y, x) = color;
          depths[pixel] =
              solution_segments.at(surface_id)
                  .getDepth(1.0 * (pixel % image_width) / image_width,
                            1.0 * (pixel / image_width) / image_height);
        } else {
          Vec3b color;
          color[0] = 255;
          color[1] = color[2] = 0;
      ...
      for (map<int, int>::const_iterator surface_it = surface_centers.begin();
           surface_it != surface_centers.end(); surface_it++) {
        char *text = new char[10];
        sprintf(text, "%d", surface_it->first);
        putText(layer_image, text, Point(surface_it->second % image_width,
                                         surface_it->second / image_width),
                FONT_HERSHEY_PLAIN, 0.6, Scalar(0, 0, 255, 1));
      }
  ...
  for (int layer_index = 0; layer_index < num_layers; layer_index++) {
    ...
    layer_image_raw_filename << "Cache/scene_" << scene_index << "/"
                             << "layer_image_raw_" << result_index << "_"
                             << layer_index << ".bmp";
    imwrite(layer_image_raw_filename.str().c_str(), layer_image_raw);
  }

  // stringstream layer_disp_image_filename;
  // layer_disp_image_filename << "Results/scene_" << SCENE_INDEX_ << "/" <<
  // "layer_disp_image_" << iteration << "_" << layer_index << ".bmp";
  // imwrite(layer_disp_image_filename.str().c_str(), layer_disp_image);

  //  layer_alpha_map[layer_index] = 255 - (255 - 30) / (num_layers - 1) *
  //  (num_layers - 1 - layer_index);

  const int IMAGE_PADDING = 0;
  const int BORDER_WIDTH = 16;
  Mat multi_layer_image(ori_image.rows + BORDER_WIDTH * 2,
                        (ori_image.cols + BORDER_WIDTH * 2 + IMAGE_PADDING) *
                            (num_layers),
                        CV_8UC3);
  multi_layer_image.setTo(Scalar(255, 255, 255));
  for (int layer_index = 0; layer_index < num_layers; layer_index++) {
    Mat layer_image_with_border =
        Mat::zeros(ori_image.rows + BORDER_WIDTH * 2,
                   ori_image.cols + BORDER_WIDTH * 2, CV_8UC3);
    Mat layer_image_region(
        layer_image_with_border,
        Rect(BORDER_WIDTH, BORDER_WIDTH, ori_image.cols, ori_image.rows));
    layer_images[layer_index].copyTo(layer_image_region);

    Mat region(
        multi_layer_image,
        Rect((layer_image_with_border.cols + IMAGE_PADDING) * layer_index, 0,
             layer_image_with_border.cols, layer_image_with_border.rows));
    layer_image_with_border.copyTo(region);
  }
  // resize(multi_layer_image, multi_layer_image, Size(1000, 200));
  // const int IMAGE_PADDING = 10;
  // Mat multi_layer_image(image_height, (image_width + IMAGE_PADDING) *
  // num_layers, CV_8UC3);
  // multi_layer_image.setTo(Scalar(255, 255, 255));
  // for (int layer_index = 0; layer_index < num_layers; layer_index++) {
  //   Mat region(multi_layer_image, Rect((image_width + IMAGE_PADDING) *
  //   layer_index, 0, image_width, image_height));
  //   layer_images[layer_index].copyTo(region);
  // }
  stringstream multi_layer_image_filename;
  multi_layer_image_filename << "Results/scene_" << scene_index << "/"
                             << "multi_layer_image_" << result_index << ".bmp";
  imwrite(multi_layer_image_filename.str().c_str(), multi_layer_image);

  Mat input_multi_layer_image(
      ori_image.rows + BORDER_WIDTH * 2,
      (ori_image.cols + BORDER_WIDTH * 2 + IMAGE_PADDING) * (num_layers + 2),
      CV_8UC3);
  input_multi_layer_image.setTo(Scalar(0, 0, 0));

  Mat image_region(
      input_multi_layer_image,
      Rect(BORDER_WIDTH, BORDER_WIDTH, ori_image.cols, ori_image.rows));
  ori_image.copyTo(image_region);
  Mat disp_image_region(input_multi_layer_image,
                        Rect(BORDER_WIDTH + (ori_image.cols + BORDER_WIDTH * 2),
                             BORDER_WIDTH, ori_image.cols, ori_image.rows));
  imwrite("Test/ori_disp_image.bmp",
          drawDispImage(ori_point_cloud, ori_image.cols, ori_image.rows));
  ...
  Mat multi_layer_image_region(input_multi_layer_image,
                               Rect((ori_image.cols + BORDER_WIDTH * 2) * 2, 0,
                                    multi_layer_image.cols,
                                    multi_layer_image.rows));
  multi_layer_image.copyTo(multi_layer_image_region);

  stringstream input_multi_layer_image_filename;
  input_multi_layer_image_filename << "Results/scene_" << scene_index << "/"
                                   << "input_multi_layer_image_" << scene_index
                                   << ".bmp";
  imwrite(input_multi_layer_image_filename.str().c_str(),
          input_multi_layer_image);

  stringstream segments_filename;
  segments_filename << "Cache/scene_" << scene_index << "/segments_"
                    << result_index << ".txt";
  ofstream segments_out_str(segments_filename.str());
  segments_out_str << solution_num_surfaces << endl;
  for (map<int, Segment>::const_iterator surface_it = solution_segments.begin();
       surface_it != solution_segments.end(); surface_it++) {
  ...
  segment_GMMs_filename << "Cache/scene_" << scene_index << "/segment_GMMs_"
                        << result_index << ".xml";
  ...
  if (write_ply_files == true) {
    vector<vector<int>> layer_visible_pixel_segment_map(
        num_layers, vector<int>(NUM_PIXELS, -1));
    for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
      for (int layer_index = 0; layer_index < num_layers; layer_index++) {
    ...
    for (int layer_index = 0; layer_index < num_layers; layer_index++) {
      ...
      for (map<int, Segment>::const_iterator segment_it =
               solution_segments.begin();
           segment_it != solution_segments.end(); segment_it++) {
        vector<int> fitted_pixels = segment_it->second.getSegmentPixels();
        vector<int> new_fitted_pixels;
        for (vector<int>::const_iterator pixel_it = fitted_pixels.begin();
             pixel_it != fitted_pixels.end(); pixel_it++)
          if (layer_visible_pixel_segment_map[layer_index][*pixel_it] ==
              segment_it->first)
      ...
      for (map<int, vector<int>>::const_iterator segment_it =
               segment_fitted_pixels.begin();
           segment_it != segment_fitted_pixels.end(); segment_it++)
      ...
      for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
        vector<double> point(point_cloud.begin() + pixel * 3,
                             point_cloud.begin() + (pixel + 1) * 3);
        for (int c = 0; c < 3; c++) {
      ...
      // for (map<int, Segment>::const_iterator segment_it =
      // solution_segments.begin(); segment_it != solution_segments.end();
      // segment_it++) {
      ...
      //    for (int c = 0; c < 3; c++)
      //      if (c != max_direction)
      //        missing_value -= plane[c] *
      // plane_vertices[vertex_index][c];
      //    missing_value = plane[max_direction] == 0 ? 0 : missing_value
      // / plane[max_direction];
      ...
      for (map<int, Segment>::const_iterator segment_it =
               solution_segments.begin();
           segment_it != solution_segments.end(); segment_it++)
        segment_plane_vertices[segment_it->first] =
            vector<vector<double>>(4, vector<double>(3));
      stringstream layer_ply_filename;
      layer_ply_filename << "Results/scene_" << scene_index << "/"
                         << "layer_ply_" << result_index << "_" << layer_index
                         << ".ply";
      ...
      layer_segment_fitting_filename << "Results/scene_" << scene_index << "/"
                                     << "layer_segment_fitting_" << result_index
                                     << "_" << layer_index << ".ply";
      out_str.open(layer_segment_fitting_filename.str());

      out_str << "ply" << endl;
      out_str << "format ascii 1.0" << endl;
      out_str << "element vertex "
              << num_fitted_pixels + solution_segments.size() * 4 << endl;
      out_str << "property float x" << endl;
      out_str << "property float y" << endl;
      out_str << "property float z" << endl;
      ...
      for (map<int, vector<int>>::const_iterator segment_it =
        ...
        for (vector<int>::const_iterator pixel_it = segment_it->second.begin();
             pixel_it != segment_it->second.end(); pixel_it++) {
          int pixel = *pixel_it;

          // double depth =
          // solution_segments.at(segment_it->first).getDepth(pixel);
      ...
      for (map<int, vector<vector<double>>>::const_iterator segment_it =
               segment_plane_vertices.begin();
           segment_it != segment_plane_vertices.end(); segment_it++) {
        int color = color_table[segment_it->first];
        int r = color / (256 * 256);
        int g = color / 256 % 256;
        int b = color % 256;
        for (vector<vector<double>>::const_iterator vertex_it =
                 segment_it->second.begin();
             vertex_it != segment_it->second.end(); vertex_it++)
          out_str << vertex_it->at(0) << ' ' << vertex_it->at(1) << ' '
                  << vertex_it->at(2) << ' ' << r << ' ' << g << ' ' << b
                  << " 10" << endl;
      }
      for (int i = 0; i < segment_fitted_pixels.size(); i++) {
        out_str << "3 " << num_fitted_pixels + i * 4 + 0 << ' '
                << num_fitted_pixels + i * 4 + 1 << ' '
                << num_fitted_pixels + i * 4 + 2 << endl;
        out_str << "3 " << num_fitted_pixels + i * 4 + 0 << ' '
                << num_fitted_pixels + i * 4 + 2 << ' '
                << num_fitted_pixels + i * 4 + 3 << endl;
      }
      out_str.close();
    }
  }
  ...
  if (write_mesh_ply_files == true) {
    ...
    for (int layer_index = 0; layer_index < num_layers; layer_index++) {
      ...
      for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
        ...
        if (cell_pixels.size() == 3)
          triangle_pixels_vec.insert(triangle_pixels_vec.end(),
                                     cell_pixels.begin(), cell_pixels.end());
...

    {
      ...
      layer_segment_mesh_filename << "Results/scene_" << scene_index << "/"
                                  << "layered_mesh.ply";
      ...
      for (int layer_index = 0; layer_index < num_layers; layer_index++) {
        for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {
          ...
          for (vector<int>::const_iterator neighbor_pixel_it =
                   neighbor_pixels.begin();
               neighbor_pixel_it != neighbor_pixels.end();
               neighbor_pixel_it++) {
            if (layer_surface_ids[layer_index][*neighbor_pixel_it] !=
                segment_id) {
              on_boundary = true;
              break;
            }
          ...
          double depth =
              solution_segments.at(segment_id)
                  .getDepth(1.0 * (pixel % image_width) / image_width,
                            1.0 * (pixel / image_width) / image_height);
          ...
          // double Y = -point_cloud[pixel * 3 + 1];
          // double Z = point_cloud[pixel * 3 + 2];
          // int color = on_boundary ? 255 * 256 * 256 + 255 * 256 + 255 :
          // layer_color_table[layer_index];
          int color = layer_color_table[layer_index];
          //    out_str << X << ' ' << Y << ' ' << depth << endl;
          out_str << X << ' ' << Y << ' ' << depth << ' ' << color / (256 * 256)
                  << ' ' << color / 256 % 256 << ' ' << color % 256 << endl;
        }
      }

      for (int layer_index = 0; layer_index < num_layers; layer_index++) {
        for (int triangle_index = 0;
             triangle_index < layer_triangle_pixels_vec[layer_index].size() / 3;
             triangle_index++) {
          out_str << "3";
          for (int c = 0; c < 3; c++) {
            int pixel =
                layer_triangle_pixels_vec[layer_index][triangle_index * 3 + c];
...
void LayerDepthRepresenter::writeRenderingInfo(
    const vector<int> &solution_labels, const int solution_num_surfaces,
    const map<int, Segment> &solution_segments) {
  ...
  // upsampled_solution_labels_filename << "Cache/scene_" << SCENE_INDEX_ <<
  // "/upsampled_solution_labels.txt";
  // ifstream
  // upsampled_solution_labels_in_str(upsampled_solution_labels_filename.str());
  ...
  upsampleSolution(solution_labels, solution_num_surfaces, solution_segments,
                   new_solution_labels, new_solution_num_surfaces,
  ...
  for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
    ...
    layer_depth_values_filename << "Results/scene_" << SCENE_INDEX_ << "/"
                                << "depth_values_" << layer_index;
    ...
    for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++) {
      int surface_id = new_solution_labels[pixel] /
                       static_cast<int>(pow(new_solution_num_surfaces + 1,
                                            num_layers_ - 1 - layer_index)) %
                       (new_solution_num_surfaces + 1);
      if (surface_id < new_solution_num_surfaces) {
        vector<int> corner_pixels(4);
        corner_pixels[0] = (pixel / ORI_IMAGE_WIDTH) * (ORI_IMAGE_WIDTH + 1) +
                           (pixel % ORI_IMAGE_WIDTH);
        corner_pixels[1] =
            (pixel / ORI_IMAGE_WIDTH + 1) * (ORI_IMAGE_WIDTH + 1) +
            (pixel % ORI_IMAGE_WIDTH);
        corner_pixels[2] = (pixel / ORI_IMAGE_WIDTH) * (ORI_IMAGE_WIDTH + 1) +
                           (pixel % ORI_IMAGE_WIDTH + 1);
        corner_pixels[3] =
            (pixel / ORI_IMAGE_WIDTH + 1) * (ORI_IMAGE_WIDTH + 1) +
            (pixel % ORI_IMAGE_WIDTH + 1);
        for (vector<int>::const_iterator corner_pixel_it =
                 corner_pixels.begin();
             corner_pixel_it != corner_pixels.end(); corner_pixel_it++) {
          //      double depth =
          //      new_solution_segments.at(surface_id).getDepth(1.0 *
          //      (*corner_pixel_it % (ORI_IMAGE_WIDTH + 1)) / (ORI_IMAGE_WIDTH
          //      + 1), 1.0 * (*corner_pixel_it / (ORI_IMAGE_WIDTH + 1)) /
          //      (ORI_IMAGE_HEIGHT + 1));
          double depth =
              solution_segments.at(surface_id)
                  .getDepth(1.0 * (*corner_pixel_it % (ORI_IMAGE_WIDTH + 1)) /
                                (ORI_IMAGE_WIDTH + 1),
                            1.0 * (*corner_pixel_it / (ORI_IMAGE_WIDTH + 1)) /
                                (ORI_IMAGE_HEIGHT + 1));
          if (depth < 10) {
    ...
    for (int pixel = 0; pixel < (ORI_IMAGE_WIDTH + 1) * (ORI_IMAGE_HEIGHT + 1);
         pixel++) {
      if (counter[pixel] == 0)
        depths[pixel] = -1;
      else
    ...
    // sub_sampled_depths = subSampleDepthMap(depths, ORI_IMAGE_WIDTH,
    // ORI_IMAGE_HEIGHT, ORI_IMAGE_WIDTH + 1, ORI_IMAGE_HEIGHT + 1);

    //    Mat test_image(ORI_IMAGE_HEIGHT + 1, ORI_IMAGE_WIDTH + 1, CV_8UC1);

    ofstream depth_values_out_str(layer_depth_values_filename.str());
    depth_values_out_str << ORI_IMAGE_WIDTH + 1 << ' ' << ORI_IMAGE_HEIGHT + 1
                         << endl;
    for (int pixel = 0; pixel < depths.size(); pixel++) {
      double depth = depths[pixel];
      depth_values_out_str << depth << endl;
      // if (depth > 0)
      //      test_image.at<uchar>(pixel / (ORI_IMAGE_WIDTH + 1), pixel %
      //      (ORI_IMAGE_WIDTH + 1)) = 255;
      // else
      //      test_image.at<uchar>(pixel / (ORI_IMAGE_WIDTH + 1), pixel %
      //      (ORI_IMAGE_WIDTH + 1)) = 0;
    }
  ...
  vector<vector<int>> layer_surface_ids(num_layers_,
                                        vector<int>(ORI_NUM_PIXELS, 0));
  ...
  //    for (vector<int>::const_iterator neighbor_pixel_it =
  // neighbor_pixels.begin(); neighbor_pixel_it != neighbor_pixels.end();
  // neighbor_pixel_it++)
  //      surface_id_counter[surface_ids[*neighbor_pixel_it]]++;
  //    int max_count = 0;
  //    int max_count_surface_id = -1;
  //    for (map<int, int>::const_iterator surface_it =
  // surface_id_counter.begin(); surface_it != surface_id_counter.end();
  // surface_it++) {
  //      if (surface_it->second > max_count && surface_it->first !=
  // solution_num_surfaces) {
  ...
  for (int pixel = 0; pixel < ORI_NUM_PIXELS; pixel++) {
    ...
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      //      int surface_id = label /
      //      static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
      //      layer_index)) % (solution_num_surfaces + 1);
      int surface_id = layer_surface_ids[layer_index][pixel];
      if (surface_id == new_solution_num_surfaces)
        continue;
  ...
  const double COLOR_LIKELIHOOD_THRESHOLD =
      1; // STATISTICS_.fitting_color_likelihood_threshold;
  for (int segment_id = 0; segment_id < new_solution_num_surfaces;
       segment_id++) {
    if (hole_pixels_vec[segment_id].size() == 0)
      continue;
    vector<bool> known_region_mask(ORI_NUM_PIXELS, false);
    for (vector<int>::const_iterator pixel_it =
             segment_pixels_vec[segment_id].begin();
         pixel_it != segment_pixels_vec[segment_id].end(); pixel_it++)
      known_region_mask[*pixel_it] = true;
    for (vector<int>::const_iterator pixel_it =
             hole_pixels_vec[segment_id].begin();
         pixel_it != hole_pixels_vec[segment_id].end(); pixel_it++)
    ...
    for (int iteration = 0; iteration < NUM_EROSION_ITERATIONS; iteration++) {
      ...
      for (vector<int>::const_iterator pixel_it = known_region_pixels.begin();
           pixel_it != known_region_pixels.end(); pixel_it++) {
        ...
        if (on_boundary == false)
          continue;
        // if
        // (new_solution_segments.at(segment_id).predictColorLikelihood(pixel,
        // blurred_hsv_image.at<Vec3b>(y, x)) < COLOR_LIKELIHOOD_THRESHOLD)
        new_hole_pixel_indices.insert(pixel_it - known_region_pixels.begin());
      }
      if (new_hole_pixel_indices.size() == 0 ||
          new_hole_pixel_indices.size() ==
              segment_pixels_vec[segment_id].size())
        break;
      for (set<int>::const_iterator index_it = new_hole_pixel_indices.begin();
           index_it != new_hole_pixel_indices.end(); index_it++) {
  ...
  //   segment_mask_image_filename << "Test/segment_mask_image_" << segment_id
  //   << ".bmp";
  //   Mat segment_mask_image = Mat::zeros(ORI_IMAGE_HEIGHT, ORI_IMAGE_WIDTH,
  //   CV_8UC1);
  //   for (vector<int>::const_iterator pixel_it =
  //   segment_pixels_vec[segment_id].begin(); pixel_it !=
  //   segment_pixels_vec[segment_id].end(); pixel_it++)
  //     segment_mask_image.at<uchar>(*pixel_it / ORI_IMAGE_WIDTH, *pixel_it %
  //     ORI_IMAGE_WIDTH) = 255;
  ...
  //   hole_mask_image_filename << "Test/hole_mask_image_" << segment_id <<
  //   ".bmp";
  //   Mat hole_mask_image = Mat::zeros(ORI_IMAGE_HEIGHT, ORI_IMAGE_WIDTH,
  //   CV_8UC1);
  //   for (vector<int>::const_iterator pixel_it =
  //   hole_pixels_vec[segment_id].begin(); pixel_it !=
  //   hole_pixels_vec[segment_id].end(); pixel_it++)
  //     hole_mask_image.at<uchar>(*pixel_it / ORI_IMAGE_WIDTH, *pixel_it %
  //     ORI_IMAGE_WIDTH) = 255;
  //   imwrite(hole_mask_image_filename.str(), hole_mask_image);
  // }
  // exit(1);

  // for (map<int, Segment>::iterator segment_it =
  // new_solution_segments.begin(); segment_it != new_solution_segments.end();
  // segment_it++)
  //   cout << segment_it->first << '\t' <<
  //   hole_pixels_vec[segment_it->first].size() << '\t' <<
  //   segment_pixels_vec[segment_it->first].size() << '\t' <<
  //   segment_it->second.getType() << endl;

  map<int, Mat> completed_images;
  // PatchMatcher patch_matcher(ori_image_);
  for (int segment_id = 0; segment_id < new_solution_num_surfaces;
       segment_id++) {
    // cout << segment_id << '\t' << hole_pixels_vec[segment_id].size() << '\t'
    // << segment_pixels_vec[segment_id].size() << '\t' <<
    // new_solution_segments[segment_id].getType() << endl;
    // if (segment_id != 4)
    //   continue;
    stringstream completed_image_filename;
    completed_image_filename << "Cache/scene_" << SCENE_INDEX_
                             << "/completed_image_" << segment_id << ".bmp";
    if (imread(completed_image_filename.str()).empty() || true) {
      ...
      for (vector<int>::const_iterator pixel_it =
               segment_pixels_vec[segment_id].begin();
           pixel_it != segment_pixels_vec[segment_id].end(); pixel_it++)
        mask_image.at<uchar>(*pixel_it / ori_image_.cols,
                             *pixel_it % ori_image_.cols) = 128;
      for (vector<int>::const_iterator pixel_it =
               hole_pixels_vec[segment_id].begin();
           pixel_it != hole_pixels_vec[segment_id].end(); pixel_it++)
        mask_image.at<uchar>(*pixel_it / ori_image_.cols,
                             *pixel_it % ori_image_.cols) = 0;

      if (false) {
        const int INPAINTING_REGION_WIDTH = 10;
        vector<int> distance_map(ORI_NUM_PIXELS, 1000000);
        for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++)
          if (mask_image.at<uchar>(pixel / ori_image_.cols,
                                   pixel % ori_image_.cols) < 100)
            distance_map[pixel] = 0;
        for (int distance = 0; distance < INPAINTING_REGION_WIDTH; distance++) {
          for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT;
               pixel++) {
            if (distance_map[pixel] != distance)
              continue;
            vector<int> neighbor_pixels;
            int x = pixel % ORI_IMAGE_WIDTH;
            int y = pixel / ORI_IMAGE_WIDTH;
            ...
            for (vector<int>::const_iterator neighbor_pixel_it =
                 ...
                 neighbor_pixel_it != neighbor_pixels.end();
                 neighbor_pixel_it++)
              distance_map[*neighbor_pixel_it] = distance + 1;
          }
        }
        for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++)
          if (mask_image.at<uchar>(pixel / ori_image_.cols,
                                   pixel % ori_image_.cols) > 100 &&
              mask_image.at<uchar>(pixel / ori_image_.cols,
                                   pixel % ori_image_.cols) < 200 &&
              distance_map[pixel] > INPAINTING_REGION_WIDTH)
            mask_image.at<uchar>(pixel / ori_image_.cols,
                                 pixel % ori_image_.cols) = 220;
      }

      {
        stringstream mask_image_filename;
        mask_image_filename << "Test/image_for_completion_" << segment_id
                            << ".bmp";
      ...
      Mat completed_image; // = patch_matcher.getCompletedImage();
      ...
      for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++)
        if (mask_image.at<uchar>(pixel / ori_image_.cols,
                                 pixel % ori_image_.cols) > 200 &&
            mask_image.at<uchar>(pixel / ori_image_.cols,
                                 pixel % ori_image_.cols) < 230)
          completed_image.at<uchar>(pixel / ori_image_.cols,
                                    pixel % ori_image_.cols) =
              ori_image_.at<uchar>(pixel / ori_image_.cols,
                                   pixel % ori_image_.cols);
  ...
  for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
    ...
    for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++) {
      ...
      if (surface_id < new_solution_num_surfaces)
        texture_image.at<Vec3b>(y, x) =
            completed_images[surface_id].at<Vec3b>(y, x);
      else {
        // texture_image.at<Vec3b>(pixel / ORI_IMAGE_WIDTH, pixel %
        // ORI_IMAGE_WIDTH) = Vec3b(255, 0, 0);
        // continue;
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          if (surface_ids[*neighbor_pixel_it] != new_solution_num_surfaces) {
            Vec3b color =
                completed_images[surface_ids[*neighbor_pixel_it]].at<Vec3b>(
                    *neighbor_pixel_it / ORI_IMAGE_WIDTH,
                    *neighbor_pixel_it % ORI_IMAGE_WIDTH);
            b_sum += color[0];
            g_sum += color[1];
            r_sum += color[2];
        ...
        if (num_valid_colors == 0)
          texture_image.at<Vec3b>(pixel / ORI_IMAGE_WIDTH,
                                  pixel % ORI_IMAGE_WIDTH) = Vec3b(255, 0, 0);
        else
          texture_image.at<Vec3b>(pixel / ORI_IMAGE_WIDTH,
                                  pixel % ORI_IMAGE_WIDTH) =
              Vec3b(b_sum / num_valid_colors, g_sum / num_valid_colors,
                    r_sum / num_valid_colors);
      }
    }

    stringstream texture_image_filename;
    texture_image_filename << "Results/scene_" << SCENE_INDEX_
                           << "/texture_image_" << layer_index << ".bmp";
    ...
    for (vector<int>::const_iterator pixel_it = visible_pixels.begin();
         pixel_it != visible_pixels.end(); pixel_it++) {
      // break;
      texture_image.at<Vec3b>(*pixel_it / ORI_IMAGE_WIDTH,
                              *pixel_it % ORI_IMAGE_WIDTH) =
          ori_image_.at<Vec3b>(*pixel_it / ORI_IMAGE_WIDTH,
                               *pixel_it % ORI_IMAGE_WIDTH);
    }
    stringstream static_texture_image_filename;
    static_texture_image_filename << "Results/scene_" << SCENE_INDEX_
                                  << "/static_texture_image_" << layer_index
                                  << ".bmp";
  ...
  Mat object_removal_image =
      Mat::zeros(ORI_IMAGE_HEIGHT, ORI_IMAGE_WIDTH, CV_8UC3);
  ...
  for (int ori_pixel = 0; ori_pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT;
       ori_pixel++) {
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      int surface_id = new_solution_labels[ori_pixel] /
                       static_cast<int>(pow(new_solution_num_surfaces + 1,
                                            num_layers_ - 1 - layer_index)) %
                       (new_solution_num_surfaces + 1);
  ...
  for (int iteration = 0; iteration < NUM_DILATION_ITERATIONS; iteration++) {
    vector<set<int>> dilated_pixel_segment_indices_map =
        pixel_segment_indices_map;
    for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
      ...
      for (vector<int>::const_iterator neighbor_pixel_it =
        ...
        //   continue;
        for (set<int>::const_iterator segment_it =
                 pixel_segment_indices_map[ori_pixel].begin();
             segment_it != pixel_segment_indices_map[ori_pixel].end();
             segment_it++) {
          if (dilated_pixel_segment_indices_map[*neighbor_pixel_it].count(
                  *segment_it) > 0)
            continue;
          dilated_pixel_segment_indices_map[*neighbor_pixel_it].insert(
              *segment_it);
        }
      }
    }
    ...
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      ...
      if (surface_id < new_solution_num_surfaces) {
        if (pixel_segment_indices_map[ori_pixel].size() == 0)
          object_removal_image.at<Vec3b>(ori_pixel / ORI_IMAGE_WIDTH,
                                         ori_pixel % ORI_IMAGE_WIDTH) =
              static_texture_images[layer_index].at<Vec3b>(
                  ori_pixel / ORI_IMAGE_WIDTH, ori_pixel % ORI_IMAGE_WIDTH);
        else
          object_removal_image.at<Vec3b>(ori_pixel / ORI_IMAGE_WIDTH,
                                         ori_pixel % ORI_IMAGE_WIDTH) =
              texture_images[layer_index].at<Vec3b>(
                  ori_pixel / ORI_IMAGE_WIDTH, ori_pixel % ORI_IMAGE_WIDTH);
        break;
      }
    }
  ...
  object_removal_image_filename << "Results/scene_" << SCENE_INDEX_ << "/"
                                << "object_removal_image.bmp";
  imwrite(object_removal_image_filename.str(), object_removal_image);

  stringstream rendering_info_filename;
  rendering_info_filename << "Results/scene_" << SCENE_INDEX_ << "/"
                          << "rendering_info";
...
vector<double> LayerDepthRepresenter::subSampleDepthMap(
    const vector<double> &depths, const int ori_width, const int ori_height,
  ...
  for (int y = 0; y < new_height; ++y) {
    for (int x = 0; x < new_width; ++x) {
      ...
      new_depth_values[y * new_width + x] =
          interpolateDepthValue(depths, ori_width, ori_height, ori_x, ori_y);
...
// bool LayerDepthRepresenter::readLayers(vector<int> &solution, int
// &solution_num_surfaces, map<int, Segment> &solution_segments, const int
// iteration)
// {
//   if (FIRST_TIME_)
//     return false;
...
//   stringstream segments_filename;
//   segments_filename << "Cache/scene_" << SCENE_INDEX_ << "/segments_" <<
//   iteration << ".txt";
...
//     assert(segment_id == i);
//     Segment segment(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_,
//     PENALTIES_, STATISTICS_);
...
//   stringstream segment_GMMs_filename;
//   segment_GMMs_filename << "Cache/scene_" << SCENE_INDEX_ << "/segment_GMMs_"
//   << iteration << ".xml";
//   FileStorage segment_GMMs_fs(segment_GMMs_filename.str(),
//   FileStorage::READ);
//   for (map<int, Segment>::iterator surface_it = solution_segments.begin();
//   surface_it != solution_segments.end(); surface_it++) {
//     stringstream segment_name;
//     segment_name << "Segment" << surface_it->first;
...
//     //cout <<
//     surface_it->second.predictColorLikelihood(static_cast<int>(6.5320558718089686e+01
//     + 0.5) * IMAGE_WIDTH_ + static_cast<int>(3.6378882105214679e+01 + 0.5),
//     Vec3b(2.1218500883957713e+01, 1.8951972423041440e+01,
//     3.3314066835337023e+01)) << endl;
...
//     layer_image_filename << "Cache/scene_" << SCENE_INDEX_ << "/" <<
//     "layer_image_raw_" << iteration << "_" << layer_index << ".bmp";
...
//       solution[pixel] += surface_id * pow(solution_num_surfaces + 1,
//       num_layers_ - 1 - layer_index);
//     }
//   }
//   return true;
...
bool readLayers(const int image_width, const int image_height,
                ...
                const DataStatistics &statistics, const int num_layers,
                vector<int> &solution, int &solution_num_surfaces,
                map<int, Segment> &solution_segments, const int scene_index,
  ...
  for (int i = 0; i < solution_num_surfaces; i++) {
    ...
    segments_in_str >> segment_id;
    assert(segment_id == i);
    Segment segment(image_width, image_height, camera_parameters, penalties,
                    statistics);
    segments_in_str >> segment;
    // cout << segment << endl;
    solution_segments[i] = segment;
  ...
  for (int layer_index = 0; layer_index < num_layers; layer_index++) {
    stringstream layer_image_filename;
    layer_image_filename << "Cache/scene_" << scene_index << "/"
                         << "layer_image_raw_" << result_index << "_"
                         << layer_index << ".bmp";
    Mat layer_image = imread(layer_image_filename.str().c_str(), 0);
    if (layer_image.empty())
...
double LayerDepthRepresenter::interpolateDepthValue(
    const vector<double> &depths, const int depth_width, const int depth_height,
  ...
  double depth_value_3 =
      (x >= 0 && y < depth_height - 1) ? depths[index_3] : -1;
  double depth_value_4 =
      (x < depth_width - 1 && y < depth_height - 1) ? depths[index_4] : -1;
  ...
  case 4: {
    ...
    interpolated_depth_value = depth_value_1 * area_1 + depth_value_2 * area_2 +
                               depth_value_3 * area_3 + depth_value_4 * area_4;
    break;
  }
  case 3: {
    if (depth_value_1 > 0 && depth_value_2 > 0 && depth_value_3 > 0) {
      ...
      if (area_2 + area_3 <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value = depth_value_1 * area_1 +
                                   depth_value_2 * area_2 +
                                   depth_value_3 * area_3;
      ...
      if (area_2 + area_3 <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value = depth_value_2 * area_2 +
                                   depth_value_3 * area_3 +
                                   depth_value_4 * area_4;
        interpolated_depth_value *= 2;
      }
  ...
  case 2: {
    if (depth_value_1 > 0 && depth_value_2 > 0) {
      if (y - static_cast<int>(y) <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value =
            depth_value_1 * length_1 + depth_value_2 * length_2;
      }
      ...
      if (x - static_cast<int>(x) <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value =
            depth_value_1 * length_1 + depth_value_3 * length_3;
      }
      ...
      if (static_cast<int>(x + 1) - x <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value =
            depth_value_2 * length_2 + depth_value_4 * length_4;
      }
      ...
      if (static_cast<int>(y + 1) - y <= INTERPOLATION_GAP_THRESHOLD) {
        ...
        interpolated_depth_value =
            depth_value_3 * length_3 + depth_value_4 * length_4;
      }
    } else if (depth_value_1 > 0 && depth_value_4 > 0) {
      if (abs((x - static_cast<int>(x)) - (y - static_cast<int>(y))) <=
          INTERPOLATION_GAP_THRESHOLD) {
        double length_1 =
            1 - ((x - static_cast<int>(x)) + (y - static_cast<int>(y))) / 2;
        double length_4 =
            ((x - static_cast<int>(x)) + (y - static_cast<int>(y))) / 2;
        interpolated_depth_value =
            depth_value_1 * length_1 + depth_value_4 * length_4;
      }
    } else if (depth_value_2 > 0 && depth_value_3 > 0) {
      if (abs((x - static_cast<int>(x)) + (y - static_cast<int>(y)) - 1) <=
          INTERPOLATION_GAP_THRESHOLD) {
        double length_2 =
            0.5 + ((x - static_cast<int>(x)) - (y - static_cast<int>(y))) / 2;
        double length_3 =
            0.5 - ((x - static_cast<int>(x)) - (y - static_cast<int>(y))) / 2;
        interpolated_depth_value =
            depth_value_2 * length_2 + depth_value_3 * length_3;
      }
    }
    break;
  }
  case 1: {
    if (depth_value_1 > 0) {
      if ((x - static_cast<int>(x)) <= INTERPOLATION_GAP_THRESHOLD &&
          (y - static_cast<int>(y)) <= INTERPOLATION_GAP_THRESHOLD)
      ...
      if ((x - static_cast<int>(x)) <= INTERPOLATION_GAP_THRESHOLD &&
          (static_cast<int>(y + 1) - y) <= INTERPOLATION_GAP_THRESHOLD)
        interpolated_depth_value = depth_value_3;
    } else if (depth_value_4 > 0) {
      if ((static_cast<int>(x + 1) - x) <= INTERPOLATION_GAP_THRESHOLD &&
          (y - static_cast<int>(y + 1) - y) <= INTERPOLATION_GAP_THRESHOLD)
...
void LayerDepthRepresenter::generateLayerImageHTML(
    const int scene_index,
    const map<int, vector<double>> &iteration_statistics_map,
    const map<int, string> &iteration_proposal_type_map) {
  ...
  double previous_energy = -1;
  for (map<int, string>::const_iterator iteration_it =
           iteration_proposal_type_map.begin();
       iteration_it != iteration_proposal_type_map.end(); iteration_it++) {
    html_out_str << "<h3>iteration " << iteration_it->first << ": "
                 << iteration_it->second << "</h3>" << endl;
    ...
    if (previous_energy < 0 || energy < previous_energy) {
      ...
      html_out_str << "<img src=\"" << image_filename.str() << "\" alt=\""
                   << image_filename.str()
...
void LayerDepthRepresenter::upsampleSolution(
    const vector<int> &solution_labels, const int solution_num_surfaces,
    const map<int, Segment> &solution_segments,
    vector<int> &upsampled_solution_labels,
    int &upsampled_solution_num_surfaces,
    map<int, Segment> &upsampled_solution_segments) {
  ...
  //   for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
  //     int surface_id = solution_label /
  //     static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
  //     layer_index)) % (solution_num_surfaces + 1);
  //     if (surface_id < solution_num_surfaces) {
  //  if (color_table.count(surface_id) == 0)
  //    color_table[surface_id] = rand() % 256;
  //  segmentation_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel % IMAGE_WIDTH_)
  // = color_table[surface_id];
  //  break;
  //     }
  //   }
  ...
  estimateCameraParameters(ori_point_cloud_, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
                           ori_camera_parameters);
  // vector<double> ori_normals = calcNormals(ori_point_cloud_, ORI_IMAGE_WIDTH,
  // ORI_IMAGE_HEIGHT);

  // if (readLayers(ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT, ori_camera_parameters,
  // PENALTIES_, STATISTICS_, num_layers_, upsampled_solution_labels,
  // upsampled_solution_num_surfaces, upsampled_solution_segments, SCENE_INDEX_,
  // 20000) == true) {
  //   return;
  // }
  ...
  // map<int, Segment> proposal_segments;
  // string proposal_type;
  // proposal_designer->getUpsamplingProposal(ori_image_, ori_point_cloud_,
  // ori_normals, ori_camera_parameters, proposal_labels, proposal_num_surfaces,
  // proposal_segments, NUM_DILATION_ITERATIONS);
  // //vector<int> previous_solution_indices =
  // proposal_designer->getCurrentSolutionIndices();

  // RepresenterPenalties upsampling_penalties = PENALTIES_;
  // upsampling_penalties.depth_inconsistency_pen =
  // PENALTIES_.color_inconsistency_pen;
  ...
  // //upsampling_penalties.color_inconsistency_pen =
  // PENALTIES_.depth_inconsistency_pen;
  // unique_ptr<TRWSFusion> TRW_solver(new TRWSFusion(ori_image_,
  // ori_point_cloud_, ori_normals, upsampling_penalties, STATISTICS_, false));

  // upsampled_solution_labels = TRW_solver->fuse(proposal_labels,
  // proposal_num_surfaces, num_layers_, proposal_segments,
  // vector<int>(ORI_NUM_PIXELS, 0));
  ...
  // for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++) {
  //   int scaled_pixel = min(static_cast<int>(round(static_cast<double>(pixel /
  //   ORI_IMAGE_WIDTH) / ORI_IMAGE_HEIGHT * IMAGE_HEIGHT_)), IMAGE_HEIGHT_ - 1)
  //   * IMAGE_WIDTH_ + min(static_cast<int>(round(static_cast<double>(pixel %
  //   ORI_IMAGE_WIDTH) / ORI_IMAGE_WIDTH * IMAGE_WIDTH_)), IMAGE_WIDTH_ - 1);
  //   new_solution_labels[pixel] = solution_labels[scaled_pixel];
  //   //if (solution_labels[scaled_pixel] != new_solution_labels[pixel])
  //   //cout << pixel << '\t' << solution_labels[scaled_pixel] << '\t' <<
  //   new_solution_labels[pixel] << endl;
  // }

  // //writeLayers(ori_image_, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
  // ori_point_cloud_, ori_camera_parameters, num_layers_,
  // upsampled_solution_labels, upsampled_solution_num_surfaces,
  // upsampled_solution_segments, SCENE_INDEX_, 100, ori_image_);

  // // stringstream upsampled_solution_labels_filename;
  // // upsampled_solution_labels_filename << "Cache/scene_" << SCENE_INDEX_ <<
  // "/upsampled_solution_labels.txt";
  // // ofstream
  // upsampled_solution_labels_out_str(upsampled_solution_labels_filename.str());
  // // upsampled_solution_labels_out_str << new_solution_labels.size() << endl;
  // // for (vector<int>::const_iterator label_it = new_solution_labels.begin();
  // label_it != new_solution_labels.end(); label_it++)
  // //   upsampled_solution_labels_out_str << *label_it << endl;
  // // upsampled_solution_labels_out_str.close();

  // // writeDispImageFromSegments(new_solution_labels, solution_num_surfaces,
  // new_solution_segments, num_layers_, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
  // "Test/final_disp_image.bmp");
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      int surface_id = solution_label /
                       static_cast<int>(pow(solution_num_surfaces + 1,
                                            num_layers_ - 1 - layer_index)) %
                       (solution_num_surfaces + 1);
      if (surface_id < solution_num_surfaces) {
  ...
  vector<vector<bool>> layer_confident_pixel_mask(
      num_layers_, vector<bool>(NUM_PIXELS_, true));
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    vector<int> neighbor_pixels;
    int x = pixel % IMAGE_WIDTH_;
    int y = pixel / IMAGE_WIDTH_;
    ...
    for (vector<int>::const_iterator neighbor_pixel_it =
             neighbor_pixels.begin();
         neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
      if (visible_layer_indices[pixel] <
          visible_layer_indices[*neighbor_pixel_it]) {
        layer_confident_pixel_mask[visible_layer_indices[pixel]][pixel] = false;
        layer_confident_pixel_mask[visible_layer_indices[pixel]]
                                  [*neighbor_pixel_it] = false;
        break;
      }
    }
  }
  ...
  //     confident_pixel_image.at<Vec3b>(pixel / IMAGE_WIDTH_, pixel %
  //     IMAGE_WIDTH_) = Vec3b(0, 0, 0);
  ...
  // for (map<int, map<int, vector<int> > >::const_iterator segment_it =
  // segment_layer_visible_pixels.begin(); segment_it !=
  // segment_layer_visible_pixels.end(); segment_it++) {
  //   for (map<int, vector<int> >::const_iterator layer_it =
  //   segment_it->second.begin(); layer_it != segment_it->second.end();
  //   layer_it++) {
  //     Segment segment = solution_segments.at(segment_it->first);
  //     //      segment.refitSegmentKeepingGeometry(image_, point_cloud_,
  //     normals_, layer_it->second);
  //     segments[segment_index] = segment;
  //     segment_layer_map[segment_index] = layer_it->first;
  //     segment_index_map[segment_index] = segment_it->first;
  //     for (vector<int>::const_iterator pixel_it = layer_it->second.begin();
  //     pixel_it != layer_it->second.end(); pixel_it++)
  ...
  // segmentation_image_low_res_filename << "Results/scene_" << SCENE_INDEX_ <<
  // "/segmentation_image_low_res.bmp";
  ...
  //     }
  //     segmentation_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
  //     IMAGE_WIDTH_) = color_table[segment_id];
  //   }
  //   imwrite(segmentation_image_low_res_filename.str(), segmentation_image);
  // } else {
  //   Mat segmentation_image =
  //   imread(segmentation_image_low_res_filename.str(), 0);
  //   for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //     int segment_id = visible_segmentation[pixel];
  //     int color = segmentation_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
  //     IMAGE_WIDTH_);
  //     color_table[segment_id] = color;
  //     reverse_color_table[color] = segment_id;
  //   }
  ...
  // segmentation_image_high_res_filename << "Results/scene_" << SCENE_INDEX_ <<
  // "/segmentation_image_high_res.bmp";
  // Mat segmentation_high_res_image =
  // imread(segmentation_image_high_res_filename.str(), 0);

  // vector<int> segmentation_high_res(ORI_NUM_PIXELS);
  // for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
  //   int color = segmentation_high_res_image.at<uchar>(ori_pixel /
  //   ORI_IMAGE_WIDTH, ori_pixel % ORI_IMAGE_WIDTH);
  //   segmentation_high_res[ori_pixel] = reverse_color_table[color];
  // }

  // for (map<int, int>::const_iterator color_it = reverse_color_table.begin();
  // color_it != reverse_color_table.end(); color_it++)
  //   cout << color_it->first << '\t' << color_it->second << '\t' <<
  //   segment_layer_map[color_it->second] << endl;
  // exit(1);
  ...
  //   solution_segments_high_res[segment_it->first] = Segment(ori_image_,
  //   ori_point_cloud_, ori_normals, ori_camera_parameters,
  //   segment_visible_pixels[segment_it->first], PENALTIES_, STATISTICS_);
  // }

  vector<vector<double>> distance_map(ORI_NUM_PIXELS,
                                      vector<double>(9, 1000000));
  for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
    int x = ori_pixel % ORI_IMAGE_WIDTH;
    int y = ori_pixel / ORI_IMAGE_WIDTH;
    for (int delta_x = -1; delta_x <= 1; delta_x++) {
      for (int delta_y = -1; delta_y <= 1; delta_y++) {
        if (x + delta_x >= 0 && x + delta_x < ORI_IMAGE_WIDTH &&
            y + delta_y >= 0 && y + delta_y < ORI_IMAGE_HEIGHT) {
  ...
  // cout << ori_image_.at<Vec3b>(639, 214) << '\t' << ori_image_.at<Vec3b>(640,
  // 214) << endl;
  // cout << distance_map[639214][7] << '\t' << distance_map[639214][5] << endl;
  // exit(1);
  const double DISTANCE_2D_WEIGHT = 0 * IMAGE_WIDTH_ / ORI_IMAGE_WIDTH;
  // cout << calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH,
  // ORI_IMAGE_HEIGHT, 639214, 645220, DISTANCE_2D_WEIGHT) << endl;
  //  cout << calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH,
  //  ORI_IMAGE_HEIGHT, 628585, 627577, DISTANCE_2D_WEIGHT) << endl;
  // exit(1);

  vector<int> solution_labels_high_res(ORI_NUM_PIXELS, 0);
  for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
    // cout << ori_pixel << endl;
    //    if (ori_pixel % ORI_IMAGE_WIDTH < 250 || ori_pixel % ORI_IMAGE_WIDTH >
    //    250 || ori_pixel / ORI_IMAGE_WIDTH < 630 || ori_pixel /
    //    ORI_IMAGE_WIDTH > 630)
    // continue;
    // cout << ori_pixel << endl;
    double x =
        1.0 * (ori_pixel % ORI_IMAGE_WIDTH) / ORI_IMAGE_WIDTH * IMAGE_WIDTH_;
    double y =
        1.0 * (ori_pixel / ORI_IMAGE_WIDTH) / ORI_IMAGE_HEIGHT * IMAGE_HEIGHT_;
    vector<int> xs;
    ...
    for (vector<int>::const_iterator y_it = ys.begin(); y_it != ys.end();
         y_it++)
      for (vector<int>::const_iterator x_it = xs.begin(); x_it != xs.end();
           x_it++)
        vertex_pixels.push_back(*y_it * IMAGE_WIDTH_ + *x_it);

    // bool has_confident_pixels = false;
    // for (vector<int>::const_iterator pixel_it = vertex_pixels.begin();
    // pixel_it != vertex_pixels.end(); pixel_it++) {
    ...
    //   xs.push_back(min(static_cast<int>(x) + 2 + dilation_iteration,
    //   IMAGE_WIDTH_ - 1));
    //   ys.push_back(max(static_cast<int>(y - 1 - dilation_iteration), 0));
    //   ys.push_back(min(static_cast<int>(y) + 2 + dilation_iteration,
    //   IMAGE_HEIGHT_ - 1));
    ...
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      ...
      // distance_segment_id_pairs.push_back(make_pair(sqrt(pow(x_1 - x, 2) +
      // pow(x_1 - y, 2)), solution_labels[y_1 * IMAGE_WIDTH_ + x_1] /
      // static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
      // layer_index)) % (solution_num_surfaces + 1)));
      vector<int> segment_indices;
      for (vector<int>::const_iterator pixel_it = vertex_pixels.begin();
           pixel_it != vertex_pixels.end(); pixel_it++)
        segment_indices.push_back(
            solution_labels[*pixel_it] /
            static_cast<int>(
                pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index)) %
            (solution_num_surfaces + 1));

      map<int, map<int, int>> surface_occluding_relations;
      for (vector<int>::const_iterator segment_it_1 = segment_indices.begin();
           segment_it_1 != segment_indices.end(); segment_it_1++) {
        if (*segment_it_1 == solution_num_surfaces)
          continue;
        int vertex_pixel =
            vertex_pixels[segment_it_1 - segment_indices.begin()];
        for (vector<int>::const_iterator segment_it_2 = segment_indices.begin();
             segment_it_2 != segment_indices.end(); segment_it_2++) {
          if (*segment_it_2 == solution_num_surfaces ||
              *segment_it_2 == *segment_it_1)
            continue;
          //    cout << vertex_pixel << '\t' << *segment_it_1 << '\t' <<
          //*segment_it_2 << '\t' <<
          // solution_segments.at(*segment_it_1).getDepth(vertex_pixel) << '\t'
          //<< solution_segments.at(*segment_it_2).getDepth(vertex_pixel) <<
          // endl;
          if (solution_segments.at(*segment_it_2).getDepth(vertex_pixel) >
                  solution_segments.at(*segment_it_1).getDepth(vertex_pixel) ||
              solution_segments.at(*segment_it_2).getDepth(vertex_pixel) < 0)
      ...
      for (vector<int>::const_iterator segment_it = segment_indices.begin();
           segment_it != segment_indices.end(); segment_it++) {
        ...
        } else if (*segment_it != solution_num_surfaces &&
                   *segment_it != selected_segment_index) {
          if (surface_occluding_relations[*segment_it][selected_segment_index] +
                  surface_occluding_relations[selected_segment_index]
                                             [*segment_it] >
              0) {
            if (solution_segments.at(*segment_it)
                        .getDepth(x / IMAGE_WIDTH_, y / IMAGE_HEIGHT_) <
                    solution_segments.at(selected_segment_index)
                        .getDepth(x / IMAGE_WIDTH_, y / IMAGE_HEIGHT_) ||
                solution_segments.at(selected_segment_index)
                        .getDepth(x / IMAGE_WIDTH_, y / IMAGE_HEIGHT_) < 0) {
              selected_segment_index = *segment_it;
              selected_vertex = segment_it - segment_indices.begin();
              // cout << "1 " << selected_segment_index << endl;
            }
          } else if (surface_occluding_relations[*segment_it]
            ...
            if (sqrt(pow(x -
                     ...
                     pow(y -
                             vertex_pixels[segment_it -
                                 ...
                                 IMAGE_WIDTH_,
                         2)) <
                sqrt(pow(x - vertex_pixels[selected_vertex] % IMAGE_WIDTH_, 2) +
                     pow(y - vertex_pixels[selected_vertex] / IMAGE_WIDTH_,
                         2))) {
              selected_segment_index = *segment_it;
              selected_vertex = segment_it - segment_indices.begin();
      ...
      for (vector<int>::const_iterator segment_it = segment_indices.begin();
        ...
        if (*segment_it == solution_num_surfaces &&
            visible_layer_indices[vertex_pixel] > layer_index) {
          has_empty_neighbor = true;
          break;
        }
      }
      if (selected_segment_index < solution_num_surfaces &&
          has_empty_neighbor) {
        set<int> selected_segment_pixels;
        set<int> empty_pixels;
        int window_size = 2;
        while (selected_segment_pixels.size() == 0 ||
               empty_pixels.size() == 0) {
          for (int delta_x = -window_size; delta_x <= window_size; delta_x++) {
            for (int delta_y = -window_size; delta_y <= window_size;
                 delta_y++) {
              int window_pixel =
                  min(max(static_cast<int>(round(y)) + delta_y, 0),
                      IMAGE_HEIGHT_ - 1) *
                      IMAGE_WIDTH_ +
                  min(max(static_cast<int>(round(x)) + delta_x, 0),
                      IMAGE_WIDTH_ - 1);
              if (layer_confident_pixel_mask[layer_index][window_pixel] ==
                  false)
                continue;
              // double distance = sqrt(pow(round(x) + delta_x - x, 2) +
              // pow(round(y) + delta_y - y, 2));
              int window_surface_id =
                  solution_labels[window_pixel] /
                  static_cast<int>(pow(solution_num_surfaces + 1,
                                       num_layers_ - 1 - layer_index)) %
                  (solution_num_surfaces + 1);
        ...
        if (window_size == 5) {
          ...
          for (set<int>::const_iterator selected_segment_pixel_it =
                   selected_segment_pixels.begin();
               selected_segment_pixel_it != selected_segment_pixels.end();
               selected_segment_pixel_it++) {
            double ori_selected_segment_pixel =
                min(static_cast<int>(round(
                        1.0 * (*selected_segment_pixel_it / IMAGE_WIDTH_) /
                        IMAGE_HEIGHT_ * ORI_IMAGE_HEIGHT)),
                    ORI_IMAGE_HEIGHT - 1) *
                    ORI_IMAGE_WIDTH +
                min(static_cast<int>(round(
                        1.0 * (*selected_segment_pixel_it % IMAGE_WIDTH_) /
                        IMAGE_WIDTH_ * ORI_IMAGE_WIDTH)),
                    ORI_IMAGE_WIDTH - 1);
            ori_window_pixels.push_back(ori_selected_segment_pixel);
          }
          for (set<int>::const_iterator empty_pixel_it = empty_pixels.begin();
               empty_pixel_it != empty_pixels.end(); empty_pixel_it++) {
            double ori_empty_pixel =
                min(static_cast<int>(round(1.0 *
                                           (*empty_pixel_it / IMAGE_WIDTH_) /
                                           IMAGE_HEIGHT_ * ORI_IMAGE_HEIGHT)),
                    ORI_IMAGE_HEIGHT - 1) *
                    ORI_IMAGE_WIDTH +
                min(static_cast<int>(round(1.0 *
          ...
          vector<double> distances; // = calcGeodesicDistances(distance_map,
                                    // ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
                                    // ori_pixel, ori_window_pixels,
                                    // DISTANCE_2D_WEIGHT);
          double min_distance = 1000000;
          int min_distance_index = -1;
          for (vector<double>::const_iterator distance_it = distances.begin();
               distance_it != distances.end(); distance_it++) {
        ...
        //     for (int delta_y = -window_size; delta_y <= window_size;
        //     delta_y++) {
        //       int window_pixel = min(max(static_cast<int>(round(y)) +
        //       delta_y, 0), IMAGE_HEIGHT_ - 1) * IMAGE_WIDTH_ +
        //       min(max(static_cast<int>(round(x)) + delta_x, 0), IMAGE_WIDTH_
        //       - 1);
        //       if (confident_pixel_mask[window_pixel] == false)
        //  continue;
        //       double distance = sqrt(pow(round(x) + delta_x - x, 2) +
        //       pow(round(y) + delta_y - y, 2));
        //       int window_surface_id = solution_labels[window_pixel] /
        //       static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1
        //       - layer_index)) % (solution_num_surfaces + 1);
        //       if (window_surface_id == selected_segment_index && distance <
        //       selected_segment_pixel_distance) {
        //  selected_segment_pixel = window_pixel;
        //  selected_segment_pixel_distance = distance;
        //       }
        //       if (window_surface_id == solution_num_surfaces && distance <
        //       empty_pixel_distance) {
        ...
        // double ori_selected_segment_pixel = min(static_cast<int>(round(1.0 *
        // (selected_segment_pixel / IMAGE_WIDTH_) / IMAGE_HEIGHT_ *
        // ORI_IMAGE_HEIGHT)), ORI_IMAGE_HEIGHT - 1) * ORI_IMAGE_WIDTH +
        // min(static_cast<int>(round(1.0 * (selected_segment_pixel %
        // IMAGE_WIDTH_) / IMAGE_WIDTH_ * ORI_IMAGE_WIDTH)), ORI_IMAGE_WIDTH -
        // 1);
        // double ori_empty_pixel = min(static_cast<int>(round(1.0 *
        // (empty_pixel / IMAGE_WIDTH_) / IMAGE_HEIGHT_ * ORI_IMAGE_HEIGHT)),
        // ORI_IMAGE_HEIGHT - 1) * ORI_IMAGE_WIDTH +
        // min(static_cast<int>(round(1.0 * (empty_pixel % IMAGE_WIDTH_) /
        // IMAGE_WIDTH_ * ORI_IMAGE_WIDTH)), ORI_IMAGE_WIDTH - 1);

        // //cout << ori_pixel << '\t' << ori_empty_pixel << '\t' <<
        // ori_selected_segment_pixel << '\t' <<
        // calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
        // ori_pixel, ori_empty_pixel, DISTANCE_2D_WEIGHT) << '\t' <<
        // calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
        // ori_pixel, ori_selected_segment_pixel, DISTANCE_2D_WEIGHT) << endl;
        // //exit(1);
        // if (calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH,
        // ORI_IMAGE_HEIGHT, ori_empty_pixel, ori_pixel, DISTANCE_2D_WEIGHT) <
        // calcGeodesicDistance(distance_map, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT,
        // ori_selected_segment_pixel, ori_pixel, DISTANCE_2D_WEIGHT))
        //   selected_segment_index = solution_num_surfaces;
      }
      // cout << selected_segment_index << endl;
      solution_labels_high_res[ori_pixel] +=
          selected_segment_index *
          pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index);
      // if (segment_indices.size() > 1 && find(segment_indices.begin(),
      // segment_indices.end(), solution_num_surfaces) < segment_indices.end())
  ...
  //   for (int target_layer_index = 0; target_layer_index < layer_index;
  //   target_layer_index++) {
  //     int surface_id = solution_label /
  //     static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
  //     target_layer_index)) % (solution_num_surfaces + 1);
  //     if (target_layer_index == layer_index)
  //  solution_label += (segment_id - surface_id) * pow(solution_num_surfaces
  // + 1, num_layers_ - 1 - target_layer_index);
  //     else
  //       solution_label += (solution_num_surfaces - surface_id) *
  //       pow(solution_num_surfaces + 1, num_layers_ - 1 - target_layer_index);
  //   }
  //   solution_labels_high_res[ori_pixel] = solution_label;
  // }

  writeLayers(ori_image_, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT, ori_point_cloud_,
              ori_camera_parameters, num_layers_, solution_labels_high_res,
              solution_num_surfaces, solution_segments, SCENE_INDEX_, 20000,
              ori_image_, ori_point_cloud_);

  upsampled_solution_labels = solution_labels_high_res;
  upsampled_solution_num_surfaces = solution_num_surfaces;

  //  SegmentationRefiner refiner(ori_image_, ori_point_cloud_,
  //  camera_parameters_, PENALTIES_, STATISTICS_, segments, segment_layer_map);
  ...
  //   int solution_label = solution_labels[pixel];
  //   int ori_segment_index = solution_label /
  //   static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
  //   refined_layer_index)) % (solution_num_surfaces + 1);

  //   if (new_segment_index != ori_segment_index) {
  //     int new_solution_label = solution_label;
  //     new_solution_label += (new_segment_index - ori_segment_index) *
  //     pow(solution_num_surfaces + 1, num_layers_ - 1 - refined_layer_index);
  //     for (int layer_index = 0; layer_index < refined_layer_index;
  //     layer_index++) {
  //  int surface_id = solution_label /
  // static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
  // layer_index)) % (solution_num_surfaces + 1);
  //       if (surface_id < solution_num_surfaces)
  //    new_solution_label += (solution_num_surfaces - surface_id) *
  // pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index);
  //     }
...
void LayerDepthRepresenter::refineSolution(
    const vector<int> &solution_labels, const int solution_num_surfaces,
    const map<int, Segment> &solution_segments,
    vector<int> &refined_solution_labels) {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
      ...
      if (surface_id < solution_num_surfaces) {
        ...
        if (is_visible == true) {
          segment_layer_visible_pixels[surface_id][layer_index].push_back(
              pixel);
          visible_layer_indices[pixel] = layer_index;
          is_visible = false;
        }
  ...
  map<int, map<int, vector<int>>> segment_layer_visible_contour_pixels;
  map<int, map<int, vector<int>>> segment_layer_invisible_contour_pixels;
  for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      int surface_id = solution_labels[pixel] /
                       static_cast<int>(pow(solution_num_surfaces + 1,
                                            num_layers_ - 1 - layer_index)) %
                       (solution_num_surfaces + 1);
      if (surface_id == solution_num_surfaces)
      ...
      for (vector<int>::const_iterator neighbor_pixel_it =
               neighbor_pixels.begin();
           neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
        int neighbor_surface_id =
            solution_labels[*neighbor_pixel_it] /
            static_cast<int>(
                pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index)) %
            (solution_num_surfaces + 1);
        ...
        if (visible_layer_indices[pixel] >= layer_index &&
            visible_layer_indices[*neighbor_pixel_it] >= layer_index) {
          segment_layer_visible_contour_pixels[surface_id][layer_index]
              .push_back(pixel);
          break;
        } else if (visible_layer_indices[pixel] < layer_index &&
                   neighbor_surface_id != solution_num_surfaces) {
          segment_layer_invisible_contour_pixels[surface_id][layer_index]
              .push_back(pixel);
          break;
        }
      }
  ...
  //   layer_it++) {
  //     Segment segment = solution_segments.at(segment_it->first);
  //     segment.refitSegmentKeepingGeometry(image_, point_cloud_, normals_,
  //     layer_it->second);
  //     segments[segment_index] = segment;
  //     segment_layer_map[segment_index] = layer_it->first;
  //     segment_index_map[segment_index] = segment_it->first;
  ...
  vector<vector<set<int>>> layer_pixel_segment_indices_map(
      num_layers_, vector<set<int>>(NUM_PIXELS_));
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    int solution_label = solution_labels[pixel];
    for (int layer_index = 1; layer_index < num_layers_ - 1; layer_index++) {
      int surface_id = solution_label /
                       static_cast<int>(pow(solution_num_surfaces + 1,
                                            num_layers_ - 1 - layer_index)) %
                       (solution_num_surfaces + 1);
  ...
  for (int layer_index = 1; layer_index < num_layers_ - 1; layer_index++) {
    vector<set<int>> pixel_segment_indices_map =
        layer_pixel_segment_indices_map[layer_index];
    while (true) {
      bool has_change = false;

      vector<set<int>> new_pixel_segment_indices_map =
          pixel_segment_indices_map;
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
        // if (pixel_segment_indices_map[pixel].size() == 0)
        //   continue;
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          int neighbor_surface_id =
              ...
              static_cast<int>(pow(solution_num_surfaces + 1,
                                   num_layers_ - 1 - layer_index)) %
              (solution_num_surfaces + 1);
          if (neighbor_surface_id != solution_num_surfaces)
            continue;
          for (set<int>::const_iterator segment_it =
                   pixel_segment_indices_map[pixel].begin();
               segment_it != pixel_segment_indices_map[pixel].end();
               segment_it++) {
            if (new_pixel_segment_indices_map[*neighbor_pixel_it].count(
                    *segment_it) > 0)
              continue;
            double segment_depth =
                solution_segments.at(*segment_it).getDepth(*neighbor_pixel_it);
            if (segment_depth < 0)
              continue;
            if (visible_layer_indices[*neighbor_pixel_it] < layer_index) {
              new_pixel_segment_indices_map[*neighbor_pixel_it].insert(
                  *segment_it);
              has_change = true;
            }
            ...
            //   cout << segment_depth << '\t' <<
            //   visible_depths[*neighbor_pixel_it] << endl;
            //   exit(1);
            // }
            // if (segment_depth >= visible_depths[*neighbor_pixel_it] &&
            // segment_depth <= background_depths[*neighbor_pixel_it]) {
  ...
  map<int, vector<vector<bool>>> segment_layer_outside_masks;
  for (map<int, Segment>::const_iterator segment_it = solution_segments.begin();
       segment_it != solution_segments.end(); segment_it++)
    segment_layer_outside_masks[segment_it->first] =
        vector<vector<bool>>(num_layers_, vector<bool>(NUM_PIXELS_, true));

  for (int layer_index = 1; layer_index < num_layers_ - 1; layer_index++)
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
      for (set<int>::const_iterator segment_it =
               layer_pixel_segment_indices_map[layer_index][pixel].begin();
           segment_it !=
           layer_pixel_segment_indices_map[layer_index][pixel].end();
           segment_it++)
        segment_layer_outside_masks[*segment_it][layer_index][pixel] = false;

  refined_solution_labels = solution_labels;
  // ContourCompleter contour_completer(IMAGE_WIDTH_, IMAGE_HEIGHT_);
  // for (map<int, map<int, vector<int> > >::const_iterator segment_it =
  // segment_layer_visible_pixels.begin(); segment_it !=
  // segment_layer_visible_pixels.end(); segment_it++) {
  //   // if (segment_it->first != 10)
  //   //   continue;
  //   for (map<int, vector<int> >::const_iterator layer_it =
  //   segment_it->second.begin(); layer_it != segment_it->second.end();
  //   layer_it++) {
  //     if (layer_it->first == 0 || layer_it->first == num_layers_ - 1)
  //  continue;
  //     cout << "complete segment " << segment_it->first << '\t' << " in layer
  //     " << layer_it->first << endl;
  //     vector<bool> segment_pixel_mask(NUM_PIXELS_, false);
  //     for (vector<int>::const_iterator pixel_it = layer_it->second.begin();
  //     pixel_it != layer_it->second.end(); pixel_it++)
  //       segment_pixel_mask[*pixel_it] = true;
  //     vector<int> invisible_contour_pixels =
  //     segment_layer_invisible_contour_pixels[segment_it->first][layer_it->first];
  //     for (vector<int>::const_iterator pixel_it =
  //     invisible_contour_pixels.begin(); pixel_it !=
  //     invisible_contour_pixels.end(); pixel_it++)
  //  segment_pixel_mask[*pixel_it] = true;
  //     vector<bool> outside_mask =
  //     segment_layer_outside_masks[segment_it->first][layer_it->first];
  //     vector<int> visible_contour_pixels =
  //     segment_layer_visible_contour_pixels[segment_it->first][layer_it->first];
  //     vector<bool> completed_segment_mask =
  //     contour_completer.completeContour(segment_pixel_mask, outside_mask,
  //     visible_contour_pixels);
  //     for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //  if (completed_segment_mask[pixel] == false)
  //    continue;
  //  int surface_id = solution_labels[pixel] /
  // static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 -
  // layer_it->first)) % (solution_num_surfaces + 1);
  //  if (surface_id == solution_num_surfaces || (surface_id !=
  // segment_it->first &&
  // solution_segments.at(segment_it->first).getDepth(pixel) <
  // solution_segments.at(surface_id).getDepth(pixel)))
  //    refined_solution_labels[pixel] += (segment_it->first - surface_id) *
  // pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_it->first);
  //     }
  //     vector<int> segment_pixels =
  //     segment_layer_pixels[segment_it->first][layer_it->first];
  //     for (vector<int>::const_iterator pixel_it = segment_pixels.begin();
  //     pixel_it != segment_pixels.end(); pixel_it++)
  //      if (completed_segment_mask[*pixel_it] == false)
  //        refined_solution_labels[*pixel_it] += (solution_num_surfaces -
  //     segment_it->first) * pow(solution_num_surfaces + 1, num_layers_ - 1 -
  //     layer_it->first);
  //   }
  // }
}
