...
LayerDepthRepresenter::LayerDepthRepresenter(const Mat &image, const vector<double> &point_cloud, const RepresenterPenalties &penalties, const DataStatistics &statistics, const int scene_index, const Mat &ori_image, const vector<double> &ori_point_cloud, const bool first_time, const int num_layers) : image_(image), point_cloud_(point_cloud), IMAGE_WIDTH_(image.cols), IMAGE_HEIGHT_(image.rows), NUM_PIXELS_(IMAGE_WIDTH_ * IMAGE_HEIGHT_), PENALTIES_(penalties), STATISTICS_(statistics), SCENE_INDEX_(scene_index), ori_image_(ori_image), ori_point_cloud_(ori_point_cloud), FIRST_TIME_(first_time), num_layers_(num_layers)
{
  ...
  if (true) {
    ...
    estimateCameraParameters(point_cloud_, IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_);
...
void LayerDepthRepresenter::optimizeLayerRepresentation()
{
  ...
  vector<shared_ptr<ParallelFusion::ProposalGenerator<LayerLabelSpace> > > generators((size_t)option.num_threads);
  vector<shared_ptr<ParallelFusion::FusionSolver<LayerLabelSpace> > > solvers((size_t)option.num_threads);
  ...
  for (auto i = 0; i < option.num_threads; ++i) {
    generators[i] = shared_ptr<ParallelFusion::ProposalGenerator<LayerLabelSpace> >(new ProposalDesigner(image_, point_cloud_, normals_, camera_parameters_, num_layers_, PENALTIES_, STATISTICS_, SCENE_INDEX_));
    solvers[i] = shared_ptr<ParallelFusion::FusionSolver<LayerLabelSpace> >(new TRWSFusion(image_, point_cloud_, normals_, PENALTIES_, STATISTICS_));
  ...
  ParallelFusion::ParallelFusionPipeline<LayerLabelSpace> parallelFusionPipeline(option);
  ...
  //   bool read_success = readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_, PENALTIES_, STATISTICS_, num_layers_, previous_solution_labels, previous_solution_num_surfaces, previous_solution_segments, SCENE_INDEX_, previous_energy_iteration) == true;
  ...
  //   // writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_, camera_parameters_, num_layers_, previous_solution_labels, previous_solution_num_surfaces, previous_solution_segments, SCENE_INDEX_, 10000, ori_image_);
  //   // exit(1);
  ...
  //   writeLayers(image_, IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_, camera_parameters_, num_layers_, current_solution_labels, proposal_num_surfaces, proposal_segments, SCENE_INDEX_, iteration, ori_image_, ori_point_cloud_);
  ...
  // bool read_success = readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_, PENALTIES_, STATISTICS_, num_layers_, solution_labels, solution_num_surfaces, solution_segments, SCENE_INDEX_, best_solution_iteration);
  ...
  if (false) {
    ...
    if (readLayers(IMAGE_WIDTH_, IMAGE_HEIGHT_, camera_parameters_, PENALTIES_, STATISTICS_, num_layers_, refined_solution_labels, refined_solution_num_surfaces, refined_solution_segments, SCENE_INDEX_, 10000) == false || true) {
...
void LayerDepthRepresenter::writeRenderingInfo(const vector<int> &solution_labels, const int solution_num_surfaces, const map<int, Segment> &solution_segments)
{
  ...
  for (int segment_id = 0; segment_id < new_solution_num_surfaces; segment_id++) {
	...
	for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++)
	  if (mask_image.at<uchar>(pixel / ori_image_.cols, pixel % ori_image_.cols) < 100)
	...
	for (int pixel = 0; pixel < ORI_IMAGE_WIDTH * ORI_IMAGE_HEIGHT; pixel++)
	  if (mask_image.at<uchar>(pixel / ori_image_.cols, pixel % ori_image_.cols) > 100 && mask_image.at<uchar>(pixel / ori_image_.cols, pixel % ori_image_.cols) < 200 && distance_map[pixel] > INPAINTING_REGION_WIDTH)
	    mask_image.at<uchar>(pixel / ori_image_.cols, pixel % ori_image_.cols) = 220;
      }
...
bool readLayers(const int image_width, const int image_height, const vector<double> &camera_parameters, const RepresenterPenalties &penalties, const DataStatistics &statistics, const int num_layers, vector<int> &solution, int &solution_num_surfaces, map<int, Segment> &solution_segments, const int scene_index, const int result_index)
{
  // if (FIRST_TIME_)
  //   return false;
...
void LayerDepthRepresenter::upsampleSolution(const vector<int> &solution_labels, const int solution_num_surfaces, const map<int, Segment> &solution_segments, vector<int> &upsampled_solution_labels, int &upsampled_solution_num_surfaces, map<int, Segment> &upsampled_solution_segments)
{
  ...
  estimateCameraParameters(ori_point_cloud_, ORI_IMAGE_WIDTH, ORI_IMAGE_HEIGHT, ori_camera_parameters);
...
void LayerDepthRepresenter::refineSolution(const vector<int> &solution_labels, const int solution_num_surfaces, const map<int, Segment> &solution_segments, vector<int> &refined_solution_labels)
{
  ...
  for (int layer_index = 0; layer_index < num_layers_; layer_index++) {
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      int surface_id = solution_labels[pixel] / static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index)) % (solution_num_surfaces + 1);
      if (surface_id == solution_num_surfaces)
	continue;
      vector<int> neighbor_pixels;
      int x = pixel % IMAGE_WIDTH_;
	...
	neighbor_pixels.push_back(pixel + 1 + IMAGE_WIDTH_);
      for (vector<int>::const_iterator neighbor_pixel_it = neighbor_pixels.begin(); neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
        int neighbor_surface_id = solution_labels[*neighbor_pixel_it] / static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_index)) % (solution_num_surfaces + 1);
	if (neighbor_surface_id == surface_id)
	  continue;
        if (visible_layer_indices[pixel] >= layer_index && visible_layer_indices[*neighbor_pixel_it] >= layer_index) {
	  segment_layer_visible_contour_pixels[surface_id][layer_index].push_back(pixel);
          break;
	} else if (visible_layer_indices[pixel] < layer_index && neighbor_surface_id != solution_num_surfaces) {
	  segment_layer_invisible_contour_pixels[surface_id][layer_index].push_back(pixel);
	  break;
	}
      }
    }
	  ...
	  if (neighbor_surface_id != solution_num_surfaces)
	    continue;
          for (set<int>::const_iterator segment_it = pixel_segment_indices_map[pixel].begin(); segment_it != pixel_segment_indices_map[pixel].end(); segment_it++) {
            if (new_pixel_segment_indices_map[*neighbor_pixel_it].count(*segment_it) > 0)
              continue;
            double segment_depth = solution_segments.at(*segment_it).getDepth(*neighbor_pixel_it);
            if (segment_depth < 0)
              continue;
	    if (visible_layer_indices[*neighbor_pixel_it] < layer_index) {
	      ...
	      has_change = true;
            ...
            //   exit(1);
            // }
            // if (segment_depth >= visible_depths[*neighbor_pixel_it] && segment_depth <= background_depths[*neighbor_pixel_it]) {
            //   new_pixel_segment_indices_map[*neighbor_pixel_it].insert(*segment_it);
            //   has_change = true;
            // }
          }
  ...
  for (map<int, Segment>::const_iterator segment_it = solution_segments.begin(); segment_it != solution_segments.end(); segment_it++)
    segment_layer_outside_masks[segment_it->first] = vector<vector<bool> >(num_layers_, vector<bool>(NUM_PIXELS_, true));
  ...
  // 	continue;
  //     cout << "complete segment " << segment_it->first << '\t' << " in layer " << layer_it->first << endl;
  //     vector<bool> segment_pixel_mask(NUM_PIXELS_, false);
  //     for (vector<int>::const_iterator pixel_it = layer_it->second.begin(); pixel_it != layer_it->second.end(); pixel_it++)
  //       segment_pixel_mask[*pixel_it] = true;
  //     vector<int> invisible_contour_pixels = segment_layer_invisible_contour_pixels[segment_it->first][layer_it->first];
  //     for (vector<int>::const_iterator pixel_it = invisible_contour_pixels.begin(); pixel_it != invisible_contour_pixels.end(); pixel_it++)
  // 	segment_pixel_mask[*pixel_it] = true;
  //     vector<bool> outside_mask = segment_layer_outside_masks[segment_it->first][layer_it->first];
  //     vector<int> visible_contour_pixels = segment_layer_visible_contour_pixels[segment_it->first][layer_it->first];
  //     vector<bool> completed_segment_mask = contour_completer.completeContour(segment_pixel_mask, outside_mask, visible_contour_pixels);
  //     for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  // 	if (completed_segment_mask[pixel] == false)
  // 	  continue;
  // 	int surface_id = solution_labels[pixel] / static_cast<int>(pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_it->first)) % (solution_num_surfaces + 1);
  // 	if (surface_id == solution_num_surfaces || (surface_id != segment_it->first && solution_segments.at(segment_it->first).getDepth(pixel) < solution_segments.at(surface_id).getDepth(pixel)))
  // 	  refined_solution_labels[pixel] += (segment_it->first - surface_id) * pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_it->first);
  //     }
  //     vector<int> segment_pixels = segment_layer_pixels[segment_it->first][layer_it->first];
  ...
  //     	  refined_solution_labels[*pixel_it] += (solution_num_surfaces - segment_it->first) * pow(solution_num_surfaces + 1, num_layers_ - 1 - layer_it->first);
  //   }
  // }
}
