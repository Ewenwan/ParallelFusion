...
// DEFINE_double(opposite_distance_threshold, 0.01, "The distance threshold
// determining whether a point is on one side of a surface or not");
// DEFINE_double(num_opposite_outliers_threshold_ratio, 0.01, "The ratio
// threshold determining whether a surface is one on side of a surface or not");
...
int main(int argc, char *argv[]) {
...

  {
    ...
    today.tm_mday = 9;
    today.tm_year = 116;
    LOG(INFO) << difftime(timer, mktime(&today)) << '\t' << -1 << '\t' << -1
              << '\t' << 0;
  }
  ...
  if (FLAGS_dataset_index == 0) {
    if (scene_type == 'R') {
      ...
      sprintf(point_cloud_filename, "Inputs/point_cloud_%02d.txt",
              FLAGS_scene_index);
      ...
      // if (segment_image) {
      //   // Mat disp_image = drawDispImage(point_cloud, image.cols,
      //   projection_matrix);
      //   // normalize(disp_image, disp_image, 0, 128, NORM_MINMAX);
      //   // imwrite("Results/disp_image.bmp", disp_image);

      //   //point_cloud = inpaintPointCloud(point_cloud, image.cols,
      //   image.rows);
      //   double focal_length = 520;
      //   //estimateCameraParameters(point_cloud, image.cols, image.rows,
      //   focal_length);
      //   ImageSegmenter image_segmenter(image, point_cloud, focal_length,
      //   scene_index);
      //   vector<int> segmentation = image_segmenter.getSegmentation();
      //   Mat segmentation_image = drawSegmentationImage(segmentation,
      //   image.cols);
      ...
      // sprintf(segmentation_filename, "Inputs/final_segmentation_%02d.txt",
      // scene_index);
      ...
      if (first_time) {
        ...
        vector<double> inpainted_point_cloud =
            point_cloud; // inpaintPointCloud(point_cloud, image.cols,
                         // image.rows);
        point_cloud = inpainted_point_cloud;

        // point_cloud = smoothPointCloud(point_cloud, segmentation, image.cols,
        // image.rows);
        if (true) {
          stringstream depth_values_filename;
          depth_values_filename << "Results/scene_" << FLAGS_scene_index << "/"
                                << "depth_values_ori";
          ...
          for (int point_index = 0;
               point_index < inpainted_point_cloud.size() / 3; point_index++)
            depth_values_out_str << inpainted_point_cloud[point_index * 3 + 2]
                                 << endl;
          depth_values_out_str.close();
        } else {
          // vector<double> triangle_vertices =
          // triangulateSceneUsingPointCloud(inpainted_point_cloud, image.cols,
          // image.rows, focal_length, cx, cy);
          // stringstream triangle_vertices_filename;
          // triangle_vertices_filename << "Results/scene_" << scene_index <<
          // "/" << "triangles_ori_";
          // triangle_vertices_filename.fill('0');
          // triangle_vertices_filename.width(3);
          // triangle_vertices_filename << scene_index;
          // ofstream
          // triangle_vertices_out_str(triangle_vertices_filename.str().c_str());
          // triangle_vertices_out_str << triangle_vertices.size() / 9 << endl;
          // for (int value_index = 0; value_index < triangle_vertices.size();
          // value_index++)
          //   triangle_vertices_out_str << triangle_vertices[value_index] <<
          //   endl;
      ...
      //      zoomScene(ori_image, ori_point_cloud, 1.0 * (image.cols *
      //      static_cast<int>(ori_image.cols / image.cols)) / ori_image.cols,
      //      1.0 * (image.cols * static_cast<int>(ori_image.cols / image.cols))
      //      / ori_image.cols);

      stringstream point_cloud_ply_filename;
      point_cloud_ply_filename << "Results/scene_" << FLAGS_scene_index
                               << "/point_cloud.ply";
      savePointCloudAsPly(point_cloud, point_cloud_ply_filename.str().c_str());

      stringstream point_cloud_mesh_filename;
      point_cloud_mesh_filename << "Results/scene_" << FLAGS_scene_index
                                << "/point_cloud_mesh.ply";
      savePointCloudAsMesh(point_cloud,
                           point_cloud_mesh_filename.str().c_str());

      projection_matrix = MatrixXd::Zero(3, 4);
      vector<double> camera_parameters =
          calcCameraParameters(point_cloud, image.cols);
      ...
      // char *point_cloud_filename = new char[100];
      // sprintf(point_cloud_filename, "GeneratedScene/point_cloud_%d",
      // scene_index);
      // point_cloud = loadPointCloud(point_cloud_filename);
      // point_cloud = normalizePointCloudByZ(point_cloud);
      // char *image_filename = new char[100];
      ...
      // ori_image = image.clone();
      // char *segmentation_filename = new char[100];
      // sprintf(segmentation_filename, "GeneratedScene/segmentation_%d",
      // scene_index);
      // segmentation = loadSegmentation(segmentation_filename);
      // imwrite("Test/segmentation_image.bmp",
      // drawSegmentationImage(segmentation, image.cols));
      // scene_index += 10000;
      //                    //
      // zoomScene(image, point_cloud, 5, 5);
      // stringstream disp_image_filename;
      // disp_image_filename << "Test/disp_image.bmp";
      // imwrite(disp_image_filename.str(), drawDispImage(point_cloud,
      // image.cols, image.rows)); //projection_matrix));
      exit(1);
    }

    stringstream zoomed_image_filename;
    zoomed_image_filename << "Results/scene_" << FLAGS_scene_index
                          << "/zoomed_image.bmp";
    ...
    //   //   test_image.at<Vec3b>(test_image.rows / 2, x) = Vec3b(255, 255,
    //   255);
    ...
    image_filename << FLAGS_scene_name << "_" << FLAGS_scene_rotation_angle
                   << ".png";
    image = imread(image_filename.str());
    ori_image = image.clone();

    stringstream point_cloud_filename;
    point_cloud_filename << FLAGS_scene_name << "_"
                         << FLAGS_scene_rotation_angle << ".obj";
    point_cloud = readPointCloudFromObj(point_cloud_filename.str(), image.cols,
                                        image.rows, FLAGS_scene_rotation_angle);
    // point_cloud = rotatePointCloud(point_cloud, rotation_angle);
    // point_cloud = readPointCloudFromObj("Inputs/cse013_0h.obj", image.cols,
    // image.rows);
    ori_point_cloud = point_cloud;

    //    zoomScene(image, point_cloud, 0.2, 0.2);
    zoomScene(image, point_cloud, 1.0 * 200 / image.cols,
              1.0 * 200 / image.cols);
    zoomScene(
        ori_image, ori_point_cloud,
        1.0 * (image.cols * static_cast<int>(ori_image.cols / image.cols)) /
            ori_image.cols,
        1.0 * (image.cols * static_cast<int>(ori_image.cols / image.cols)) /
            ori_image.cols);
    ...
    // cout << point_cloud[new_index * 3 + 0] << '\t' << point_cloud[new_index *
    // 3 + 1] << '\t' << point_cloud[new_index * 3 + 2] << endl;
    // int ori_index = 369 * ori_image.cols + 533;
    // cout << ori_point_cloud[ori_index * 3 + 0] << '\t' <<
    // ori_point_cloud[ori_index * 3 + 1] << '\t' << ori_point_cloud[ori_index *
    // 3 + 2] << endl;
    // exit(1);

    // cout << point_cloud[(116 * image.cols + 57) * 3 + 0] << '\t' <<
    // point_cloud[(116 * image.cols + 57) * 3 + 1] << '\t' << point_cloud[(116
    // * image.cols + 57) * 3 + 2] << endl;
    // exit(1);
    // double max_depth = 0;
    ...
    large_image_filename << "Results/scene_" << FLAGS_scene_index
                         << "/large_image.bmp";
  ...
  if (false) {
    ...
    for (auto thread_id = 0; thread_id < 4; ++thread_id) {
      ...
      if (video_writer.isOpened()) {
        ...
        imwrite("Test/solution_images/multi_way/solution_image_0_0.png",
                previous_image);
        // exit(1);
        ...
        for (int frame = 0; frame < max_time * FPS; frame++) {
          ...
          if (time_solution_map.count(time) > 0) {
            ...
            imwrite("Test/solution_images/multi_way/solution_image_" +
                        to_string(time) + "_" + to_string(thread_id) + ".png",
                    image);
            continue;
            string text =
    ...
    for (int y = 0; y < test_image.rows; y++) {
      for (int x = 0; x < test_image.cols; x++) {
        ...
        if (energy_1 != energy_2)
          cout << pixel % image.cols << '\t' << pixel / image.cols << '\t'
               << energy_1 << '\t' << energy_2 << endl;
        test_image.at<uchar>(y, x) =
            min(max(1.0 * (energy_1 - energy_2) / 100 * 128 + 128, 0.0), 255.0);
      }
    }
  ...
  if (true) {
    ...
    texture_ori_filename << "Results/scene_" << FLAGS_scene_index << "/"
                         << "texture_image_ori.bmp";
    ...
    if (!ifstream(depth_values_filename.str().c_str())) {
      vector<double> inpainted_point_cloud =
          point_cloud; // inpaintPointCloud(ori_point_cloud, ori_image.cols,
                       // ori_image.rows);
      ofstream depth_values_out_str(depth_values_filename.str().c_str());
      depth_values_out_str << ori_image.cols << '\t' << ori_image.rows << endl;
      for (int point_index = 0; point_index < inpainted_point_cloud.size() / 3;
           point_index++)
        depth_values_out_str << inpainted_point_cloud[point_index * 3 + 2]
                             << endl;
      depth_values_out_str.close();
    ...
    stringstream disp_image_filename;
    disp_image_filename << "Results/scene_" << FLAGS_scene_index
                        << "/disp_image.bmp";
    if (imread(disp_image_filename.str()).empty())
      imwrite(disp_image_filename.str(),
              drawDispImage(point_cloud, image.cols, image.rows));

    stringstream ori_disp_image_filename;
    ori_disp_image_filename << "Results/scene_" << FLAGS_scene_index
                            << "/disp_image_ori.bmp";
    if (imread(ori_disp_image_filename.str()).empty())
      imwrite(ori_disp_image_filename.str(),
  ...
  if (FLAGS_dataset_index == 0 && false) {
    ...
    statistics.depth_conflict_threshold =
        statistics.pixel_fitting_distance_threshold;
    statistics.depth_change_smoothness_threshold = 0.02;
    statistics.bspline_surface_num_pixels_threshold =
        image.cols * image.rows / 50;
  ...
  // BSplineSurface surface(point_cloud, pixels, image.cols, image.rows, 5, 5,
  // 3);
  // exit(1);

  // vector<double> normals = calcNormals(point_cloud, image.cols, image.rows,
  // 21);
  // Mat normal_image = Mat(image.rows, image.cols, CV_8UC3);
  // for (int y = 0; y < image.rows; y++) {
  //   for (int x = 0; x < image.cols; x++) {
  //     int pixel = y * image.cols + x;
  //     vector<double> normal(normals.begin() + pixel * 3, normals.begin() +
  //     (pixel + 1) * 3);
  //     Vec3b color;
  //     color[0] = max(min(static_cast<double>((normal[0] + 1) * 128), 255.0),
  //     0.0) + 0.5;
  //     color[1] = max(min(static_cast<double>((normal[1] + 1) * 128), 255.0),
  //     0.0) + 0.5;
  //     color[2] = max(min(static_cast<double>((normal[2] + 1) * 128), 255.0),
  //     0.0) + 0.5;
  ...
  if (test_image_completion) {
    ...
    //   Vec3b color = test_image.at<Vec3b>(pixel / test_image.cols, pixel %
    //   test_image.cols);
    //   if (color != Vec3b(255, 255, 255))
    //  segment_pixels.push_back(pixel);
    //   if (color == Vec3b(0, 0, 0)) {
    //     hole_pixels.push_back(pixel);
    //  //  test_image.at<Vec3b>(pixel / test_image.cols, pixel %
    // test_image.cols) = Vec3b(255, 255, 255);
    //   }
    // }
    ...
    //  mask_image.at<uchar>(pixel / test_image.cols, pixel % test_image.cols) =
    // 128;
    // }
  ...
  LayerDepthRepresenter representer(
      image, point_cloud, penalties, statistics, FLAGS_scene_index, ori_image,
      ori_point_cloud, first_time, FLAGS_num_layers, pipeline_params);
  ...
  // LinearProgrammingSolver solver = LinearProgrammingSolver(2, 3, c, A, blc,
  // buc, bkc, blx, bux, bkx);
  // solver.solve();
  // return 0;
}
