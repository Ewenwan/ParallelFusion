...
ProposalDesigner::ProposalDesigner(
    ...
    const vector<double> &normals, const vector<double> &camera_parameters,
    const int num_layers, const RepresenterPenalties penalties,
    const DataStatistics statistics, const int scene_index,
    const bool use_concave_hull_proposal_first)
    : image_(image), point_cloud_(point_cloud), normals_(normals),
      IMAGE_WIDTH_(image.cols), IMAGE_HEIGHT_(image.rows),
      CAMERA_PARAMETERS_(camera_parameters), penalties_(penalties),
      statistics_(statistics), NUM_PIXELS_(image.cols * image.rows),
      NUM_LAYERS_(num_layers), SCENE_INDEX_(scene_index),
      NUM_ALL_PROPOSAL_ITERATIONS_(3), proposal_iteration_(0),
      use_concave_hull_proposal_first_(use_concave_hull_proposal_first) {
  binary_proposal_designer_ =
      unique_ptr<BinaryProposalDesigner>(new BinaryProposalDesigner(
          image, point_cloud, normals, camera_parameters, num_layers, penalties,
          statistics, scene_index));
  // layer_inpainter_ = unique_ptr<LayerInpainter>(new LayerInpainter(image_,
  // segmentation_, surface_depths_, penalties_, false, true));
  // layer_estimator_ = unique_ptr<LayerEstimator>(new LayerEstimator(image_,
  // point_cloud_, segmentation_, surface_depths_, NUM_LAYERS_, penalties_,
  // surface_colors_, SCENE_INDEX_));
...
void ProposalDesigner::setCurrentSolution(
    const LayerLabelSpace &current_solution_label_space) {
  vector<int> current_solution_labels(NUM_PIXELS_);
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
    current_solution_labels[pixel] =
        current_solution_label_space.getLabelOfNode(pixel)[0];
  int current_solution_num_surfaces =
      current_solution_label_space.getNumSegments();
  map<int, Segment> current_solution_segments =
      current_solution_label_space.getSegments();
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label /
                       static_cast<int>(pow(current_solution_num_surfaces + 1,
                                            NUM_LAYERS_ - 1 - layer_index)) %
                       (current_solution_num_surfaces + 1);
  ...
  for (map<int, Segment>::const_iterator segment_it =
           current_solution_segments.begin();
       segment_it != current_solution_segments.end(); segment_it++) {
    if (surface_id_map.count(segment_it->first) > 0) {
      current_solution_segments_[surface_id_map[segment_it->first]] =
          segment_it->second;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    int current_solution_label = current_solution_labels[pixel];
    if (checkLabelValidity(pixel, current_solution_label,
                           current_solution_num_surfaces,
                           current_solution_segments) == false) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      ...
      new_label += surface_id_map[surface_id] *
                   pow(new_current_solution_num_surfaces + 1,
                       NUM_LAYERS_ - 1 - layer_index);
    }
  ...
  // for (map<int, int>::const_iterator segment_it = surface_id_map.begin();
  // segment_it != surface_id_map.end(); segment_it++)
  //   segment_layer_labels[segment_it->second] =
  //   current_solution_labels[NUM_PIXELS_ + segment_it->first];
  // new_current_solution_labels.insert(new_current_solution_labels.end(),
  // segment_layer_labels.begin(), segment_layer_labels.end());

  current_solution_labels_ = new_current_solution_labels;
  current_solution_num_surfaces_ = new_current_solution_num_surfaces;

  // writeDispImageFromSegments(current_solution_labels_,
  // current_solution_num_surfaces_, current_solution_segments_, NUM_LAYERS_,
  // IMAGE_WIDTH_, IMAGE_HEIGHT_, "Test/disp_image_0_new.bmp");
...
void ProposalDesigner::getProposals(LayerLabelSpace &proposal_label_space,
                                    const LayerLabelSpace &current_solution,
                                    const int N) {
  setCurrentSolution(current_solution);

  // if (iteration != 2) {
  //   cout << current_solution_num_surfaces_ << '\t' <<
  //   current_solution_segments_.size() << endl;
  //   exit(1);
  // }
  // bool test = false;
  // if (test) {
  ...
  if (RANDOM_PROPOSAL) {
    for (int label_space_index = 0; label_space_index < N;
         label_space_index++) {
      ...
      while (generate_success == false) {
        int proposal_type_index = rand() % (NUM_PROPOSAL_TYPES - 1 +
                                            current_solution_num_surfaces_ * 2);
        ...
        default:
          generate_success = generateSingleSurfaceExpansionProposal(
              (proposal_type_index - (NUM_PROPOSAL_TYPES - 1)) %
                  current_solution_num_surfaces_,
              (proposal_type_index - (NUM_PROPOSAL_TYPES - 1)) /
                  current_solution_num_surfaces_);
          break;
        }
  ...
  if (BINARY_PROPOSAL) {
    ...
    for (int label_space_index = 0; label_space_index < N;
      ...
      if (proposal_iteration_ == 0)
        generate_success =
            binary_proposal_designer_->getProposal(label_space, 0);
      else if (proposal_iteration_ == 1)
        generate_success =
            binary_proposal_designer_->getProposal(label_space, 1);
      while (generate_success == false) {
        int proposal_type_index = rand() % (NUM_PROPOSAL_TYPES - 1 +
                                            current_solution_num_surfaces_ * 2);
        generate_success = binary_proposal_designer_->getProposal(
            label_space, proposal_type_index);
      }
  ...
  if (proposal_type_index_ptr_ < 0 ||
      proposal_type_index_ptr_ >= proposal_type_indices_.size()) {
    std::random_shuffle(proposal_type_indices_.begin(),
                   proposal_type_indices_.end());
  ...
  //   if (generateSegmentationProposal(iteration % segmentations_.size()) ==
  //   false)
  //     return false;
  ...
  if (proposal_iteration_ == 0) {
    ...
    while (true) {
      ...
      if (single_surface_candidate_pixels_.size() > 0) {
        ...
        // proposal_type_index = min(proposal_type_index, NUM_PROPOSAL_TYPES -
        // 1);

        //  int proposal_type_index = rand() % NUM_PROPOSAL_TYPES;
        int proposal_type_index =
            proposal_type_indices_[proposal_type_index_ptr_];
        ...
        case 2:
          generate_success = generateSingleSurfaceExpansionProposal();
          break;
        // if (randomProbability() < 1.0 / NUM_PROPOSAL_TYPES / pow(1 - 1.0 /
        // NUM_PROPOSAL_TYPES, 3))
...
bool ProposalDesigner::getLastProposal(vector<vector<int>> &proposal_labels,
                                       int &proposal_num_surfaces,
                                       map<int, Segment> &proposal_segments,
                                       string &proposal_type) {
  NUM_LAYERS_++;
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
    current_solution_labels_[pixel] +=
        current_solution_num_surfaces_ *
        pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1);
...
void ProposalDesigner::addIndicatorVariables(
    const int num_indicator_variables) {
  // int num = num_indicator_variables == -1 ? pow(NUM_SURFACES_ + 1,
  // NUM_LAYERS_) : num_indicator_variables;
  int num = num_indicator_variables == -1 ? NUM_LAYERS_ * proposal_num_surfaces_
                                          : num_indicator_variables;
...
void ProposalDesigner::addSegmentLayerProposals(
    const bool restrict_segment_in_one_layer) {
  // int num = num_indicator_variables == -1 ? pow(NUM_SURFACES_ + 1,
  // NUM_LAYERS_) : num_indicator_variables;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (vector<int>::const_iterator label_it = pixel_proposals.begin();
         label_it != pixel_proposals.end(); label_it++) {
      for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
        int surface_id = *label_it /
                         static_cast<int>(pow(proposal_num_surfaces_ + 1,
                                              NUM_LAYERS_ - 1 - layer_index)) %
                         (proposal_num_surfaces_ + 1);
  ...
  for (map<int, vector<bool>>::const_iterator segment_it =
           segment_layer_mask_map.begin();
       segment_it != segment_layer_mask_map.end(); segment_it++) {
    ...
    if (restrict_segment_in_one_layer == true) {
      ...
      for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++)
        if (layer_mask[layer_index] == true)
          segment_layer_proposals[segment_it->first].push_back(
              pow(2, NUM_LAYERS_ - 1 - layer_index));
    } else {
      for (int proposal = 0; proposal < static_cast<int>(pow(2, NUM_LAYERS_));
           proposal++) {
        bool has_conflict = false;
        for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
          if (layer_mask[layer_index] == 0 &&
              proposal / static_cast<int>(
                             pow(2, NUM_LAYERS_ - 1 - layer_index) + 0.5) %
                      2 ==
                  1) {
            has_conflict = true;
            break;
  ...
  for (int segment_id = 0; segment_id < current_solution_num_surfaces_;
       segment_id++)
    if (find(segment_layer_proposals[segment_id].begin(),
             segment_layer_proposals[segment_id].end(),
             current_solution_labels_[NUM_PIXELS_ + segment_id]) ==
        segment_layer_proposals[segment_id].end())
      segment_layer_proposals[segment_id].push_back(
          current_solution_labels_[NUM_PIXELS_ + segment_id]);

  proposal_labels_.insert(proposal_labels_.end(),
                          segment_layer_proposals.begin(),
                          segment_layer_proposals.end());
}

bool ProposalDesigner::checkLabelValidity(const int pixel, const int label,
                                          const int num_surfaces,
                                          const map<int, Segment> &segments) {
  ...
  for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
    int surface_id =
        label /
        static_cast<int>(pow(num_surfaces + 1, NUM_LAYERS_ - 1 - layer_index)) %
        (num_surfaces + 1);
    ...
    // if (layer_index == NUM_LAYERS_ - 1 &&
    // ((estimated_background_surfaces.count(segment_id) > 0 && surface_id !=
    // segment_id) || estimated_background_surfaces.count(surface_id) == 0)) {
    //   background_inconsistency = true;
    //   break;
    // }
    // if (confident_segment_layer_map.count(surface_id) > 0 &&
    // confident_segment_layer_map[surface_id] != layer_index) {
    ...
    if (depth < previous_depth - statistics_.depth_conflict_threshold) {
      // if (pixel == 20803)
      //  cout << "depth conflict: " << depth << '\t' << previous_depth <<
      // endl;
  ...
  if (has_depth_conflict == false && has_same_label == false &&
      empty_background == false) // && background_inconsistency == false &&
                                 // has_layer_estimation_conflict == false &&
                                 // sub_region_extended == false &&
                                 // segmentation_inconsistency == false)
    return true;
  else
    return false;
}

void ProposalDesigner::writeSegmentationImage(const vector<int> &segmentation,
...
void ProposalDesigner::writeSegmentationImage(const Mat &segmentation_image,
                                              const string filename) {
  map<int, int> segment_center_x;
  map<int, int> segment_center_y;
  map<int, int> segment_pixel_counter;
...
bool ProposalDesigner::generateSegmentRefittingProposal() {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label /
                       static_cast<int>(pow(current_solution_num_surfaces_ + 1,
                                            NUM_LAYERS_ - 1 - layer_index)) %
                       (current_solution_num_surfaces_ + 1);
  ...
  // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
  //   distance_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel % IMAGE_WIDTH_) =
  //   current_solution_segments_[8].calcDistance2D(1.0 * (pixel % IMAGE_WIDTH_)
  //   / IMAGE_WIDTH_, 1.0 * (pixel / IMAGE_WIDTH_) / IMAGE_HEIGHT_);
  ...
  for (map<int, map<int, vector<int>>>::const_iterator segment_it =
           segment_layer_pixels.begin();
       segment_it != segment_layer_pixels.end(); segment_it++) {
    for (map<int, vector<int>>::const_iterator layer_it =
             segment_it->second.begin();
         layer_it != segment_it->second.end(); layer_it++)
      for (vector<int>::const_iterator pixel_it = layer_it->second.begin();
           pixel_it != layer_it->second.end(); pixel_it++)
        layer_pixel_segment_indices_map[layer_it->first][*pixel_it].insert(
            segment_it->first);
  ...
  for (map<int, map<int, vector<int>>>::const_iterator segment_it =
           segment_layer_visible_pixels.begin();
       segment_it != segment_layer_visible_pixels.end(); segment_it++) {
    vector<int> visible_pixels;

    for (map<int, vector<int>>::const_iterator layer_it =
             segment_it->second.begin();
         layer_it != segment_it->second.end(); layer_it++)
      visible_pixels.insert(visible_pixels.end(), layer_it->second.begin(),
                            layer_it->second.end());
    ...
    {
      Segment segment(image_, point_cloud_, normals_, CAMERA_PARAMETERS_,
                      visible_pixels, penalties_, statistics_);
      ...
      if (fitted_pixels.size() >= SMALL_SEGMENT_NUM_PIXELS_THRESHOLD &&
        ...
        for (map<int, vector<int>>::const_iterator layer_it =
                 segment_layer_pixels[segment_it->first].begin();
             layer_it != segment_layer_pixels[segment_it->first].end();
             layer_it++)
          for (vector<int>::const_iterator pixel_it = layer_it->second.begin();
               pixel_it != layer_it->second.end(); pixel_it++)
            layer_pixel_segment_indices_map[layer_it->first][*pixel_it].insert(
...

    {
      ...
      Segment segment(image_, point_cloud_, normals_, CAMERA_PARAMETERS_,
                      fitting_pixels, penalties_, statistics_);
      vector<int> fitted_pixels = segment.getSegmentPixels();
      if (fitted_pixels.size() >= SMALL_SEGMENT_NUM_PIXELS_THRESHOLD &&
          segment.getType() >= 0) {
        ...
        if (angle > statistics_.similar_angle_threshold) {
          for (vector<int>::const_iterator pixel_it = fitted_pixels.begin();
               pixel_it != fitted_pixels.end(); pixel_it++)
            layer_pixel_segment_indices_map[segment_it->second.begin()->first]
                                           [*pixel_it]
                                               .insert(
                                                   new_proposal_segment_index);
          // expansion_segments.insert(new_proposal_segment_index);
    ...
    if (background_segment_mask[segment_it->first] == false) {
      if (visible_pixels.size() <=
          statistics_.bspline_surface_num_pixels_threshold) {
        Segment segment(image_, point_cloud_, normals_, CAMERA_PARAMETERS_,
                        visible_pixels, penalties_, statistics_, 2);
        vector<int> fitted_pixels = segment.getSegmentPixels();
        if (fitted_pixels.size() >= SMALL_SEGMENT_NUM_PIXELS_THRESHOLD &&
            segment.getType() >= 0) {
          for (map<int, vector<int>>::const_iterator layer_it =
            ...
            for (vector<int>::const_iterator pixel_it =
                     layer_it->second.begin();
                 pixel_it != layer_it->second.end(); pixel_it++)
              layer_pixel_segment_indices_map[layer_it->first][*pixel_it]
                  .insert(new_proposal_segment_index);
          // if (segment_it->second.size() == 1)
          //   expansion_segments.insert(new_proposal_segment_index);
  ...
  //   } else {
  //     for (map<int, vector<int> >::const_iterator layer_it =
  //     segment_layer_pixels[segment_it->first].begin(); layer_it !=
  //     segment_layer_pixels[segment_it->first].end(); layer_it++) {
      ...
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          for (set<int>::const_iterator segment_it =
            ...
            //   continue;
            if (proposal_segments_[*segment_it].checkPixelFitting(
                    blurred_hsv_image_, point_cloud_, normals_,
                    *neighbor_pixel_it) &&
                dilated_pixel_segment_indices_map[*neighbor_pixel_it].count(
                    *segment_it) == 0) {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          for (set<int>::const_iterator segment_it =
            ...
            if (dilated_pixel_segment_indices_map[*neighbor_pixel_it].count(
                    *segment_it) > 0)
              continue;
            if (proposal_segments_[*segment_it].getDepth(*neighbor_pixel_it) >
                0)
              dilated_pixel_segment_indices_map[*neighbor_pixel_it].insert(
                  *segment_it);
          }
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    segment_index =
        layer_pixel_segment_indices_map[1][pixel].count(8) > 0 ? 1 : 0;
    ...
    new_segment_image.at<Vec3b>(pixel / IMAGE_WIDTH_, pixel % IMAGE_WIDTH_) =
        color_table[segment_index];
  }
  imwrite("Test/refitted_segment_image.bmp", new_segment_image);

  // for (map<int, Segment>::const_iterator segment_it =
  // proposal_segments_.begin(); segment_it != proposal_segments_.end();
  // segment_it++)
  //   cout << segment_it->first << '\t' << segment_it->second.getType() << '\t'
  //   << segment_it->second.getSegmentPixels().size() << endl;

  // vector<vector<set<int> > > pixel_layer_surface_ids_map(NUM_PIXELS_,
  // vector<set<int> >(NUM_LAYERS_));
  // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //   int current_solution_label = current_solution_labels_[pixel];
  //   for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
  //     for (set<int>::const_iterator segment_it =
  //     layer_surface_ids_vec[layer_index].begin(); segment_it !=
  //     layer_surface_ids_vec[layer_index].end(); segment_it++)
  //  if
  // (proposal_segments_.at(*segment_it).checkPixelFitting(blurred_hsv_image_,
  // point_cloud_, normals_, pixel))
  //    pixel_layer_surface_ids_map[pixel][layer_index].insert(*segment_it);
  //     int surface_id = current_solution_label /
  //     static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ -
  //     1 - layer_index)) % (current_solution_num_surfaces_ + 1);
  //     if (surface_id < current_solution_num_surfaces_)
  //  break;
  //   }
  // }
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    // vector<set<int> > layer_surface_ids_map =
    // pixel_layer_surface_ids_map[pixel];
    // vector<int> layer_segment_index_map =
    // pixel_layer_segment_index_map[pixel];
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label /
                       static_cast<int>(pow(current_solution_num_surfaces_ + 1,
                                            NUM_LAYERS_ - 1 - layer_index)) %
      ...
      if (surface_id < current_solution_num_surfaces_) {
        ...
        // else
        //   pixel_layer_surfaces_map[layer_index].insert(segment_new_segments_map[surface_id].begin(),
        //   segment_new_segments_map[surface_id].end());
      } else {
        pixel_layer_surfaces_map[layer_index].insert(proposal_num_surfaces_);
        // for (set<int>::const_iterator surface_it =
        // layer_surface_ids_map[layer_index].begin(); surface_it !=
        // layer_surface_ids_map[layer_index].end(); surface_it++)
        //   //if
        //   (proposal_segments_[*surface_it].checkPixelFitting(point_cloud_,
        //   normals_, pixel))
        //   pixel_layer_surfaces_map[layer_index].insert(*surface_it);
        // //
        // pixel_layer_surfaces_map[layer_index].insert(layer_surface_ids_map[layer_index].begin(),
        // layer_surface_ids_map[layer_index].end());
      }
    }

    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++)
      pixel_layer_surfaces_map[layer_index].insert(
          layer_pixel_segment_indices_map[layer_index][pixel].begin(),
          layer_pixel_segment_indices_map[layer_index][pixel].end());
    ...
    vector<int> pixel_proposals =
        calcPixelProposals(proposal_num_surfaces_, pixel_layer_surfaces_map);

    // if (pixel == 11084) {
    //   for (set<int>::const_iterator segment_it =
    //   segment_new_segments_map[1].begin(); segment_it !=
    //   segment_new_segments_map[1].end(); segment_it++)
    //  cout << *segment_it << endl;
    //   for (vector<int>::const_iterator label_it = pixel_proposals.begin();
    //   label_it != pixel_proposals.end(); label_it++) {
    //  for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
    //    int proposal_surface_id = *label_it /
    //    static_cast<int>(pow(proposal_num_surfaces_ + 1, NUM_LAYERS_ - 1 -
    //    layer_index)) % (proposal_num_surfaces_ + 1);
    ...
    for (vector<int>::const_iterator label_it = pixel_proposals.begin();
         label_it != pixel_proposals.end(); label_it++)
      if (checkLabelValidity(pixel, *label_it, proposal_num_surfaces_,
                             proposal_segments_) == true)
        valid_pixel_proposals.push_back(*label_it);

    if (valid_pixel_proposals.size() == 0) {
      cout << "empty proposal at pixel: " << pixel << endl;
      // for (int proposal_index = 0; proposal_index < pixel_proposal.size();
      // proposal_index++)
    ...
    if (current_solution_num_surfaces_ > 0) {
      current_solution_indices_[pixel] =
          find(valid_pixel_proposals.begin(), valid_pixel_proposals.end(),
               convertToProposalLabel(current_solution_label)) -
          valid_pixel_proposals.begin();
    ...
    //   cout << "max number of proposals: " << pixel % IMAGE_WIDTH_ << '\t' <<
    //   pixel / IMAGE_WIDTH_ << '\t' << valid_pixel_proposals.size() << endl;
...
//   //int new_proposal_segment_index = current_solution_num_surfaces_;
//   for (map<int, Segment>::const_iterator segment_it =
//   current_solution_segments_.begin(); segment_it !=
//   current_solution_segments_.end(); segment_it++) {
//     //    proposal_segments_[segment_it->first +
//     current_solution_num_surfaces_] = segment_it->second;
//     proposal_segments_[segment_it->first + current_solution_num_surfaces_] =
//     Segment(image_, point_cloud_, normals_, CAMERA_PARAMETERS_,
//     segment_visible_pixels[segment_it->first], penalties_, statistics_, 2);
//   }

//   //vector<vector<int> > pixel_layer_segment_index_map(NUM_PIXELS_,
//   vector<int>(NUM_LAYERS_, -1));
...
//   //     new_segment.fitBSplineSurface(image_, point_cloud_, normals_,
//   visible_pixels);
...
//   //     new_proposal_segment_index++;
//   //     //if (new_segment.buildSubSegment(image_, point_cloud_, normals_,
//   segment_visible_pixels[ *segment_it])) {
//   //     //  proposal_segments_[new_proposal_segment_index] = new_segment;
//   //     //
//   layer_surface_ids_vec[layer_index].insert(new_proposal_segment_index);
//   //     // }
...
//   // Mat new_segment_image = Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_,
//   CV_8UC3);
//   // map<int, Vec3b> color_table;
//   // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
//   //   for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
//   //     // int segment_index =
//   pixel_layer_segment_index_map[pixel][layer_index];
//   //     // if (segment_index == -1)
//   //     //   continue;
//   //     if (color_table.count(segment_index) == 0) {
//   //       Vec3b color;
...
//   //       color_table[segment_index] = color;
//   //     }
//   //     new_segment_image.at<Vec3b>(pixel / IMAGE_WIDTH_, pixel %
//   IMAGE_WIDTH_) = color_table[segment_index];
//   //   }
//   // }
//   // imwrite("Test/refitted_segment_image.bmp", new_segment_image);
...
//   layer_surface_ids_vec[layer_index].end(); segment_it++)
//   //       if
//   (proposal_segments_.at(*segment_it).checkPixelFitting(point_cloud_,
//   normals_, pixel))
//   // pixel_layer_surface_ids_map[pixel][layer_index].insert(*segment_it);
//   //     int surface_id = current_solution_label /
//   static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 -
//   layer_index)) % (current_solution_num_surfaces_ + 1);
//   //     if (surface_id < current_solution_num_surfaces_)
//   //       break;
//   //   }
//   // }
...
//         pixel_layer_surfaces_map[layer_index].insert(proposal_num_surfaces_);
//  for (set<int>::const_iterator surface_it =
// layer_surface_ids_vec[layer_index].begin(); surface_it !=
// layer_surface_ids_vec[layer_index].end(); surface_it++)
//    if
// (proposal_segments_[*surface_it].checkPixelFitting(blurred_hsv_image_,
// point_cloud_, normals_, pixel))
//      pixel_layer_surfaces_map[layer_index].insert(*surface_it);
//       }
//     }

//     vector<int> pixel_proposals = calcPixelProposals(proposal_num_surfaces_,
//     pixel_layer_surfaces_map);
...
//     if (current_solution_num_surfaces_ > 0) {
//       current_solution_indices_[pixel] = find(valid_pixel_proposals.begin(),
//       valid_pixel_proposals.end(),
//       convertToProposalLabel(current_solution_label)) -
//       valid_pixel_proposals.begin();
//       if (current_solution_indices_[pixel] == valid_pixel_proposals.size()) {
//         cout << "has no current solution label at pixel: " << pixel << endl;
...
//     segment_it->second;
//     proposal_segments_[segment_it->first +
//     current_solution_num_surfaces_].refitSegment(image_, point_cloud_,
//     segment_visible_pixels[segment_it->first]);
//   }
//   proposal_num_surfaces_ = proposal_segments_.size();
...
//       if (surface_id < current_solution_num_surfaces_) {
//  pixel_layer_surfaces_map[layer_index].insert(surface_id);
//  for (int target_layer_index = 0; target_layer_index < NUM_LAYERS_;
// target_layer_index++)
...
bool ProposalDesigner::generateSingleSurfaceExpansionProposal(
    const int denoted_expansion_segment_id, const int denoted_expansion_type) {
  ...
  if (current_solution_segments_.count(expansion_segment_id) == 0 ||
    ...
    int random_pixel = single_surface_candidate_pixels_
        [rand() % single_surface_candidate_pixels_.size()];
    int current_solution_label =
        current_solution_labels_[random_pixel % NUM_PIXELS_];
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label /
                       static_cast<int>(pow(current_solution_num_surfaces_ + 1,
                                            NUM_LAYERS_ - 1 - layer_index)) %
    ...
    // for (map<int, double>::const_iterator segment_it =
    // segment_confidence_map.begin(); segment_it !=
    // segment_confidence_map.end(); segment_it++) {
  ...
  //     return false;
  //   num_attempts++;
  //   expansion_segment_id = segment_id == -1 ? rand() %
  //   current_solution_num_surfaces_ : segment_id;

  //   if (current_solution_segments_.count(expansion_segment_id) == 0)
  //     continue;

  //   if (current_solution_segments_[expansion_segment_id].getConfidence() <
  //   0.5)
  //     continue;
  //   break;
  // }
  ...
  for (vector<int>::const_iterator pixel_it =
           single_surface_candidate_pixels_.begin();
       pixel_it != single_surface_candidate_pixels_.end(); pixel_it++)
    if (*pixel_it / NUM_PIXELS_ != expansion_type ||
        expansion_segment_visible_pixel_mask[*pixel_it % NUM_PIXELS_] == false)
  ...
  //  int expansion_segment_layer_index =
  //  expansion_segment_layer_counter.begin()->first;

  int expansion_segment_layer_index = -1;
  int max_layer_count = 0;
  for (map<int, int>::const_iterator layer_it =
           expansion_segment_layer_counter.begin();
       layer_it != expansion_segment_layer_counter.end(); layer_it++) {
  ...
  cout << "segment: " << expansion_segment_id
       << "\texpansion type: " << expansion_type << endl;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    if (expansion_type == 0) {
      ...
      pixel_layer_surfaces_map[expansion_segment_layer_index].insert(
          expansion_segment_id);
      int surface_id = current_solution_label /
                       static_cast<int>(pow(
                           current_solution_num_surfaces_ + 1,
                           NUM_LAYERS_ - 1 - expansion_segment_layer_index)) %
                       (current_solution_num_surfaces_ + 1);
      if (surface_id < current_solution_num_surfaces_ &&
          surface_id != expansion_segment_id)
        for (int target_layer_index = 0;
             target_layer_index < expansion_segment_layer_index;
             target_layer_index++)
          pixel_layer_surfaces_map[target_layer_index].insert(surface_id);
      for (int target_layer_index = 0;
           target_layer_index < expansion_segment_layer_index;
           target_layer_index++)
        pixel_layer_surfaces_map[target_layer_index].insert(
            proposal_num_surfaces_);
    }
    ...
    //   for (vector<int>::const_iterator label_it =
    //   valid_pixel_proposals.begin(); label_it != valid_pixel_proposals.end();
    //   label_it++) {
    //  for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
    //    int proposal_surface_id = *label_it /
    // static_cast<int>(pow(proposal_num_surfaces_ + 1, NUM_LAYERS_ - 1 -
...
bool ProposalDesigner::generateLayerSwapProposal() {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          for (set<int>::const_iterator segment_it =
            ...
            if (new_pixel_segment_indices_map[*neighbor_pixel_it].count(
                    *segment_it) > 0)
              continue;
            if (current_solution_segments_[*segment_it].getDepth(
                    *neighbor_pixel_it) > 0)
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_ - 1; layer_index++) {
      ...
      for (int target_layer_index = max(0, layer_index - 1);
           target_layer_index <= min(NUM_LAYERS_ - 1, layer_index + 1);
           target_layer_index++)
        pixel_layer_surfaces_map[target_layer_index].insert(segments.begin(),
                                                            segments.end());
    }
    ...
    //   cout << current_solution_segments_[12].getDepth(pixel) << '\t' <<
    //   current_solution_segments_[1].getDepth(pixel) << endl;
    //   for (vector<int>::const_iterator label_it =
    //   valid_pixel_proposals.begin(); label_it != valid_pixel_proposals.end();
    //   label_it++) {
    //     for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
  ...
  //     neighbor_pixel_it++) {
  //  int current_solution_neighbor_label =
  // current_solution_labels_[*neighbor_pixel_it];
  //  int neighbor_surface_id = current_solution_neighbor_label /
  // static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_  - 1 -
  // layer_index)) % (current_solution_num_surfaces_ + 1);
  //  if (neighbor_surface_id == surface_id)
  //    continue;
  //  int neighbor_proposal_label = current_solution_neighbor_label +
  // (surface_id - neighbor_surface_id) * pow(current_solution_num_surfaces_ +
  // 1, NUM_LAYERS_  - 1 - layer_index);
  //  vector<int> neighbor_pixel_proposals =
  // proposal_labels_[*neighbor_pixel_it];
  //  if (find(neighbor_pixel_proposals.begin(),
  // neighbor_pixel_proposals.end(), neighbor_proposal_label) ==
  // neighbor_pixel_proposals.end()) {
  //    if (checkLabelValidity(*neighbor_pixel_it, neighbor_proposal_label,
  // proposal_num_surfaces_, proposal_segments_) == true) {
...
bool ProposalDesigner::generateConcaveHullProposal(
    const bool consider_background) {
  ...
  for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
    ...
    if (layer_index == 0) { // || (consider_background == false && layer_index
                            // == NUM_LAYERS_ - 1)) {
      layer_pixel_inpainting_surface_ids[layer_index] =
          pixel_inpainting_surface_ids;
      continue;
    }
    ...
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      if (visited_pixel_mask[pixel] == true ||
          layer_surface_ids[pixel] == current_solution_num_surfaces_)
        continue;
      ...
      while (true) {
        ...
        for (vector<int>::const_iterator border_pixel_it =
                 border_pixels.begin();
             border_pixel_it != border_pixels.end(); border_pixel_it++) {
          ...
          for (vector<int>::const_iterator neighbor_pixel_it =
               ...
               neighbor_pixel_it++) {
            if (layer_surface_ids[*neighbor_pixel_it] !=
                    current_solution_num_surfaces_ &&
                visited_pixel_mask[*neighbor_pixel_it] == false) {
      ...
      unique_ptr<ConcaveHullFinder> concave_hull_finder(new ConcaveHullFinder(
          IMAGE_WIDTH_, IMAGE_HEIGHT_, point_cloud_, layer_surface_ids,
          current_solution_segments_, region_mask, penalties_, statistics_,
          consider_background));
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      int surface_id = current_solution_label /
                       ...
                       (current_solution_num_surfaces_ + 1);
      pixel_layer_surfaces_map[layer_index].insert(surface_id);
      if (layer_pixel_inpainting_surface_ids[layer_index][pixel] != -1 &&
          layer_pixel_inpainting_surface_ids[layer_index][pixel] !=
              surface_id) {
        pixel_layer_surfaces_map[layer_index].insert(
            layer_pixel_inpainting_surface_ids[layer_index][pixel]);
    ...
    for (int target_layer_index = 0; target_layer_index < NUM_LAYERS_ - 1;
         target_layer_index++)
      pixel_layer_surfaces_map[target_layer_index].insert(
          proposal_num_surfaces_);
    ...
    // if (pixel == 20803) {
    //   for (map<int, set<int> >::const_iterator layer_it =
    //   pixel_layer_surfaces_map.begin(); layer_it !=
    //   pixel_layer_surfaces_map.end(); layer_it++)
    //     for (set<int>::const_iterator segment_it = layer_it->second.begin();
    //     segment_it != layer_it->second.end(); segment_it++)
    ...
    //   cout << current_solution_label << '\t' << valid_pixel_proposals[0] <<
    //   '\t' << valid_pixel_proposals[1] << endl;
    //   exit(1);
    // }
    ...
    //  (current_solution_num_surfaces_ - surface_id) *
    //  pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - layer_index);
...
//       // if (surface_id == current_solution_num_surfaces_) {
//       //   for (int other_layer_index = 0; other_layer_index <
//       NUM_LAYERS_; other_layer_index++) {
//       //     if (other_layer_index == layer_index)
//       //       continue;
//       //     int other_surface_id = current_solution_label /
//       static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ -
//       1 - other_layer_index)) % (current_solution_num_surfaces_ + 1);
...
//  vector<int> new_border_pixels;
//  for (vector<int>::const_iterator border_pixel_it =
// border_pixels.begin(); border_pixel_it != border_pixels.end();
// border_pixel_it++) {
...
//       const int COMMON_BOUNDARY_LENGTH_THRESHOLD = boundary_length *
//       COMMON_BOUNDARY_LENGTH_THRESHOLD_RATIO + 0.5;
//       set<int> neighbor_segment_ids;
//       for (map<int, int>::const_iterator neighbor_segment_it =
//       neighbor_segment_counter.begin(); neighbor_segment_it !=
//       neighbor_segment_counter.end(); neighbor_segment_it++)
//  if (neighbor_segment_it->second > COMMON_BOUNDARY_LENGTH_THRESHOLD)
//    neighbor_segment_ids.insert(neighbor_segment_it->first);

//       for (vector<int>::const_iterator segment_pixel_it =
//       segment_pixels.begin(); segment_pixel_it != segment_pixels.end();
//       segment_pixel_it++)
//         for (set<int>::const_iterator neighbor_segment_it =
//         neighbor_segment_ids.begin(); neighbor_segment_it !=
//         neighbor_segment_ids.end(); neighbor_segment_it++)
...
//       segment_pixels.begin(); segment_pixel_it != segment_pixels.end();
//       segment_pixel_it++)
//       //     pixel_inpainting_surface_ids[*segment_pixel_it] =
//       current_solution_num_surfaces_;
//       // } else {
//       //   for (vector<int>::const_iterator segment_pixel_it =
//       segment_pixels.begin(); segment_pixel_it != segment_pixels.end();
//       segment_pixel_it++) {
//       //     double depth =
//       current_solution_segments_[segment_id].getDepth(pixel);
//       //     double min_depth = 1000000;
//       //     int min_depth_segment_id = -1;
//       //     for (set<int>::const_iterator neighbor_segment_it =
//       neighbor_segments.begin(); neighbor_segment_it !=
//       neighbor_segments.end(); neighbor_segment_it++) {
//       //       double neighbor_segment_depth =
//       current_solution_segments_[*neighbor_segment_it].getDepth(pixel);
//       //       if (neighbor_segment_depth > depth -
//       penalties_.depth_conflict_threshold && neighbor_segment_depth <
//       min_depth) {
...
//   // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
//   //   int current_solution_label = current_solution_labels_[pixel];
//   //   vector<bool> surface_fitting_mask(current_solution_num_surfaces_,
//   false);
//   //   for (int surface_id = 0; surface_id < current_solution_num_surfaces_;
//   surface_id++)
//   //     surface_fitting_mask[surface_id] =
//   current_solution_segments_[surface_id].checkPixelFitting(blurred_hsv_image_,
//   point_cloud_, normals_, pixel);
//   //   for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
//   //     for (set<int>::const_iterator surface_it =
//   layer_surface_ids_vec[layer_index].begin(); surface_it !=
...
//   //     for (int layer_index = NUM_LAYERS_ - 2; layer_index >= 0;
//   layer_index--) {
//   //       int current_solution_surface_id = current_solution_label /
//   static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_  - 1 -
//   layer_index)) % (current_solution_num_surfaces_ + 1);
...
//       // bool is_visible = true;
//       // for (int target_layer_index = 0; target_layer_index <= layer_index -
//       1; target_layer_index++) {
//       //   int target_layer_surface_id = current_solution_label /
//       static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_  -
//       1 - target_layer_index)) % (current_solution_num_surfaces_ + 1);
...
//       set<int> neighbor_segments =
//       layer_pixel_inpainting_surface_ids[layer_index][pixel];
//       pixel_layer_surfaces_map[layer_index].insert(neighbor_segments.begin(),
//       neighbor_segments.end());
//     }
//     for (int target_layer_index = 0; target_layer_index <= NUM_LAYERS_ - 1;
//     target_layer_index++)
...
//     // for (int layer_index = 0; layer_index < NUM_LAYERS_ - 1;
//     layer_index++) {
//     //   int surface_id = current_solution_label /
//     static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1
//     - layer_index)) % (current_solution_num_surfaces_ + 1);
...
//   // vector<int> correct_indicators(proposal_num_surfaces_ * NUM_LAYERS_ +
//   NUM_LAYERS_, 0);
//   // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
...
//   layer_index)) % (current_solution_num_surfaces_ + 1);
//   //     if (surface_id < current_solution_num_surfaces_) {
//   //       correct_indicators[current_solution_num_surfaces_ * layer_index +
//   surface_id] = 1;
//   //       correct_indicators[current_solution_num_surfaces_ * NUM_LAYERS_ +
//   layer_index] = 1;
//   //     }
//   //   }
//   // }
//   // for (vector<int>::const_iterator indicator_it =
//   correct_indicators.begin(); indicator_it != correct_indicators.end();
//   indicator_it++)
...
bool ProposalDesigner::generateSegmentAddingProposal(
    const int denoted_segment_adding_type) {
  ...
  if (segment_adding_type == -1)
    segment_adding_type =
        current_solution_num_surfaces_ == 0 ? 0 : rand() % 3 != 0 ? 1 : 1;
  segment_adding_type = current_solution_num_surfaces_ == 0 ? 0 : 1;

  // const double FITTING_DISTANCE_THRESHOLD =
  // statistics_.fitting_distance_threshold;
  // const int SEGMENT_NUM_PIXELS_THRESHOLD = IMAGE_WIDTH_ * IMAGE_HEIGHT_ /
  // 1000;

  vector<bool> bad_fitting_pixel_mask(NUM_PIXELS_, true);
  if (segment_adding_type != 0) {
    Mat bad_fitting_pixel_image =
        Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_, CV_8UC1);
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      int current_solution_label = current_solution_labels_[pixel];
      for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
        int surface_id =
            current_solution_label /
            static_cast<int>(pow(current_solution_num_surfaces_ + 1,
                                 NUM_LAYERS_ - 1 - layer_index)) %
            (current_solution_num_surfaces_ + 1);
        if (surface_id < current_solution_num_surfaces_) {
          if (current_solution_segments_.at(surface_id)
                      .checkPixelFitting(blurred_hsv_image_, point_cloud_,
                                         normals_, pixel) == false &&
              (point_cloud_[pixel * 3 + 2] <
                   current_solution_segments_.at(surface_id).getDepth(pixel) +
                       statistics_.depth_conflict_threshold ||
               layer_index == 0))
            bad_fitting_pixel_image.at<uchar>(pixel / IMAGE_WIDTH_,
                                              pixel % IMAGE_WIDTH_) = 255;
          break;
        }
      }
    }
    ...
    for (int iteration = 0; iteration < 1; iteration++) {
      erode(closed_bad_fitting_pixel_image, closed_bad_fitting_pixel_image,
            element);
      dilate(closed_bad_fitting_pixel_image, closed_bad_fitting_pixel_image,
             element);
      erode(closed_bad_fitting_pixel_image, closed_bad_fitting_pixel_image,
            element);
      dilate(closed_bad_fitting_pixel_image, closed_bad_fitting_pixel_image,
    ...
    imwrite("Test/bad_fitting_pixel_mask_image.bmp",
            closed_bad_fitting_pixel_image);
    // exit(1);
    bad_fitting_pixel_mask = vector<bool>(NUM_PIXELS_, false);
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
      if (closed_bad_fitting_pixel_image.at<uchar>(pixel / IMAGE_WIDTH_,
                                                   pixel % IMAGE_WIDTH_) > 128)
        bad_fitting_pixel_mask[pixel] = true;

    // drawMaskImage(bad_fitting_pixel_mask, IMAGE_WIDTH_, IMAGE_HEIGHT_,
    // "Test/bad_fitting_pixel_mask_image.bmp");
  }
  ...
  if (segment_adding_type == 1) {
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      int current_solution_label = current_solution_labels_[pixel];
      for (int layer_index = 0; layer_index < foremost_empty_layer_index + 1;
           layer_index++) {
        int current_solution_surface_id =
            current_solution_label /
            static_cast<int>(pow(current_solution_num_surfaces_ + 1,
                                 NUM_LAYERS_ - 1 - layer_index)) %
            (current_solution_num_surfaces_ + 1);
  ...
  //  const int NUM_NEW_SEGMENTS_THRESHOLD = segment_adding_type == 0 ? 20 :
  //  segment_adding_type == 1 ? 5 : 1;

  {
    ...
    // for (int proposal_segment_index = new_proposal_segment_start_index;
    // proposal_segment_index < new_proposal_segment_start_index +
    // NUM_NEW_SEGMENTS_THRESHOLD; proposal_segment_index++) {
    while (true) {
      vector<int> visible_pixels;
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
      ...
      Segment segment(image_, point_cloud_, normals_, CAMERA_PARAMETERS_,
                      visible_pixels, penalties_, statistics_);
      // vector<int> fitted_pixels = segment.getFittedPixels(point_cloud_,
      // visible_pixels);
      vector<int> fitted_pixels = segment.getSegmentPixels();
      if (fitted_pixels.size() < SMALL_SEGMENT_NUM_PIXELS_THRESHOLD)
        break;
...

  {
    if (segment_adding_type != 0) {
      ...
      int num_new_planes =
          proposal_segments_.size() - current_solution_num_surfaces_;
    ...
    while (true) {
      ...
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
             ...
             neighbor_pixel_it != neighbor_pixels.end(); neighbor_pixel_it++) {
          for (set<int>::const_iterator neighbor_segment_it =
                   pixel_segment_indices_map[*neighbor_pixel_it].begin();
               neighbor_segment_it !=
               pixel_segment_indices_map[*neighbor_pixel_it].end();
               neighbor_segment_it++) {
        ...
        if (new_pixel_segment_indices_map[pixel].size() >=
            NUM_PIXEL_SEGMENTS_THRESHOLD)
          unfitted_pixel_mask[pixel] = false;
        if (new_pixel_segment_indices_map[pixel].size() !=
            pixel_segment_indices_map[pixel].size())
          has_change = true;
      }
      pixel_segment_indices_map = new_pixel_segment_indices_map;
      if (has_change == false)
  ...
  vector<vector<set<int>>> layer_pixel_segment_indices_map(
      NUM_LAYERS_, pixel_segment_indices_map);
  {
    Mat new_segment_image = Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_, CV_8UC3);
    map<int, Vec3b> color_table;
    for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
      ...
      for (set<int>::const_iterator segment_it =
               pixel_segment_indices_map[pixel].begin();
           segment_it != pixel_segment_indices_map[pixel].end(); segment_it++)
        if (*segment_it >= current_solution_num_surfaces_)
          segment_index *= (*segment_it + 1);
      if (color_table.count(segment_index) == 0) {
        Vec3b color;
  ...
  //     while (true) {
  //       bool has_change = false;
  //       vector<set<int> > expanded_pixel_segment_indices_map =
  //       layer_pixel_segment_indices_map[layer_index];
  //       for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //         // if (pixel_segment_indices_map[pixel].size() == 0)
  //         //   continue;
  ...
  //           neighbor_pixels.push_back(pixel + 1 + IMAGE_WIDTH_);
  //         for (vector<int>::const_iterator neighbor_pixel_it =
  //         neighbor_pixels.begin(); neighbor_pixel_it !=
  //         neighbor_pixels.end(); neighbor_pixel_it++) {
  //      int current_solution_surface_id =
  // current_solution_labels_[*neighbor_pixel_it] /
  // static_cast<int>(pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_  - 1 -
  // layer_index)) % (current_solution_num_surfaces_ + 1);
  //      if (current_solution_surface_id == current_solution_num_surfaces_)
  //        continue;
  //      double current_solution_depth =
  // current_solution_segments_.at(current_solution_surface_id).getDepth(*neighbor_pixel_it);
  //      for (set<int>::const_iterator segment_it =
  // expanded_pixel_segment_indices_map[pixel].begin(); segment_it !=
  // expanded_pixel_segment_indices_map[pixel].end(); segment_it++) {
  //        double depth =
  // proposal_segments_[*segment_it].getDepth(*neighbor_pixel_it);
  //             if
  //             (expanded_pixel_segment_indices_map[*neighbor_pixel_it].count(*segment_it)
  //             == 0 && depth < current_solution_depth +
  //             statistics_.depth_conflict_threshold && depth >
  //             visible_depths[*neighbor_pixel_it] -
  //             statistics_.depth_conflict_threshold) {
  ...
  //         break;
  //       layer_pixel_segment_indices_map[layer_index] =
  //       expanded_pixel_segment_indices_map;
  //     }
  //   }
  // }
  ...
  //   map<int, Vec3b> color_table;
  //   for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //     if (layer_pixel_segment_indices_map[NUM_LAYERS_ - 1][pixel].size() ==
  //     0)
  //  continue;
  //     int segment_index = *layer_pixel_segment_indices_map[NUM_LAYERS_ -
  //     1][pixel].begin();
  //     if (color_table.count(segment_index) == 0) {
  //  Vec3b color;
  //  for (int c = 0; c < 3; c++)
  ...
  // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
  //   for (set<int>::const_iterator segment_it =
  //   pixel_segment_indices_map[pixel].begin(); segment_it !=
  //   pixel_segment_indices_map[pixel].end(); segment_it++)
  //     assert(proposal_segments_[*segment_it].getDepth(pixel) > 0);
  // cout << "done" << endl;

  cout << "number of new segments: "
       << proposal_segments_.size() - current_solution_num_surfaces_ << endl;
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    pixel_layer_surfaces_map[max(foremost_empty_layer_index, 0)].insert(
        pixel_segment_indices_map[pixel].begin(),
        pixel_segment_indices_map[pixel].end());
    if (segment_adding_type == 0 &&
        pixel_segment_indices_map[pixel].size() == 0)
      for (int new_segment_index = current_solution_num_surfaces_;
           new_segment_index < proposal_num_surfaces_; new_segment_index++)
        pixel_layer_surfaces_map[max(foremost_empty_layer_index, 0)].insert(
            new_segment_index);

    // } else if (segment_adding_type == 1) {
    //   pixel_layer_surfaces_map[max(foremost_empty_layer_index,
    //   0)].insert(pixel_segment_indices_map[pixel].begin(),
    //   pixel_segment_indices_map[pixel].end());
    // } else {
    //   for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++)
    //  pixel_layer_surfaces_map[layer_index].insert(layer_pixel_segment_indices_map[layer_index][pixel].begin(),
    // layer_pixel_segment_indices_map[layer_index][pixel].end());
    // }

    // if (pixel_segment_index_map[pixel] != -1)
    //   //      for (int target_layer_index = 0; target_layer_index <
    //   NUM_LAYERS_; target_layer_index++)
    //   pixel_layer_surfaces_map[max(foremost_empty_layer_index,
    //   0)].insert(pixel_segment_index_map[pixel]);
    // else
    //   for (int new_segment_index = current_solution_num_surfaces_;
    //   new_segment_index < proposal_num_surfaces_; new_segment_index++)
    //  if
    // (proposal_segments_[new_segment_index].checkPixelFitting(blurred_hsv_image_,
    // point_cloud_, normals_, pixel) || bad_fitting_mask[pixel] == true)
    //       pixel_layer_surfaces_map[max(foremost_empty_layer_index,
    //       0)].insert(new_segment_index);
    ...
    //   //   for (int layer_index = 0; layer_index < NUM_LAYERS_;
    //   layer_index++) {
    //   //   int surface_id = proposal_label /
    //   static_cast<int>(pow(proposal_num_surfaces_ + 1, NUM_LAYERS_  - 1 -
    //   layer_index)) % (proposal_num_surfaces_ + 1);
    //   //   pixel_proposals.push_back(proposal_label + (new_segment_index
    //   - surface_id) * pow(proposal_num_surfaces_ + 1, NUM_LAYERS_ - 1 -
    //   layer_index));
    //   //   if (surface_id < proposal_num_surfaces_)
    //   //     break;
    //   //   }
    ...
    // //   int foremost_layer_surface_id = proposal_label /
    // static_cast<int>(pow(proposal_num_surfaces_ + 1, NUM_LAYERS_  - 1 -
    // max(foremost_empty_layer_index, 0))) % (proposal_num_surfaces_ + 1);
    // //   for (int new_segment_index = current_solution_num_surfaces_;
    // new_segment_index < proposal_num_surfaces_; new_segment_index++) {
    // //   if (pixel_segment_index_map[pixel] == -1 || new_segment_index ==
    // pixel_segment_index_map[pixel]) {
    // //     int new_proposal_label = proposal_label + (new_segment_index -
    // foremost_layer_surface_id) * pow(proposal_num_surfaces_ + 1, NUM_LAYERS_
    // - 1 - max(foremost_empty_layer_index, 0));
    // //     if (checkLabelValidity(pixel, new_proposal_label,
    // proposal_num_surfaces_, proposal_segments_))
    // //         pixel_proposals.push_back(new_proposal_label);
    // //   }
    // //   }

    // //   if (foremost_empty_layer_index == -1 && foremost_layer_surface_id !=
    // proposal_num_surfaces_) {
    // //   int new_proposal_label = proposal_label + (proposal_num_surfaces_ -
    // foremost_layer_surface_id) * pow(proposal_num_surfaces_ + 1, NUM_LAYERS_
    // - 1 - 0);
    // //     if (checkLabelValidity(pixel, new_proposal_label,
    // proposal_num_surfaces_, proposal_segments_))
    ...
    // //   if (pixel_segment_index_map[pixel] == -1 || new_segment_index ==
    // pixel_segment_index_map[pixel])
    // //     pixel_proposals.push_back(pow(proposal_num_surfaces_ + 1,
    // NUM_LAYERS_) - 1 - proposal_num_surfaces_ + new_segment_index);
    // // }
    ...
    //   cout << "max number of proposals: " << pixel % IMAGE_WIDTH_ << '\t' <<
    //   pixel / IMAGE_WIDTH_ << '\t' << pixel_proposals.size() << endl;
...
bool ProposalDesigner::generateStructureExpansionProposal(
    const int denoted_expansion_layer_index,
    const int denoted_expansion_pixel) {
  ...
  vector<int> segment_backmost_layer_index_map(current_solution_num_surfaces_,
                                               0);
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      ...
      if (surface_id < current_solution_num_surfaces_) {
        ...
        segment_backmost_layer_index_map[surface_id] =
            max(segment_backmost_layer_index_map[surface_id], layer_index);
      }
    }
  }
  for (map<int, Segment>::const_iterator segment_it =
           current_solution_segments_.begin();
       segment_it != current_solution_segments_.end(); segment_it++) {
    if (segment_it->second.getType() != 0)
      candidate_segment_mask[segment_it->first] = false;
  }

  unique_ptr<StructureFinder> structure_finder(new StructureFinder(
      IMAGE_WIDTH_, IMAGE_HEIGHT_, current_solution_segments_,
      candidate_segment_mask, visible_segmentation, visible_depths,
      background_depths, segment_backmost_layer_index_map, penalties_,
      statistics_));
  vector<pair<double, vector<int>>> structure_score_surface_ids_pairs =
      structure_finder->getStructures();
  if (structure_score_surface_ids_pairs.size() == 0)
    return false;

  double score_sum = 0;
  for (int pair_index = 0;
       pair_index < structure_score_surface_ids_pairs.size(); pair_index++)
  ...
  for (int pair_index = 0;
    ...
    if (score_sum >= selected_score) {
      ...
      structure_surface_ids =
          structure_score_surface_ids_pairs[pair_index].second;
      break;
    }
  }
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    if (surface_id == -1)
      continue;
    backmost_layer_index =
        max(backmost_layer_index, segment_backmost_layer_index_map[surface_id]);
  }
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      ...
      if (surface_id < proposal_num_surfaces_ &&
          layer_index <= backmost_layer_index)
        for (int target_layer_index = 0; target_layer_index < layer_index;
             target_layer_index++)
          pixel_layer_surfaces_map[target_layer_index].insert(surface_id);
    }
    if (structure_surface_ids[pixel] != -1)
      pixel_layer_surfaces_map[backmost_layer_index].insert(
          structure_surface_ids[pixel]);

    for (int target_layer_index = 0; target_layer_index < backmost_layer_index;
         target_layer_index++)
      pixel_layer_surfaces_map[target_layer_index].insert(
          proposal_num_surfaces_);
...
bool ProposalDesigner::generateBackwardMergingProposal(
    const int denoted_target_layer_index) {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    for (int layer_index = 0; layer_index < target_layer_index; layer_index++) {
      int surface_id = current_solution_label /
                       ...
                       (current_solution_num_surfaces_ + 1);
      if (surface_id < current_solution_num_surfaces_ &&
          current_solution_segments_[surface_id].getType() == 0)
      ...
      for (vector<int>::const_iterator neighbor_pixel_it =
        ...
        for (set<int>::const_iterator segment_it =
          ...
          double segment_depth =
              current_solution_segments_[*segment_it].getDepth(
                  *neighbor_pixel_it);
          if (segment_depth < 0)
            continue;
          ...
          if ((segment_depth > visible_depths[*neighbor_pixel_it] -
               ...
               segment_depth < background_depths[*neighbor_pixel_it] +
                                   statistics_.depth_conflict_threshold) ||
              current_solution_segments_[*segment_it].checkPixelFitting(
                  blurred_hsv_image_, point_cloud_, normals_,
                  *neighbor_pixel_it)) {
            new_pixel_segment_indices_map[*neighbor_pixel_it].insert(
                *segment_it);
  ...
  //  cout << pixel_segment_indices_map[139 * IMAGE_WIDTH_ + 126].size() <<
  //  endl;
  Mat new_segment_image = Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_, CV_8UC3);
  map<int, Vec3b> color_table;
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
...
bool ProposalDesigner::generateBoundaryRefinementProposal() {
      ...
      for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
        ...
        for (vector<int>::const_iterator neighbor_pixel_it =
          ...
          for (set<int>::const_iterator segment_it =
            ...
            //   if (segment_depth > visible_depths[*neighbor_pixel_it] -
            //   statistics_.depth_conflict_threshold ||
            //   current_solution_segments_[*segment_it].checkPixelFitting(blurred_hsv_image_,
            //   point_cloud_, normals_, *neighbor_pixel_it)) {
            //  new_pixel_segment_indices_map[*neighbor_pixel_it].insert(*segment_it);
            //  has_change = true;
            //   }
            // } else {
            //   if (segment_depth > visible_depths[*neighbor_pixel_it] -
            //   statistics_.depth_conflict_threshold && segment_depth <
            //   background_depths[*neighbor_pixel_it]) {
            //     new_pixel_segment_indices_map[*neighbor_pixel_it].insert(*segment_it);
            //     has_change = true;
            //   }
            // }
  ...
  // Mat boundary_refinement_image = Mat::zeros(IMAGE_HEIGHT_, IMAGE_WIDTH_,
  // CV_8UC1);
  // map<int, Vec3b> color_table;
  // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++)
  //   if (layer_pixel_segment_indices_map[2][pixel].count(3) > 0)
  //     boundary_refinement_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
  //     IMAGE_WIDTH_) = 255;
...
//       neighbor_pixel_it++) {
//  int neighbor_segment_id = layer_surface_ids[*neighbor_pixel_it];
//  if (neighbor_segment_id == current_solution_num_surfaces_ ||
// neighbor_segment_id == segment_id)
//    continue;
...
// current_solution_segments_[segment_id].getDepth(pixel);
//  double depth_1_2 =
// current_solution_segments_[segment_id].getDepth(*neighbor_pixel_it);
//  double depth_2_1 =
// current_solution_segments_[neighbor_segment_id].getDepth(pixel);
//  double depth_2_2 =
// current_solution_segments_[neighbor_segment_id].getDepth(*neighbor_pixel_it);
//  if (depth_1_1 <= 0 || depth_1_2 <= 0 || depth_2_1 <= 0 || depth_2_2 <=
// 0)
//    continue;
...
//   vector<set<int> > layer_surface_ids_vec(NUM_LAYERS_);
//   vector<vector<int> >
//   segment_visible_pixels_vec(current_solution_num_surfaces_);
...
//     segment_completed_mask_map[segment_it->first] =
//     contour_completer.completeContour(segment_mask, vector<bool>(NUM_PIXELS_,
//     false));
//   }

//   for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
//     for (set<int>::const_iterator surface_it_1 =
//     layer_surface_ids_vec[layer_index].begin(); surface_it_1 !=
//     layer_surface_ids_vec[layer_index].end(); surface_it_1++) {
//       if (segment_completed_mask_map.count(*surface_it_1) == 0)
//         continue;
//       for (set<int>::const_iterator surface_it_2 =
//       layer_surface_ids_vec[layer_index].begin(); surface_it_2 !=
//       layer_surface_ids_vec[layer_index].end(); surface_it_2++) {
...
//  if (segment_boundary_pixel_counter.count(*surface_it_1) > 0 &&
// segment_boundary_pixel_counter[*surface_it_1].count(*surface_it_2) > 0)
//    continue;
//  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
//    if (segment_mask_1[pixel] == false || segment_mask_2[pixel] == false)
//      continue;
//    double depth_1 =
// current_solution_segments_[*surface_it_1].getDepth(pixel);
//    double depth_2 =
// current_solution_segments_[*surface_it_2].getDepth(pixel);
...
//   vector<vector<set<int> > > layer_pixel_segment_indices_map(NUM_LAYERS_,
//   vector<set<int> >(NUM_PIXELS_));
//   for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
//     int current_solution_label = current_solution_labels_[pixel];
//     bool is_visible = true;
//     for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
...
// segment_layer_votes[surface_id][target_layer_index] > 0)
...
// target_layer_index++)
//             if (target_layer_index == layer_index ||
//             segment_layer_votes[*surface_it][target_layer_index] > 0)
...
bool ProposalDesigner::generateDesiredProposal() {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
    ...
    int layer_0_surface_id =
        current_solution_label /
        static_cast<int>(
            pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 0)) %
        (current_solution_num_surfaces_ + 1);
    int layer_1_surface_id =
        current_solution_label /
        static_cast<int>(
            pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 1)) %
        (current_solution_num_surfaces_ + 1);
    int layer_2_surface_id =
        current_solution_label /
        static_cast<int>(
            pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 2)) %
        (current_solution_num_surfaces_ + 1);

    int proposal_label = current_solution_label;
    // if (layer_1_surface_id == 3) {
    //   if (current_solution_segments_[5].getDepth(pixel) <
    //   current_solution_segments_[4].getDepth(pixel))
    //   proposal_label -= 1;
    //   //visible_pixels.push_back(pixel);
    // }
    ...
    if (layer_1_surface_id == 5 && layer_0_surface_id == 6)
      proposal_label +=
          (current_solution_num_surfaces_ - layer_1_surface_id) *
          pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 1);

    // if (current_solution_segments_[1].getDepth(pixel) <
    // current_solution_segments_[layer_2_surface_id].getDepth(pixel))
    //   proposal_label += (1 - layer_2_surface_id) *
    //   pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 2);

    // if (layer_0_surface_id == 4 || layer_0_surface_id == 7) {
    //   proposal_label += (current_solution_num_surfaces_ - layer_0_surface_id)
    //   * pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 0) +
    //   (layer_0_surface_id - current_solution_num_surfaces_) *
    //   pow(current_solution_num_surfaces_ + 1, NUM_LAYERS_ - 1 - 1);
    // }
...
bool ProposalDesigner::generateSingleProposal() {
  ...
  // for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  //   color_likelihood_image.at<uchar>(pixel / IMAGE_WIDTH_, pixel %
  //   IMAGE_WIDTH_) =
  //   min(max(proposal_segments_[7].predictColorLikelihood(pixel,
  //   blurred_hsv_image_.at<Vec3f>(pixel / IMAGE_WIDTH_, pixel % IMAGE_WIDTH_))
  //   + 255, 0.0), 255.0);
  //   double color_likelihood =
  //   proposal_segments_[7].predictColorLikelihood(pixel,
  //   blurred_hsv_image_.at<Vec3f>(pixel / IMAGE_WIDTH_, pixel %
  //   IMAGE_WIDTH_));
...
//  visible_pixels.push_back(pixel);
//     proposal_segments_[segment_id] = Segment(image_, point_cloud_,
//     visible_pixels, vector<int>(visible_pixels.size(), -1), penalties_,
//     statistics_);
//     vector<int> fitted_pixels =
//     proposal_segments_[segment_id].getFittedPixels(point_cloud_,
//     visible_pixels);
//     for (vector<int>::const_iterator pixel_it = fitted_pixels.begin();
//     pixel_it != fitted_pixels.end(); pixel_it++)
...
//     for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
//       for (int segment_id = 0; segment_id < proposal_num_surfaces_;
//       segment_id++)
...
vector<int> ProposalDesigner::calcPixelProposals(
    const int num_surfaces,
    const map<int, set<int>> &pixel_layer_surfaces_map) {
  vector<int> pixel_proposals(1, 0);
  for (map<int, set<int>>::const_iterator layer_it =
           pixel_layer_surfaces_map.begin();
       layer_it != pixel_layer_surfaces_map.end(); layer_it++) {
    vector<int> new_pixel_proposals;
    for (set<int>::const_iterator segment_it = layer_it->second.begin();
         segment_it != layer_it->second.end(); segment_it++)
      for (vector<int>::const_iterator label_it = pixel_proposals.begin();
           label_it != pixel_proposals.end(); label_it++)
        new_pixel_proposals.push_back(
            *label_it +
            *segment_it *
                pow(num_surfaces + 1, NUM_LAYERS_ - 1 - layer_it->first));
...
int ProposalDesigner::convertToProposalLabel(const int current_solution_label) {
  ...
  for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
    ...
    if (surface_id == current_solution_num_surfaces_)
      proposal_label +=
          proposal_num_surfaces_ *
          pow(proposal_num_surfaces_ + 1, NUM_LAYERS_ - 1 - layer_index);
    else
      proposal_label += (surface_id)*pow(proposal_num_surfaces_ + 1,
                                         NUM_LAYERS_ - 1 - layer_index);
  }
...
// vector<int> calcPixelProposals(const int num_layers, const int num_surfaces,
// const map<int, set<int> > &pixel_layer_surfaces_map)
// {
//   vector<int> pixel_proposals(1, 0);
//   for (map<int, set<int> >::const_iterator layer_it =
...
//       for (vector<int>::const_iterator label_it = pixel_proposals.begin();
//       label_it != pixel_proposals.end(); label_it++)
//         new_pixel_proposals.push_back(*label_it + *segment_it *
//         pow(num_surfaces + 1, num_layers - 1 - layer_it->first));
//     pixel_proposals = new_pixel_proposals;
//   }
//   return pixel_proposals;
// }

void ProposalDesigner::getUpsamplingProposal(
    const Mat &ori_image, const vector<double> &ori_point_cloud,
    const vector<double> &ori_normals,
    const vector<double> &ori_camera_parameters,
    vector<vector<int>> &proposal_labels, int &proposal_num_surfaces,
    map<int, Segment> &proposal_segments, const int num_dilation_iterations) {
  ...
  vector<vector<bool>> segment_masks(current_solution_num_surfaces_,
                                     vector<bool>(NUM_PIXELS_, false));
  vector<int> visible_segmentation(NUM_PIXELS_, -1);
  vector<set<int>> layer_surface_ids_vec(NUM_LAYERS_);
  vector<vector<bool>> layer_empty_masks(NUM_LAYERS_ - 1,
                                         vector<bool>(NUM_PIXELS_, false));
  for (int pixel = 0; pixel < NUM_PIXELS_; pixel++) {
  ...
  vector<vector<int>> upsampled_segment_visible_pixels(
      current_solution_num_surfaces_);
  vector<vector<bool>> upsampled_segment_masks(
      current_solution_num_surfaces_, vector<bool>(ORI_NUM_PIXELS, false));
  vector<vector<bool>> upsampled_layer_empty_masks(
      NUM_LAYERS_ - 1, vector<bool>(ORI_NUM_PIXELS, false));
  for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
    int x = min(static_cast<int>(round(1.0 * (ori_pixel % ORI_IMAGE_WIDTH) /
                                       ORI_IMAGE_WIDTH * IMAGE_WIDTH_)),
                IMAGE_WIDTH_ - 1);
    int y = min(static_cast<int>(round(1.0 * (ori_pixel / ORI_IMAGE_WIDTH) /
                                       ORI_IMAGE_HEIGHT * IMAGE_HEIGHT_)),
                IMAGE_HEIGHT_ - 1);
    int pixel = y * IMAGE_WIDTH_ + x;
    upsampled_segment_visible_pixels[visible_segmentation[pixel]].push_back(
        ori_pixel);
    for (int segment_id = 0; segment_id < current_solution_num_surfaces_;
         segment_id++)
      upsampled_segment_masks[segment_id][ori_pixel] =
          segment_masks[segment_id][pixel];
    for (int layer_index = 0; layer_index < NUM_LAYERS_ - 1; layer_index++)
      upsampled_layer_empty_masks[layer_index][ori_pixel] =
          layer_empty_masks[layer_index][pixel];
  }

  proposal_segments.clear();
  for (int segment_id = 0; segment_id < current_solution_num_surfaces_;
       segment_id++)
    proposal_segments[segment_id] =
        upsampleSegment(current_solution_segments_.at(segment_id), ori_image,
                        ori_point_cloud, ori_normals, ori_camera_parameters,
                        upsampled_segment_visible_pixels[segment_id]);
  ...
  for (int ori_pixel = 0; ori_pixel < ORI_NUM_PIXELS; ori_pixel++) {
    ...
    for (int layer_index = 0; layer_index < NUM_LAYERS_; layer_index++) {
      for (set<int>::const_iterator segment_it =
               layer_surface_ids_vec[layer_index].begin();
           segment_it != layer_surface_ids_vec[layer_index].end();
           segment_it++) {
    ...
    proposal_labels[ori_pixel] =
        calcPixelProposals(proposal_num_surfaces, pixel_layer_surfaces_map);
    // if (proposal_labels[ori_pixel].size() > 2)
    //   cout << ori_pixel << '\t' << proposal_labels[ori_pixel].size() << endl;
  }

  // vector<bool> layer_segment_indicators(proposal_num_surfaces * num_layers_,
  // false);
  // for (int layer_index = 0; layer_index < num_layers_; layer_index++)
  //   for (set<int>::const_iterator segment_it =
  //   layer_surface_ids_vec[layer_index].begin(); segment_it !=
  //   layer_surface_ids_vec[layer_index].end(); segment_it++)
  //     layer_segment_indicators[layer_index * proposal_num_surfaces +
  //     *segment_it] = true;
...
void ProposalDesigner::writeSolution(
    const std::pair<double, LayerLabelSpace> &solution, const int thread_index,
    const int iteration, const std::vector<int> &selected_threads) {
  time_t timer;
  time(&timer);
  struct tm today = {0};
  ...
  LOG(INFO) << time << '\t' << iteration << '\t' << thread_index << '\t'
            << solution.first << endl;

  solutions_.push_back(
      Solution(0, time, solution.first, -1, selected_threads, solution.second));
  for (vector<LayerLabelSpace>::const_iterator proposal_space_it =
           proposal_spaces_.begin();
       proposal_space_it != proposal_spaces_.end(); proposal_space_it++)
    solutions_.push_back(
        Solution(proposal_space_it - proposal_spaces_.begin() + 1, time, -1, -1,
                 vector<int>(), *proposal_space_it));
...
Mat Solution::drawSolutionImage(const Mat &image) {
  ...
  for (int layer_index = 0; layer_index < NUM_LAYERS; layer_index++) {
    ...
    for (int pixel = 0; pixel < NUM_PIXELS; pixel++)
      surface_ids[pixel] = solution_space.getLabelOfNode(pixel)[0] /
      ...
      if (surface_id != SOLUTION_NUM_SURFACES) {
        ...
        for (int c = 0; c < 3; c++)
          blended_color[c] = min(image_color[c] * BLENDING_ALPHA +
                                     segment_color[c] * (1 - BLENDING_ALPHA),
                                 255.0);
        layer_mask_image.at<Vec3b>(y, x) = blended_color;
      }

      vector<int> neighbor_pixels =
          findNeighbors(pixel, IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  Mat multi_layer_image(
      image.rows + BORDER_WIDTH * 2,
      (image.cols + BORDER_WIDTH * 2 + IMAGE_PADDING) * (NUM_LAYERS), CV_8UC3);
  multi_layer_image.setTo(Scalar(255, 255, 255));
  for (int layer_index = 0; layer_index < NUM_LAYERS; layer_index++) {
    Mat layer_image_with_border = Mat::zeros(
        image.rows + BORDER_WIDTH * 2, image.cols + BORDER_WIDTH * 2, CV_8UC3);
    Mat layer_image_region(
        layer_image_with_border,
        Rect(BORDER_WIDTH, BORDER_WIDTH, image.cols, image.rows));
